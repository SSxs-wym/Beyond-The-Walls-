...
const PIPE_NAVIGATION_MAP_STORAGE_KEY = "shawshank_pipe_navigation_map_v1";
...
const OPENING_NOTICE_DURATION = 5;
...
const OPERATING_ROOM_BLACKOUT_MESSAGE = "发生了什么？我不是从大门逃出去了吗....怎么会出现在手术台上？.......难道这里还藏着不为人知的秘密";
...
const TextData = {
  ...
  openingPrelude: [
    {
      title: "",
      body: "故事始于一场无人愿意再提起的风波。银行家安迪因为一份指向自己的“答案”，被送进黑墙庄园。从此，铁门与重复的脚步声，成为生活的全部。有人逐渐习惯规则，也有人慢慢忘记了墙外的模样。而安迪的故事，才刚刚开始。"
    },
    {
      title: "",
  ...
  openingQuote: "那一天之后，银行家安迪的人生被彻底改写，故事也从黑墙庄园开始。",
  whiteLightLines: [
    "电视的声音……是不是离我越来越近了？",
...
const SideRouteBrooksDialogueLines = [
  ...
  "安迪：混乱会让守备出现空当。",
  "老布（声音很轻）：也许。但别把机会当成保证，壁垒最会让人空欢喜。",
  "安迪（心理）：只要撑到明天，我就可能找到另一条离开壁垒的路。"
];
...
const OutsidePlanDialogueLines = [
  "安迪（回望壁垒）：我曾经以为，所有出口都需要一张画了二十年的地图。",
...
const CorridorDeveloperMessageLines = [
  ...
  "开发者：谢谢你记得那些没有走出壁垒的人。也愿屏幕前的你，永远保留走向墙外的勇气。"
];
...
const AchievementDefinitions = [
  ...
  { id: "four_corners", title: "壁垒的四个角", description: "踏遍监狱院子的四个角落。", icon: "four_corners.webp" },
  { id: "outside_the_plan", title: "计划之外", description: "在既定计划之外找到另一条离开壁垒的路。", icon: "outside_the_plan.webp" },
  { id: "person_behind_door", title: "门后的人", description: "在走廊尽头想起那些没有离开壁垒的人。", icon: "person_behind_door.webp" }
];
  ...
  renderPanel(ctx) {
    ...
    AchievementDefinitions.forEach((definition, index) => {
      ...
      if (unlocked && image && image.complete && image.naturalWidth > 0) {
        ctx.drawImage(image, x + 12, y + 16, 164, 164);
      } else {
        ctx.fillStyle = "#050505";
        ctx.fillRect(x + 12, y + 16, 164, 164);
        ctx.fillStyle = "#555";
        ctx.font = "bold 46px monospace";
        ctx.fillText("?", x + cardWidth / 2, y + 114);
      }
      ctx.fillStyle = unlocked ? "#f3dfaa" : "#777";
      ctx.font = "bold 17px 'Microsoft YaHei', monospace";
      ctx.fillText(unlocked ? definition.title : "隐藏成就", x + cardWidth / 2, y + 218);
    });
...
const RoamingNpcConfigs = [
  {
    ...
    travelChance: 0.26, allowedScenes: ["yard", "library", "cellCorridor", "cell"],
    yardWeights: { library: 70, cellCorridor: 30 },
    corridorWeights: { yard: 80, cell: 20 }
  },
  {
    id: "haywood", assetKey: "haywood", name: "海伍德", role: "prisoner", source: "named", sourceIndex: 1,
    travelChance: 0.32, allowedScenes: ["yard", "library", "cellCorridor", "cell"],
    yardWeights: { library: 30, cellCorridor: 70 },
    corridorWeights: { yard: 65, cell: 35 }
  },
  {
  ...
  },
  {
    ...
    travelChance: 0.34, allowedScenes: ["yard", "library", "cellCorridor", "cell", "solitary"],
    yardWeights: { library: 5, cellCorridor: 95 },
    corridorWeights: { yard: 45, cell: 35, solitary: 20 }
  },
  {
    id: "guard_2", assetKey: "guard", name: "狱警", role: "guard", source: "guard", sourceIndex: 1,
    travelChance: 0.37, allowedScenes: ["yard", "library", "cellCorridor", "cell", "solitary"],
    yardWeights: { library: 10, cellCorridor: 90 },
    corridorWeights: { yard: 55, cell: 20, solitary: 25 }
  },
  {
    id: "guard_3", assetKey: "guard", name: "狱警", role: "guard", source: "guard", sourceIndex: 2,
    travelChance: 0.40, allowedScenes: ["yard", "library", "cellCorridor", "cell", "solitary"],
    yardWeights: { library: 15, cellCorridor: 85 },
    corridorWeights: { yard: 50, cell: 30, solitary: 20 }
  }
];
...
const OpeningNoticeSystem = {
  ...
  init() {
    ...
    this.elements = {
      root,
      seconds: document.getElementById("openingNoticeSeconds"),
      progress: document.getElementById("openingNoticeProgress"),
      progressFill: document.getElementById("openingNoticeProgressFill")
    };
    this.hide();
  },
  ...
  sync() {
    ...
    const progress = clamp(this.remaining / OPENING_NOTICE_DURATION, 0, 1);
    this.elements.root.hidden = false;
    this.elements.root.setAttribute("aria-hidden", "false");
    this.elements.seconds.textContent = String(Math.max(1, Math.ceil(this.remaining)));
    this.elements.progress.setAttribute("aria-valuenow", this.remaining.toFixed(1));
    this.elements.progressFill.style.transform = "scaleX(" + progress.toFixed(4) + ")";
  },
  ...
  hide() {
    ...
    this.elements.seconds.textContent = String(OPENING_NOTICE_DURATION);
    this.elements.progress.setAttribute("aria-valuenow", String(OPENING_NOTICE_DURATION));
...
function changeScene(sceneName) {
  ...
  // 暂停只是覆盖层：进入和退出时不重新执行游戏场景的 enter/exit，
...
const QuestSystem = {
  labels: {
    ...
    quest_recap: "剧情分镜正在展开。点击画面或按空格键逐页推进，先看清这座拘禁地的规则。",
    quest_white_light: "白光正在吞没客厅。稍等片刻，你会在电影世界里拘禁地院子上方的锁门口醒来。",
    quest_yard_first_walk: "我先四处探索一下吧，看看怎么离开这里...",
    quest_gate_blackout: "穿过拘禁地大门后，眼前陷入黑暗。",
    quest_operating_room_1: "意识在冰冷的手术室里浮沉。",
    quest_operating_room: "痛觉与恐惧一同逼近。",
    quest_operating_room_blackout: "一切再度陷入黑暗。",
    quest_cell_wake_question: "这里是牢房吗？",
    quest_cell_wake_explore: "那我先探索一下吧！",
    quest_corridor_to_yard: "沿牢房走廊向下，从最下方的门回到院子。",
    quest_corridor_to_yard_morning: "穿过走廊回到院子，观察大门附近的狱警。",
    quest_corridor_escort_yard: "两名狱警正把你从房间押往院子，无法反抗。",
    quest_corridor_escort_solitary: "两名狱警正把你押向走廊上方的小黑屋。",
    ...
    quest_cell_prepare_confiscation_inspection: "典狱长要检查房间。石锤还在身上，保持镇定。",
    ...
    quest_cell_inspection: "典狱长正在检查房间。保持镇定，点击画面或按空格键推进对话。",
    quest_cell_inspection_wait: "典狱长和狱警正在进门。站在床边别乱动，别让锤子暴露。",
    quest_cell_inspection_leave: "检查房间已经通过。等典狱长和狱警离开，再继续行动。",
    quest_cell_hammer_confiscated: "石锤已被没收。两名狱警正把你拖出房间。",
    ...
    quest_cell_map_obtained: "典狱长办公室在拘禁地左上方",
    ...
    quest_pipe_opened: "管道被你凿开了！从破口落入下方，寻找出路。",
    quest_pipe_network: "沿管道中心线前进，从地图右上角紧贴右侧的出口离开。",
    quest_gate_escape: "你穿过了拘禁地大门。",
    quest_yard_cell_door: "这里通向房间。靠近右侧建筑的房间入口，点击右下按钮或按空格键返回房间。",
    quest_yard_free_time: "在院内自由活动，检查人物与物品，继续收集能够相互印证的线索。",
    quest_yard_warden_office: "这里是左上角的典狱长办公室入口。拿到地图后，账本调包会从这里开始。",
    quest_yard_red: "找到瑞德",
    ...
    quest_yard_soil: "在偏右侧椭圆深棕色土地中倒土。每次长按 5 秒；第 3 秒最近狱警会靠近。",
    quest_yard_soil_partial: "谨慎倒土，当心被狱警发现！",
    quest_yard_soil_done: "土已经处理干净。回到右侧房间入口，躺到床上推进二十年计划。",
    quest_side_route_crowd: "两名狱警正把你押往院子中央。保持原地，不要反抗。",
    quest_side_route_warden_address: "典狱长正在当众宣告对你的惩罚。",
    ...
    quest_side_route_morning_patrol: "三名狱警都守在大门附近。先别贸然离开，去左侧典狱长办公室看看。",
    quest_side_route_gate_caught: "脱困被发现，狱警正向你围来。",
    quest_side_route_office_search: "办公室没人。走到典狱长书桌前，翻找可疑记录。",
    quest_side_route_office_hide: "有人要进来！十秒内躲到上方窗帘后面。",
    quest_side_route_office_listen: "躲在窗帘后，听典狱长和狱警交谈。",
    quest_side_route_office_search_again: "典狱长和狱警已经离开。回到书桌前继续翻找。",
    quest_side_route_office_leave: "他们离开后，必须带着证据设法脱离控制举报典狱长。",
    quest_side_route_awaiting_continuation: "新的脱困路线已经有了线索，等待下一段剧情。",
    quest_yard_reserved_interaction: "这里暂时没有新的线索。回到当前目标地点，继续推进脱困计划。",
    quest_office_first_warning: "典狱长发现了你。听完他的警告，立刻离开办公室。",
    ...
    quest_office_warden: "与典狱长交谈",
    quest_office_warden_leaving: "典狱长正在离开办公室，等他走远后再搜寻。",
    quest_office_embroidery: "典狱长已离开。去检查左侧的刺绣，寻找隐藏的保险柜。",
  ...
  render(ctx) {
    ...
    const emphasizeWakeExplore = GameState.currentQuest === "quest_cell_wake_explore";
    renderQuestText(ctx, text, emphasizeWakeExplore ? {
      fontSize: CELL_WAKE_EXPLORE_PROMPT_FONT_SIZE
    } : null);
  }
};
...
const CheckpointSystem = {
  ...
  load() {
    try {
      ...
      const record = JSON.parse(serialized);
      if (record && [2, 3, 4, 5, 6].includes(record.version) && record.checkpoint === legacyCheckpoint) {
...
function applyCheckpointProgress(checkpointKey, storedProgress) {
  ...
  Object.keys(persistentFields).forEach((field) => {
    if (field === "soilDumpCompletedCount" || field === "sideRouteActive" ||
      field === "sideRouteStage" || field === "sideTalk" || field === "yardMap" || field === "pipeMap") {
      return;
    }
  ...
  Object.keys(combined).forEach((field) => {
    if (field !== "sideTalk" && Object.prototype.hasOwnProperty.call(GameState, field)) {
...
function getActiveCellInteraction() {
  ...
  return getNearbyRoamingNpcInteraction("cell", foot);
}
...
function updatePipePostSmash(dt) {
  updatePipePlayer(dt);
  if (isPipePlayerAtNetworkEntry()) {
    startPipeDropToCrawl();
  }
}

function isPipePlayerAtNetworkEntry() {
  const pipe = GameState.pipe;
  const centerX = pipe.playerX + pipe.playerW / 2;
  return centerX >= PipeLayout.networkEntryZone.x &&
    centerX <= PipeLayout.networkEntryZone.x + PipeLayout.networkEntryZone.w;
}
...
function restorePipeDrownFailureToOpenedPipe() {
  ...
  pipe.drownTimer = 0;
  pipe.playerX = clamp(
    PipeLayout.networkEntryZone.x - pipe.playerW - 12,
...
const MINI_GAME_ASSET_VERSION = String(window.BeyondWallsAssetVersion || "dev");

function getVersionedMiniGameAssetUrl(assetPath) {
  const url = new URL(assetPath, document.baseURI);
  url.searchParams.set("v", MINI_GAME_ASSET_VERSION);
  return url.href;
}
...
const MiniGameHost = {
  ...
  async mountComponent(type, config, token) {
    ...
    fontStyle.href = getVersionedMiniGameAssetUrl("../shared/mini-games-font.css");
    const style = document.createElement("link");
    style.rel = "stylesheet";
    style.href = getVersionedMiniGameAssetUrl(config.style);
...
function getActiveYardInteraction() {
  ...
  // 倒土任务期间，泥地区域拥有最高交互优先级；站在区域内不会误触附近人物或狱警对话。
  const soilInteraction = getYardInteraction("soil");
  ...
  if (npcInteraction && npcInteraction.id === "brooks" && getPendingBrooksMainDialogueKind()) {
...
function getOffscreenGuardSpawnScene() {
  return ["cellCorridor", "yard", "library", "cell", "solitary"]
...
function getRoamingDestinationOptions(agent) {
  ...
  if (agent.scene === "cellCorridor") {
    return Object.assign({}, config.corridorWeights);
  }
...
function isRoamingTransitionAllowed(agent, destinationScene) {
  const config = getRoamingNpcConfig(agent.roamingId);
  return Boolean(config && config.allowedScenes.includes(destinationScene) &&
...
function renderCellEntities(ctx) {
  ...
  const playerFoot = canvasPointToCellImage(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  const entities = [{ type: "player", y: playerFoot ? playerFoot.y : 0 }];
  getRoamingNpcsInScene("cell").forEach((agent) => {
  ...
  entities.forEach((entity) => {
    if (entity.type === "player") {
      ...
      renderRoamingNpc(ctx, entity.agent, cellImageToCanvasPoint);
    }
  });
}
...
function renderPlayerOverheadThought(ctx, text, alpha) {
  ...
  const x = clamp(
    ...
    CANVAS_WIDTH - canvasMargin - widestLine / 2
  );
...
function renderOperatingRoomBlackoutMessage(ctx, text, x, y, maxWidth, lineHeight) {
  ...
  const secretColor = getOperatingRoomSecretColor(easedFadeRatio);
  ...
  lines.forEach((currentLine, lineIndex) => {
    ...
    if (secret && lineEnd > secretStart) {
      ...
      ctx.fillText(secret, drawX, drawY);
      drawX += ctx.measureText(secret).width;
    }
...
function getOperatingRoomSecretColor(progress) {
...
function renderPipeInteractionHints(ctx) {
  if (GameState.pipe.smashCompleted && GameState.pipe.phase === "smash") {
    const exitRect = pipeImageRectToCanvas(PipeLayout.networkEntryZone);
    ctx.save();
    ctx.strokeStyle = "#f5df9d";
    ctx.lineWidth = 4;
