...
function k() {
	...
	let e = new URLSearchParams(window.location.search), t = __bwmMountOptions || {};
	return {
		seed: t.seed || e.get("seed") || Date.now(),
		embedded: typeof t.embedded === "boolean" ? t.embedded : e.get("embed") === "1",
		nextDestination: t.nextDestination || e.get("next")
	};
}
function he(e, t) {
	let n = {
...
function De() {
	...
	x.current = Ve, (0, _.useEffect)(() => {
		...
		function handleKeyDown(event) {
			...
			// 集成版的 WASD 已由主游戏统一转发；组件再次处理会让一次
	...
	return /* @__PURE__ */ (0, T.jsxs)("main", {
		className: `pipe-game-shell ${e.embedded ? "is-embedded" : ""}`,
		children: [
