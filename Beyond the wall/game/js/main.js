...
const PlaystyleSystem = window.BeyondWallsPlaystyle;
if (!PlaystyleSystem) {
  throw new Error("Player behavior scoring is unavailable.");
}
// Image and audio URLs resolve against the root index.html base URL, not this script file.
...
const GAME_BGM_RESUME_KEY = "beyond_walls_game_bgm_time_v1";
const CHECKPOINT_STORAGE_KEY = "beyond_walls_checkpoint_v1";
const SAVE_DATA_STORAGE_KEY = "beyond_walls_save_v2";
const YARD_NAVIGATION_MAP_STORAGE_KEY = "beyond_walls_yard_navigation_map_v1";
const PIPE_MAZE_BLUEPRINT_STORAGE_KEY = "pipe-maze-blueprint";
const PIPE_NAVIGATION_MAP_STORAGE_KEY = "beyond_walls_pipe_navigation_map_v1";
const ACHIEVEMENT_STORAGE_KEY = "beyond_walls_achievements_v1";
const LEGACY_STORAGE_KEYS = Object.freeze({
  [GAME_BGM_RESUME_KEY]: "shawshank_game_bgm_time",
  [CHECKPOINT_STORAGE_KEY]: "shawshank_pixel_escape_checkpoint",
  [SAVE_DATA_STORAGE_KEY]: "shawshank_pixel_escape_save_v2",
  [YARD_NAVIGATION_MAP_STORAGE_KEY]: "shawshank_yard_navigation_map_v1",
  [PIPE_NAVIGATION_MAP_STORAGE_KEY]: "shawshank_pipe_navigation_map_v1"
});

function reportNonFatalError(context, error) {
  console.warn(`[高墙之外] ${context}`, error);
}

function reportExpectedBrowserLimitation(context, error) {
  console.debug(`[高墙之外] ${context}`, error);
}

function migrateLegacyStorageKeys() {
  try {
    Object.entries(LEGACY_STORAGE_KEYS).forEach(([currentKey, legacyKey]) => {
      if (localStorage.getItem(currentKey) !== null) return;
      const legacyValue = localStorage.getItem(legacyKey);
      if (legacyValue !== null) localStorage.setItem(currentKey, legacyValue);
    });
  } catch (error) {
    reportNonFatalError("旧版本地存档迁移失败", error);
  }
}

migrateLegacyStorageKeys();

function getStorageKeysIncludingLegacy(...currentKeys) {
  return [...new Set(currentKeys.flatMap((key) => [key, LEGACY_STORAGE_KEYS[key]].filter(Boolean)))];
}

function removeStorageKeysIncludingLegacy(...currentKeys) {
  getStorageKeysIncludingLegacy(...currentKeys).forEach((key) => localStorage.removeItem(key));
}
// 复杂玩法使用任务牌；关键普通目标改为一次性的安迪头顶心理提示。
...
const OPENING_NOTICE_DURATION = 3;
...
const GATE_BLACKOUT_MESSAGE = "我真的逃走了吗....";
const GATE_BLACKOUT_TYPEWRITER_CHAR_SECONDS = 0.18;
const OPERATING_ROOM_DISPLAY_SECONDS = 0.5;
const OPERATING_ROOM_BLACKOUT_SECRET = "不为人知的秘密？";
const OPERATING_ROOM_BLACKOUT_MESSAGE = "发生了什么？我不是从大门逃出去了吗....怎么会出现在手术台上？.......难道这里还藏着" + OPERATING_ROOM_BLACKOUT_SECRET;
const OPERATING_ROOM_BLACKOUT_TYPEWRITER_CHAR_SECONDS = 0.045;
const OPERATING_ROOM_BLACKOUT_SECRET_FADE_SECONDS = 2.8;
const OPERATING_ROOM_BLACKOUT_HOLD_SECONDS = 1.2;
const OPERATING_ROOM_BLACKOUT_SECRET_GRADIENT_CHARACTERS = 5;
const OPERATING_ROOM_BLACKOUT_SECRET_GRADIENT_RATE = 2.5;
const CELL_WAKE_EXPLORE_PROMPT_SECONDS = 5;
const CELL_WAKE_EXPLORE_PROMPT_FONT_SIZE = 27;
...
// NPC 坐标使用院子底图坐标。中心相隔两个标准人物宽度时，
// 两个侧身精灵之间会留出约一个人的空位。
const AMBIENT_CHAT_STANDING_DISTANCE = YARD_CHARACTER_FALLBACK_WIDTH * 2;
...
const ROAMING_NPC_NAV_GRID_SIZE = 32;
const ROAMING_NPC_ROUTE_SAMPLE_DISTANCE = 2;
const ROAMING_NPC_SOFT_SPACING = 48;
const ROAMING_NPC_TRAVEL_MIN_SECONDS = 20;
const ROAMING_NPC_TRAVEL_MAX_SECONDS = 60;
const ROAMING_NPC_TRAVEL_TIMEOUT_SECONDS = 12;
const ROAMING_NPC_TRAVEL_RETRY_SECONDS = 5;
const ROAMING_NPC_PORTAL_LOCK_SECONDS = 1;
const ROAMING_GUARD_MIN_COUNT = 3;
const ROAMING_GUARD_MAX_COUNT = 8;
const ROAMING_GUARD_SPAWN_SPACING = 160;
const ROAMING_GUARD_TARGET_SPACING = 140;
const ROAMING_GUARD_CROWD_DISTANCE = 96;
const ROAMING_GUARD_CROWD_SECONDS = 3;
const ROAMING_GUARD_SOIL_BUFFER = 48;
const ROAMING_GUARD_GATE_BUFFER = 64;
const ROAMING_GUARD_COUNT_WEIGHTS = [
  { count: 3, weight: 0.10 },
  { count: 4, weight: 0.15 },
  { count: 5, weight: 0.25 },
  { count: 6, weight: 0.25 },
...
const OUTSIDE_PLAN_RUN_SECONDS = 0.65;
const OUTSIDE_PLAN_BLACK_SECONDS = 0.75;
const CORRIDOR_DOOR_MEMORY_IDLE_SECONDS = 3;
const CORRIDOR_DEVELOPER_HOLD_SECONDS = 5;
...
const OFFICE_INSPECTION_HIDE_SECONDS = 10;
...
// The supplied pipe map is intentionally much larger than the viewport. Andy keeps
// his courtyard screen size while the artwork is enlarged around him.
const PIPE_NETWORK_WORLD_SCALE = 4;
const PIPE_NETWORK_MOVE_SPEED = PLAYER_SPEED;
const PIPE_NETWORK_PLAYER_W = Math.round(52 * YARD_CHARACTER_RENDER_SCALE);
const PIPE_NETWORK_PLAYER_H = Math.round(108 * YARD_CHARACTER_RENDER_SCALE);
// Only Andy's chest and abdomen constrain him to the route. His head, arms and
// legs may extend beyond it, while the hidden route point must remain covered by
// this torso rectangle.
const PIPE_NETWORK_TORSO_W = Math.round(PIPE_NETWORK_PLAYER_W * 0.46);
const PIPE_NETWORK_TORSO_H = Math.round(PIPE_NETWORK_PLAYER_H * 0.34);
const PIPE_NETWORK_TORSO_HALF_W = PIPE_NETWORK_TORSO_W / 2;
const PIPE_NETWORK_TORSO_HALF_H = PIPE_NETWORK_TORSO_H / 2;
const PIPE_NETWORK_TORSO_CENTER_Y_OFFSET = Math.round(PIPE_NETWORK_PLAYER_H * 0.02);
const PIPE_NETWORK_VISION_INNER_RADIUS = 190;
const PIPE_NETWORK_VISION_OUTER_RADIUS = 350;
// Players may press the next direction before reaching a corner. The buffered
// turn completes the remaining approach and transfers them onto the selected
// branch while the route remains covered by Andy's torso.
const PIPE_NETWORK_TURN_ASSIST_DISTANCE = 120;
const PIPE_NETWORK_TURN_BUFFER_SECONDS = 0.65;
const PIPE_NETWORK_TURN_MIN_DOT = 0.18;
const PIPE_NAVIGATION_MAP_VERSION = 1;
const PIPE_NAVIGATION_MAP_LAYOUT_ID = "fixed-pipe-network-v1";
const PIPE_NAVIGATION_MAP_PERSIST_INTERVAL_SECONDS = 0.75;
const PIPE_NAVIGATION_MAP_THUMBNAIL_WIDTH = 213;
const PIPE_NAVIGATION_MAP_THUMBNAIL_HEIGHT = 320;
const PIPE_NAVIGATION_MAP_SOURCE_LINE_WIDTH = 96;
...
const AssetManifest = {
  images: {
    ...
    pipe_network: "pipe_network.webp",
    pipe_network_map_thumbnail: "pipe_network_map_thumbnail.webp",
    ...
    dialogue_developer: "dialogue_developer.webp",
...
const TextData = {
  ...
  openingPrelude: [
    {
      title: "",
      body: "故事始于一场无人愿意再提起的风波。银行家安迪因为一份指向自己的“答案”，被送进监狱。从此，铁门与重复的脚步声，成为生活的全部。有人逐渐习惯规则，也有人慢慢忘记了墙外的模样。而安迪的故事，才刚刚开始。"
    },
    {
      title: "",
      body: "安迪从未真正接受这里的生活。瑞德、老布，以及几件不起眼的物品，悄悄改变了他的计划。一条无人察觉的路，也正在慢慢成形。"
    }
  ],
  openingQuote: "那一天之后，银行家安迪的人生被彻底改写，故事也从监狱开始。",
  whiteLightLines: [
    "电视的声音……是不是离我越来越近了？",
    "不对，那不是电影的声音，好像就在这个房间里。",
    "这里是……电视里的监狱？"
  ],
  pauseTitle: "暂停",
  pauseHint: "点击右上角继续，或按 Esc / P · 点击下方查看成就",
  failTitle: "失败",
...
const RedDialogueLines = [
  "安迪：听说在这里想弄到东西，得来找你。",
  "瑞德（打量着安迪）：我偶尔替人想想办法。你要什么？",
  "安迪：一把小石锤，地质锤那么大，打磨石头用。",
  "瑞德：金属家伙风险不小。你不会打算靠它挖穿这堵墙吧？",
  "安迪（淡淡一笑）：真靠它挖出去，恐怕得花上几百年。我只是喜欢石头。",
  "瑞德：行。先替我修好收音机，我就替你找。"
];

const CellWakeThoughtLines = [
  "安迪（心理）：我穿越到电影里了？我要脱离控制！还要揭露这该死的拘禁地。",
  "安迪（心理）：我要先出去找找瑞德。"
];
...
const OpeningGateApproachThought = "这是大门吗？我是不是可以从这里逃出去...";

const RedEvidenceDialogueLines = [
  "瑞德（压低声音）：收音机修好了，我欠你一次。三笔钱打给“东港建材”“哈德森修缮”和“格林运输”，可院里从没见过这些人。",
  "安迪：空壳公司。结算日期呢？",
  "瑞德：六月十二日、七月三日、七月十九日；每次当晚，后门都有车出去。我不会留下任何纸。",
  "安迪：我会记住，再拿这些日期去核对别的线索。"
];

const TommyEvidenceDialogueLines = [
  "汤米（声音放得很低）：有件事我一直压在心里。以前我替档案室搬过一批写着“转移”的牛皮纸袋。",
  "汤米：每个袋子都贴着被困者编号，却没有接收拘禁地的盖章；狱警只说“送去 D 区”，不许我问。",
...
const HaywoodEvidenceDialogueLines = [
  "海伍德（看了眼远处的岗楼）：夜里总有一辆没有拘禁地标记的灰色卡车从后门出去，左后门少了一块漆。",
...
const YardGuardConversationLines = [
  "狱警（压低声音）：别在那片深色泥地停太久。夜班的“转送”前，总要先把那边清出来。",
  "狱警：名单上多一行少一行不归我管，可有些人走了，连个去处都没有。",
  "狱警：上头只要数字对得上，谁还会问那些空出来的床位？"
];

const GateGuardConversationLines = [
  "狱警（望着大门外）：上级来访还得装得像样一点。昨晚后门那辆没编号的车，又把值班表弄得一团糟。",
  "狱警：有人说是正常转送，可门禁簿上连接收地点都没写。上面的人总爱让下面的人替他们对数字。",
  "狱警：你最好当没听见。这里有些人一出门，名字就再也不会回到名册上。"
];
...
const AmbientSocialPoints = {
  // 老布的默认活动始终贴着图书馆门口；来找他闲聊的人也在门边会合。
  libraryForecourt: { x: 238, y: 1124 },
...
const RedPosterDialogueLines = [
  ...
  "瑞德：海报？那东西搜房时可藏不住。",
  "安迪：这间屋子，总得有一点像人住的地方。",
  "瑞德（会意一笑）：有道理。女明星，还是别的？"
];

const RedPosterDeliveryLines = [
  "瑞德环顾四周确认没有狱警注视，悄悄从衣襟内拿出纸筒，快速塞到安迪手里。",
  "瑞德：卷紧藏好，别让狱警看见。",
  "安迪：多谢。"
];
...
const RedHammerDeliveryLines = [
  "收音机重新响起微弱的电流声。",
  "瑞德：手艺不错。答应你的石锤，收好；图书馆的老布那边，我也替你打过招呼了。",
  "安迪：谢了，瑞德。我去找他。"
];
...
const BrooksLibraryOpeningLines = [
  "老布（低头整理书箱）：瑞德说你会来。你想从图书馆带走什么，新来的？",
  "安迪：一本属于自己的圣经。夜里读，也好让心静下来。",
  "老布：私人书不外借。可这些书架乱得厉害，我一个人收拾不完。",
  "安迪：我替您整理，您把圣经给我。",
  "老布（露出一点笑意）：书要有耐心，人也一样。做好了，它就是你的。",
  "安迪：成交，布鲁克斯先生。"
];

const BrooksBibleDeliveryLines = [
  "—— 一段时间后，图书整理完毕 ——",
  "老布（双手递出一本圣经）：书架终于有了样子。答应你的圣经，它归你，不用还。",
  "安迪：谢谢您，布鲁克斯先生。",
  "老布：慢慢读。有些答案，藏在字缝里才安全。"
];

const InspectionConfiscationDialogueLines = [
  "典狱长：例行检查房间，所有人站到床边，不许乱动，不许藏匿违禁品！",
  "狱警：报告典狱长，在该被困者的物品中发现一把石锤。",
  "典狱长（盯着安迪）：一把石锤？你打算用它做什么？",
  "安迪：只是雕刻时用的工具。",
  "典狱长：在拘禁地，被困者不需要这种工具。没收。",
  "典狱长：把他拖出房间。我会亲自问清楚。",
  "狱警：是，典狱长。"
];

const SideRouteWardenAddressLines = [
  "狱警：人已经带到了。从他的房间里搜出了这把违禁的石锤。",
  "典狱长（面向围观的被困者）：都看清楚了。就在安迪的房间里，我发现了一把石锤。",
  "典狱长：他心怀不轨，私藏违禁物，想把拘禁地的规矩当成摆设。",
  "典狱长：任何人敢效仿他，都会知道后果。",
  "典狱长：把他送进密闭房间。"
];
...
const SideRouteBrooksDialogueLines = [
  "老布（看着安迪手腕上的红痕）：石锤若藏在圣经里，也许昨晚就躲过搜查了。拿着，这本给你。",
  "安迪：谢谢你，老布。",
  "老布：我明天脱离拘禁，上级也会来巡视。院门和办公室会忙起来，狱警也会被调走。",
  "安迪：混乱会让守备出现空当。",
  "老布（声音很轻）：也许。但别把机会当成保证，高墙最会让人空欢喜。",
  "安迪（心理）：只要撑到明天，我就可能找到另一条离开高墙的路。"
];

const SideRouteOfficeEvidenceLines = [
  "你翻开典狱长桌上的文件夹，里面夹着一叠被反复涂改的转运记录。",
  "记录里有几名没有身份编号的“转运对象”，姓名与拘禁地名册完全对不上。",
  "安迪（心理）：这些不是正常的调动……典狱长在利用拘禁地贩卖人口，还伪造了去向。",
  "门外响起了脚步声。有人来了！快躲起来！"
];

const SideRouteOfficeWardenLines = [
  "典狱长：哈哈哈，这些领导真好糊弄。他们今天还夸我把拘禁地治理得井井有条。",
  "典狱长：今天有一位领导差点走到房间内查人了；还好我聪明把他们引开了，没发现拘禁地人数与名单对不上的事情。",
  "狱警：还得是典狱长您英明。",
  "典狱长：那是。",
  "典狱长：哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈。"
];

const SideRouteOfficeSearchAgainLines = [
  "典狱长和狱警的脚步声渐远，办公室重新安静下来。",
  "安迪（心理）：趁他们离开，再把书桌翻一遍。"
];

const SideRouteOfficeFinalEvidenceLines = [
  "你继续翻找，在文件夹最底下找到了那把被没收的石锤。",
  "石锤旁压着完整的转运账目和伪造名单；这就是典狱长贩卖人口的实锤。",
  "安迪获得了石锤。",
  "安迪（心理）：看来典狱长真的在干一些见不得人的勾当。",
  "安迪（心理）：不行，我必须要脱离控制举报他。"
];

const OutsidePlanDialogueLines = [
  "安迪（回望高墙）：我曾经以为，所有出口都需要一张画了二十年的地图。",
  "安迪（心理）：但当机会真正出现时，自由不会询问你是否准备好了。"
];
...
const CorridorDoorMemoryLines = [
  "安迪（望向一扇扇紧闭的牢门）：这里的每一扇门后，都有人计算着失去的日子。",
  "安迪（心理）：有些人离开了牢房，却从来没有真正走出这里。"
];

const CorridorDeveloperMessageLines = [
  "开发者：如果你愿意在这里多停留一会，说明你看到的不只是安迪的故事。",
  "开发者：谢谢你记得那些没有走出高墙的人。也愿屏幕前的你，永远保留走向墙外的勇气。"
];
...
const GameState = {
  ...
  playTime: 0,
  playstyle: PlaystyleSystem.createState(),
  fatalError: null,
  ...
  corridor: {
    ...
    remarkText: "",
    remarkStartedAt: -1,
    doorMemoryIdleTimer: 0,
    doorMemoryViewedThisVisit: false,
    doorMemoryUnlockPending: false,
    developerMessageArmed: false,
    developerMessageHoldTimer: 0,
    developerMessageShownThisVisit: false,
    guards: [
  ...
  yardNavigationMap: {
    ...
    firstWalkIntroPending: false,
    postTutorialThoughtActive: false,
    postTutorialThoughtIndex: 0,
    postTutorialThoughtTimer: 0,
    expanded: false,
    revealedCells: [],
  ...
  yardPrisoners: [],
  roamingBackgroundPrisoners: [],
  roamingNpcSystem: {
    initialized: false,
    travelCooldown: 0,
    activeTravelerId: null,
    portalLocks: Object.create(null),
    guardTargetCount: 0,
    guardRollCheckpoint: null,
    guardRollReason: null,
    pendingRetireIds: []
  },
  redNpc: {
    initialized: false,
  ...
  pipeNetwork: {
    ...
    edgeProgress: 0,
    lastTravelSign: 1,
    pendingTurnEdgeIndex: -1,
    pendingTurnNodeId: null,
    turnBufferTimer: 0,
    bufferedInputX: 0,
    bufferedInputY: 0,
    routeX: 0,
  ...
  pipeNavigationMap: {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds: [],
    drawnEdgeIds: [],
    exploredEdgeRanges: {},
    expanded: false,
  ...
  unauthorizedOfficeEntrySnapshot: null,
...
const AchievementDefinitions = [
  ...
  { id: "beyond_evidence", title: "证据之外", description: "调包账本后调查典狱长的办公桌。", icon: "beyond_evidence.webp" },
  ...
  { id: "outside_the_plan", title: "计划之外", description: "在既定计划之外找到另一条离开高墙的路。", icon: "outside_the_plan.webp" },
  { id: "person_behind_door", title: "门后的人", description: "在走廊尽头想起那些没有离开高墙的人。", icon: "person_behind_door.webp" }
];
...
const AchievementPanelLayout = {
  closeButton: { x: 1210, y: 18, w: 46, h: 46 },
  columns: 6,
  cardWidth: 188,
  cardHeight: 246,
  columnGap: 20,
  rowGap: 29,
  gridStartY: 82,
  detailCard: { x: 86, y: 120, w: 410, h: 480 },
  detailContent: { x: 570, y: 170, w: 600 }
};
...
const AchievementSystem = {
  ...
  init() {
    try {
      ...
      reportNonFatalError("读取成就存档失败，已使用空成就记录", error);
      this.unlocked = Object.create(null);
    }
  ...
  unlock(id) {
    ...
    try {
      ...
      reportNonFatalError("非致命操作失败", error);
    }
  ...
  openPanel() {
    ...
    GameState.achievementDetailId = null;
  },
  ...
  getCardRect(index) {
    const layout = AchievementPanelLayout;
    const gridWidth = layout.columns * layout.cardWidth +
      (layout.columns - 1) * layout.columnGap;
    const gridStartX = (CANVAS_WIDTH - gridWidth) / 2;
    const column = index % layout.columns;
    const row = Math.floor(index / layout.columns);
    return {
      x: gridStartX + column * (layout.cardWidth + layout.columnGap),
      y: layout.gridStartY + row * (layout.cardHeight + layout.rowGap),
      w: layout.cardWidth,
      h: layout.cardHeight
    };
  },
  ...
  handlePanelInput() {
    const closePressed = InputSystem.actionPressed("pause");
    const pointerPressed = InputSystem.pointerJustPressed;
    const closeClicked = pointerPressed &&
      InputSystem.pointerInRect(AchievementPanelLayout.closeButton);

    if (GameState.achievementDetailId) {
      if (closePressed || closeClicked) {
        GameState.achievementDetailId = null;
      }
      return true;
    }
    ...
    if (pointerPressed) {
      const selectedIndex = AchievementDefinitions.findIndex((definition, index) =>
        this.isUnlocked(definition.id) && InputSystem.pointerInRect(this.getCardRect(index))
      );
      if (selectedIndex >= 0) {
        GameState.achievementDetailId = AchievementDefinitions[selectedIndex].id;
      }
    }
    return true;
  },
  ...
  clearSavedProgress() {
    ...
    try {
      ...
      cleared = window.localStorage.getItem(this.storageKey) === null;
    } catch (error) {
      cleared = false;
      reportNonFatalError("非致命操作失败", error);
    }
  ...
  renderCloseButton(ctx) {
    const button = AchievementPanelLayout.closeButton;
    ctx.fillStyle = InputSystem.pointerInRect(button) ? "#7f6a38" : "#403621";
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(button.x + 14, button.y + 14);
    ctx.lineTo(button.x + button.w - 14, button.y + button.h - 14);
    ctx.moveTo(button.x + button.w - 14, button.y + 14);
    ctx.lineTo(button.x + 14, button.y + button.h - 14);
    ctx.stroke();
  },

  renderCard(ctx, definition, rect, unlocked, hovered) {
    ctx.fillStyle = unlocked ? (hovered ? "#211a0f" : "#17140e") : "#0d0d0d";
    ctx.fillRect(rect.x, rect.y, rect.w, rect.h);
    ctx.strokeStyle = unlocked ? (hovered ? "#d9c27f" : "#92763d") : "#343434";
    ctx.lineWidth = hovered ? 4 : 3;
    ctx.strokeRect(rect.x, rect.y, rect.w, rect.h);

    const imageInsetX = rect.w * 12 / AchievementPanelLayout.cardWidth;
    const imageInsetY = rect.h * 16 / AchievementPanelLayout.cardHeight;
    const imageSize = Math.min(
      rect.w - imageInsetX * 2,
      rect.h * 164 / AchievementPanelLayout.cardHeight
    );
    const image = this.images[definition.id];
    if (unlocked && image && image.complete && image.naturalWidth > 0) {
      ctx.drawImage(image, rect.x + imageInsetX, rect.y + imageInsetY, imageSize, imageSize);
    } else {
      ctx.fillStyle = "#050505";
      ctx.fillRect(rect.x + imageInsetX, rect.y + imageInsetY, imageSize, imageSize);
      ctx.fillStyle = "#555";
      ctx.font = "bold " + Math.round(rect.w * 46 / AchievementPanelLayout.cardWidth) + "px monospace";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("?", rect.x + rect.w / 2, rect.y + imageInsetY + imageSize / 2);
    }

    ctx.fillStyle = unlocked ? "#f3dfaa" : "#777";
    ctx.font = "bold " + Math.round(rect.w * 17 / AchievementPanelLayout.cardWidth) +
      "px 'Microsoft YaHei', monospace";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(
      unlocked ? definition.title : "隐藏成就",
      rect.x + rect.w / 2,
      rect.y + rect.h * 218 / AchievementPanelLayout.cardHeight
    );
  },

  renderDetail(ctx, definition) {
    const card = AchievementPanelLayout.detailCard;
    const content = AchievementPanelLayout.detailContent;
    ctx.save();
    ctx.fillStyle = "#050505";
    ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    this.renderCloseButton(ctx);
    this.renderCard(ctx, definition, card, true, false);

    ctx.fillStyle = "#f5df9d";
    ctx.font = "bold 32px 'Microsoft YaHei', 'SimHei', sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "top";
    ctx.fillText("触发方式", content.x, content.y);
    ctx.strokeStyle = "#7f6a38";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(content.x, content.y + 54);
    ctx.lineTo(content.x + content.w, content.y + 54);
    ctx.stroke();
    ctx.fillStyle = "#efe4c3";
    ctx.font = "24px 'Microsoft YaHei', 'SimHei', sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "top";
    renderWrappedText(ctx, definition.description, content.x, content.y + 92, content.w, 38);
    ctx.restore();
  },

  renderPanel(ctx) {
    const selectedDefinition = AchievementDefinitionsById[GameState.achievementDetailId];
    if (selectedDefinition && this.isUnlocked(selectedDefinition.id)) {
      this.renderDetail(ctx, selectedDefinition);
      return;
    }
    ...
    AchievementDefinitions.forEach((definition, index) => {
      ...
      const hovered = unlocked && InputSystem.pointerInRect(rect);
      this.renderCard(ctx, definition, rect, unlocked, hovered);
    });
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "alphabetic";
    ctx.fillText("点击已解锁成就查看详情", CANVAS_WIDTH / 2, 690);
    ctx.restore();
  }
};
...
const CorridorLayout = {
  ...
  doorMemoryZone: { x: 250, y: 176, w: 250, h: 78 },
  spawns: {
...
const SideRouteYardLayout = {
  ...
  // 典狱长必须和受审的安迪同时进入院子镜头；原 y=476 会被当前放大镜头裁到屏幕上方。
  warden: { x: 622, y: 610 },
  guards: [
    { x: 556, y: 694 },
    { x: 688, y: 694 }
  ...
  crowdPlans: [
    ...
    { startX: 350, startY: 1060, x: 410, y: 650, delay: 0.55, speed: 68 },
...
const PipeLayout = {
  ...
  networkEntryZone: { x: 972, y: 264, w: 51, h: 133 },
...
// Coarse graph traced from the user's red-line reference. Junctions are split
// into explicit nodes so the player can choose branches while remaining exactly
// on a pipe centre line. These coordinates are never rendered in-game.
const PipeNetworkSourceNodes = {
  entrance: { x: 0, y: 205 },
  leftTop: { x: 180, y: 205 },
  leftCurve: { x: 198, y: 225 },
  leftOuterMid: { x: 182, y: 525 },
  leftOuterJunction: { x: 158, y: 815 },
  leftLowerCurve: { x: 140, y: 958 },
  leftLowerRight: { x: 264, y: 958 },
  leftBottomTurn: { x: 264, y: 1197 },
  leftBottomJoin: { x: 264, y: 1240 },
  leftBottomLeft: { x: 115, y: 1240 },
  leftBottomDead: { x: 115, y: 1088 },
  leftBottomRight: { x: 360, y: 1240 },
  leftBottomUp: { x: 360, y: 1100 },
  leftBottomArch: { x: 439, y: 1100 },
  bottomMiddleLeft: { x: 460, y: 1240 },
  bottomMiddleRight: { x: 672, y: 1240 },
  bottomMiddleUp: { x: 672, y: 960 },
  leftInnerLower: { x: 268, y: 815 },
  leftInnerCorner: { x: 286, y: 444 },
  topLeftJunction: { x: 294, y: 327 },
  topCenterJunction: { x: 471, y: 327 },
  topRightDown: { x: 544, y: 327 },
  middleRightCorner: { x: 556, y: 440 },
  middleRightDead: { x: 760, y: 444 },
  topCenterUp: { x: 471, y: 205 },
  topLeftDead: { x: 294, y: 205 },
  topULeft: { x: 625, y: 205 },
  topULowLeft: { x: 615, y: 332 },
  topULowRight: { x: 738, y: 339 },
  topURight: { x: 738, y: 205 },
  topRightJunction: { x: 830, y: 205 },
  exit: { x: 1024, y: 205 },
  rightUpper: { x: 842, y: 330 },
  rightMiddle: { x: 856, y: 560 },
  rightCross: { x: 868, y: 822 },
  rightBottom: { x: 868, y: 960 },
  innerTop: { x: 371, y: 444 },
  crossLeft: { x: 371, y: 564 },
  crossCenter: { x: 458, y: 564 },
  crossRightDead: { x: 754, y: 564 },
  verticalTopDead: { x: 458, y: 440 },
  verticalMiddle: { x: 458, y: 693 },
  verticalBottomDead: { x: 458, y: 843 },
  middleRight: { x: 750, y: 693 },
  rightCrossInner: { x: 750, y: 822 },
  rightLowerInner: { x: 750, y: 960 },
  innerLowerCross: { x: 371, y: 693 },
  innerBottomLeft: { x: 368, y: 958 },
  innerBottomRight: { x: 556, y: 958 },
  innerStemDead: { x: 556, y: 1110 },
  bottomRightDown: { x: 790, y: 1240 },
  bottomRightEnd: { x: 898, y: 1240 },
  bottomRightDead: { x: 892, y: 1080 }
};

const PipeNetworkSourceEdges = [
  ["entrance-left-top", "entrance", "leftTop"],
  ["left-top-curve", "leftTop", "leftCurve"],
  ["left-curve-outer-mid", "leftCurve", "leftOuterMid"],
  ["left-outer-mid-junction", "leftOuterMid", "leftOuterJunction"],
  ["left-junction-lower-curve", "leftOuterJunction", "leftLowerCurve"],
  ["left-lower-horizontal", "leftLowerCurve", "leftLowerRight"],
  ["left-lower-right-bottom-turn", "leftLowerRight", "leftBottomTurn"],
  ["left-bottom-turn-down", "leftBottomTurn", "leftBottomJoin"],
  ["left-bottom-dead", "leftBottomLeft", "leftBottomDead"],
  ["left-bottom-horizontal-left", "leftBottomLeft", "leftBottomJoin"],
  ["left-bottom-horizontal-right", "leftBottomJoin", "leftBottomRight"],
  ["left-bottom-right-up", "leftBottomRight", "leftBottomUp"],
  ["left-bottom-arch", "leftBottomUp", "leftBottomArch"],
  ["left-bottom-arch-down", "leftBottomArch", "bottomMiddleLeft"],
  ["bottom-middle-horizontal", "bottomMiddleLeft", "bottomMiddleRight"],
  ["bottom-middle-right-up", "bottomMiddleRight", "bottomMiddleUp"],
  ["left-junction-inner", "leftOuterJunction", "leftInnerLower"],
  ["left-inner-vertical", "leftInnerLower", "leftInnerCorner"],
  ["left-inner-to-top", "leftInnerCorner", "topLeftJunction"],
  ["top-middle-left", "topLeftJunction", "topCenterJunction"],
  ["top-middle-right", "topCenterJunction", "topRightDown"],
  ["middle-right-down", "topRightDown", "middleRightCorner"],
  ["middle-right-dead", "middleRightCorner", "middleRightDead"],
  ["top-center-up", "topCenterJunction", "topCenterUp"],
  ["top-left-dead", "topCenterUp", "topLeftDead"],
  ["top-u-left", "topCenterUp", "topULeft"],
  ["top-u-down-left", "topULeft", "topULowLeft"],
  ["top-u-low", "topULowLeft", "topULowRight"],
  ["top-u-up-right", "topULowRight", "topURight"],
  ["top-right-horizontal", "topURight", "topRightJunction"],
  ["top-exit", "topRightJunction", "exit"],
  ["right-outer-upper", "topRightJunction", "rightUpper"],
  ["right-outer-middle", "rightUpper", "rightMiddle"],
  ["right-outer-lower", "rightMiddle", "rightCross"],
  ["right-outer-bottom", "rightCross", "rightBottom"],
  ["inner-top-horizontal", "leftInnerCorner", "innerTop"],
  ["inner-left-upper", "innerTop", "crossLeft"],
  ["center-horizontal-left", "crossLeft", "crossCenter"],
  ["center-horizontal-right", "crossCenter", "crossRightDead"],
  ["center-vertical-top", "crossCenter", "verticalTopDead"],
  ["center-vertical-middle", "crossCenter", "verticalMiddle"],
  ["center-vertical-dead", "verticalMiddle", "verticalBottomDead"],
  ["middle-horizontal-right", "verticalMiddle", "middleRight"],
  ["middle-right-vertical", "middleRight", "rightCrossInner"],
  ["right-cross-link", "rightCrossInner", "rightCross"],
  ["right-inner-lower", "rightCrossInner", "rightLowerInner"],
  ["right-lower-link", "rightLowerInner", "rightBottom"],
  ["bottom-middle-link", "rightLowerInner", "bottomMiddleUp"],
  ["inner-left-lower", "crossLeft", "innerLowerCross"],
  ["inner-left-bottom", "innerLowerCross", "innerBottomLeft"],
  ["inner-bottom-horizontal", "innerBottomLeft", "innerBottomRight"],
  ["inner-stem-dead", "innerBottomRight", "innerStemDead"],
  ["bottom-right-down", "rightLowerInner", "bottomRightDown"],
  ["bottom-right-horizontal", "bottomRightDown", "bottomRightEnd"],
  ["bottom-right-dead", "bottomRightEnd", "bottomRightDead"]
];

const PipeNetworkLayout = {
  imageWidth: 1024,
  imageHeight: 1536,
  worldWidth: 1024 * PIPE_NETWORK_WORLD_SCALE,
  worldHeight: 1536 * PIPE_NETWORK_WORLD_SCALE,
  entranceEdgeId: "entrance-left-top",
  exitNodeId: "exit",
  graph: createPipeNetworkGraph(PipeNetworkSourceNodes, PipeNetworkSourceEdges)
};

const PipeNavigationMapLayout = {
  compactFrame: { x: 52, y: 53, w: 102, h: 153 },
  compactPlot: { x: 52, y: 53, w: 102, h: 153 },
  expandedFrame: { x: 176, y: 36, w: 928, h: 648 },
  expandedPlot: { x: 451, y: 88, w: 377, h: 566 },
  expandedCloseButton: { x: 1030, y: 58, w: 46, h: 46 },
  entranceHint: { x: 172, y: 62, w: 520, h: 82 }
};

function createPipeNetworkGraph(sourceNodes, sourceEdges) {
  const nodes = {};
  Object.keys(sourceNodes).forEach((id) => {
    const source = sourceNodes[id];
    nodes[id] = {
      id,
      x: source.x * PIPE_NETWORK_WORLD_SCALE,
      y: source.y * PIPE_NETWORK_WORLD_SCALE,
      edgeIndexes: []
    };
  });

  const edgeIndexById = {};
  const edges = sourceEdges.map((definition, index) => {
    const id = definition[0];
    const aId = definition[1];
    const bId = definition[2];
    const a = nodes[aId];
    const b = nodes[bId];
    const dx = b.x - a.x;
    const dy = b.y - a.y;
    ...
    const edge = {
      ...
      bId,
      length,
      tangentX: dx / Math.max(1, length),
      tangentY: dy / Math.max(1, length)
    };
    edgeIndexById[id] = index;
    a.edgeIndexes.push(index);
    b.edgeIndexes.push(index);
    return edge;
  });

  return { nodes, edges, edgeIndexById };
}
...
const YardLayout = {
  ...
  interactions: [
    { id: "prisonGate", label: "脱离拘禁地", rect: { x: 505, y: 218, w: 230, h: 82 }, tint: "#f2d26d" },
    { id: "cellDoor", label: "进入房间", rect: { x: 1032, y: 925, w: 125, h: 92 }, tint: "#b977ff" },
    { id: "wardenOffice", label: "典狱长办公室", rect: { x: 96, y: 504, w: 130, h: 92 }, tint: "#ff9bd8" },
    { id: "library", label: "图书馆", rect: { x: 96, y: 1093, w: 134, h: 72 }, tint: "#e0b45d" },
    // 地图偏右侧深棕色泥地：椭圆区域既是撒土判定区，也是狱警的巡逻范围。
...
const YardNavigationMapLayout = {
  ...
  expandedCloseButton: { x: 1030, y: 58, w: 46, h: 46 },
  expandedAchievementButton: { x: 202, y: 600, w: 136, h: 54 }
};
...
const YardNavigationMapLegendItems = [
  ...
  { color: "#ffe36f", label: "当前主线方向", shape: "arrow" },
  { color: "#ffe36f", label: "当前任务位置", shape: "ring" },
  { color: "#292925", label: "尚未探索区域" }
];

const YardMapPostTutorialThoughtLines = [
  "我记得在电影中，瑞德是我最好的朋友",
  "我现在应该去找找他"
];
const YARD_MAP_POST_TUTORIAL_THOUGHT_SECONDS = 2.8;
const YARD_MAP_POST_TUTORIAL_THOUGHT_FADE_SECONDS = 0.3;
...
const OpeningYardLayout = {
  ...
  gateApproachZone: { x: 470, y: 184, w: 317, h: 142 },
  gateApproachSpeedScale: 0.3,
  gateLightSeam: { x: 626, topY: 108, thresholdY: 190 },
  gateLightPools: [
    { x: 626, y: 195, radiusX: 25, radiusY: 18, alpha: 0.22 },
    { x: 623, y: 226, radiusX: 40, radiusY: 43, alpha: 0.14 },
    { x: 631, y: 271, radiusX: 58, radiusY: 66, alpha: 0.09 },
    { x: 620, y: 310, radiusX: 70, radiusY: 52, alpha: 0.055 }
  ],
  gateLightFadeSeconds: 0.4,
  gateThoughtDelaySeconds: 0.45,
  gateThoughtFadeSeconds: 0.24,
...
// ======================================================
// Persistent free-roaming NPC configuration
// ======================================================
const RoamingNpcConfigs = [
  {
    id: "tommy", assetKey: "tommy", name: "汤米", role: "prisoner", source: "named", sourceIndex: 0,
    travelChance: 0.26, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 70, cellCorridor: 30 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "haywood", assetKey: "haywood", name: "海伍德", role: "prisoner", source: "named", sourceIndex: 1,
    travelChance: 0.32, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 30, cellCorridor: 70 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "floyd", assetKey: "floyd", name: "弗洛伊德", role: "prisoner", source: "named", sourceIndex: 2,
    travelChance: 0.22, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 60, cellCorridor: 40 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "prisoner_01", assetKey: "prisoner_01", name: "", role: "prisoner", source: "background", sourceIndex: 0,
    travelChance: 0.12, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 70, cellCorridor: 30 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "prisoner_02", assetKey: "prisoner_02", name: "", role: "prisoner", source: "background", sourceIndex: 1,
    travelChance: 0.16, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 40, cellCorridor: 60 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "prisoner_03", assetKey: "prisoner_03", name: "", role: "prisoner", source: "background", sourceIndex: 2,
    travelChance: 0.20, allowedScenes: ["yard", "library", "cellCorridor"],
    yardWeights: { library: 55, cellCorridor: 45 },
    corridorWeights: { yard: 100 }
  },
  {
    id: "guard_1", assetKey: "guard", name: "狱警", role: "guard", source: "guard", sourceIndex: 0,
    travelChance: 0.34, allowedScenes: ["yard", "library", "cellCorridor", "solitary"],
    yardWeights: { library: 5, cellCorridor: 95 },
    corridorWeights: { yard: 69, solitary: 31 }
  },
  {
    id: "guard_2", assetKey: "guard", name: "狱警", role: "guard", source: "guard", sourceIndex: 1,
    travelChance: 0.37, allowedScenes: ["yard", "library", "cellCorridor", "solitary"],
    yardWeights: { library: 10, cellCorridor: 90 },
    corridorWeights: { yard: 69, solitary: 31 }
  },
  {
    id: "guard_3", assetKey: "guard", name: "狱警", role: "guard", source: "guard", sourceIndex: 2,
    travelChance: 0.40, allowedScenes: ["yard", "library", "cellCorridor", "solitary"],
    yardWeights: { library: 15, cellCorridor: 85 },
    corridorWeights: { yard: 71, solitary: 29 }
  }
];

// guard_4…guard_8 循环复用前三名狱警的跨场景概率，但保持独立状态与速度。
const BaseRoamingGuardConfigs = RoamingNpcConfigs.filter((config) => config.source === "guard");
for (let guardNumber = 4; guardNumber <= ROAMING_GUARD_MAX_COUNT; guardNumber += 1) {
  const profile = BaseRoamingGuardConfigs[(guardNumber - 1) % BaseRoamingGuardConfigs.length];
  RoamingNpcConfigs.push(Object.assign({}, profile, {
    id: "guard_" + guardNumber,
    sourceIndex: guardNumber - 1,
    yardWeights: Object.assign({}, profile.yardWeights),
    corridorWeights: Object.assign({}, profile.corridorWeights),
    allowedScenes: profile.allowedScenes.slice()
  }));
}

const RoamingPortalLinks = {
  "yard:library": {
    from: { x: 165, y: 1104 },
    spawn: { x: 626, y: 1010 }
  },
  ...
  "cellCorridor:solitary": {
    from: { x: 620, y: 380 },
    spawn: { x: 440, y: 440 }
  },
  "solitary:cellCorridor": {
    from: { x: 400, y: 410 },
    spawn: { x: 620, y: 380 }
  }
};
...
const ASSET_RETRY_BASE_DELAY_MS = 1000;
const ASSET_RETRY_MAX_DELAY_MS = 30000;
...
function getAssetRetryDelay(failureCount) {
  const exponent = Math.max(0, Math.min(5, failureCount - 1));
  return Math.min(ASSET_RETRY_MAX_DELAY_MS, ASSET_RETRY_BASE_DELAY_MS * (2 ** exponent));
}

function markAssetLoadFailed(record) {
  record.loading = false;
  record.loaded = false;
  record.failed = true;
  record.failureCount += 1;
  record.retryAt = Date.now() + getAssetRetryDelay(record.failureCount);
  record.element = null;
}

function prepareAssetRetry(record) {
  if (!record.failed) {
    return true;
  }
...
function ensureImageAsset(key) {
  ...
  if (!record || record.loaded || record.loading || !prepareAssetRetry(record)) {
    return record || null;
  }
  ...
  const onImageLoad = () => {
    if (record.element !== image) return;
    record.loading = false;
    record.loaded = true;
    record.failed = false;
  ...
  const onImageError = () => {
    if (record.element !== image) return;
    markAssetLoadFailed(record);
  };
  record.loadHandler = onImageLoad;
  record.errorHandler = onImageError;
...
function getRenderableAnimationFrame(frames, preferredIndex) {
  if (!frames || frames.length === 0) {
    return null;
  }

  const safeIndex = Math.max(0, Math.floor(preferredIndex || 0)) % frames.length;
  const preferred = frames[safeIndex] || frames[0];
  const preferredRecord = getImageAssetRecord(preferred.assetKey);
  if (preferredRecord && preferredRecord.loaded && !preferredRecord.failed) {
    return preferred;
  }

  for (let offset = 1; offset < frames.length; offset += 1) {
    const candidate = frames[(safeIndex - offset + frames.length) % frames.length];
    const candidateRecord = getImageAssetRecord(candidate.assetKey);
    if (candidateRecord && candidateRecord.loaded && !candidateRecord.failed) {
      return candidate;
    }
  }
...
function releaseImageAsset(record) {
  ...
  if (record.loadHandler) record.element.removeEventListener("load", record.loadHandler);
  if (record.errorHandler) record.element.removeEventListener("error", record.errorHandler);
...
function ensureAudioAsset(key) {
  ...
  if (!record || record.element || record.loading || !prepareAssetRetry(record)) {
    return record || null;
  }
  ...
  record.loading = true;
  const markLoaded = () => {
    if (record.element !== audio) return;
    record.loading = false;
    record.loaded = true;
    record.failed = false;
    record.failureCount = 0;
...
function saveGameBgmResumeTime() {
  ...
  try {
    ...
    reportNonFatalError("保存音乐播放进度失败", error);
  }
}
...
function createAudioElements(key, gainMultiplier, playbackRate, loop) {
  ...
  return getAudioVolumes(gainMultiplier).map((volume) => {
    ...
    try {
      ...
      reportExpectedBrowserLimitation("音频元数据就绪前无法重置播放位置", error);
    }
    return audio;
  });
}
...
function safePlayAudio(audio) {
  try {
    ...
    if (result && typeof result.catch === "function") {
      result.catch((error) => {
        reportExpectedBrowserLimitation("浏览器阻止了音频自动播放", error);
      });
    }
  } catch (error) {
    reportNonFatalError("音频播放失败", error);
  }
}
...
function stopAudioElement(audio) {
  ...
  try {
    ...
    reportExpectedBrowserLimitation("停止音频时无法重置播放位置", error);
  }
}
...
function getFootstepAudioKey() {
  ...
  if (GameState.scene === "pipeNetwork") {
    return GameState.pipeNetwork.isMoving ? "footsteps_concrete" : null;
  }
...
const Scenes = {
  menu: createBaseScene("menu", {
    ...
    handleInput() {
      ...
      if (secretClick) {
        ...
        if (GameState.secrets.menuClicks >= 3) {
          recordPlaystyleDiscovery("menuSecret");
        }
      ...
      if (selectedChoice.id === "newGame") {
        // 新一局同时清除流程存档和成就，确保所有彩蛋都可从零重新触发。
        CheckpointSystem.clear({ preserveYardMap: false });
        AchievementSystem.clearSavedProgress();
        AudioSystem.playOneShot("thunder", THUNDER_VOLUME);
        changeScene("livingRoom");
        return;
      }
      if (selectedChoice.id === "start") {
        OpeningNoticeSystem.start();
        return;
      }
      if (selectedChoice.id === "continue") {
  ...
  cell: createBaseScene("cell", {
    enter() {
      ...
      if (wakingFromOperatingRoom) {
        ...
        startCellWakePrompt();
        startOverheadThought(CellWakeThoughtLines);
      } else if (returningAfterSoilDump) {
        startOverheadThought(SoilRestThoughtLines);
      } else if (returningFromMontage) {
        startOverheadThought(
    ...
    handleInput() {
      ...
      if ((InputSystem.actionPressed("interact") || wantsWallPictureClick) && activeInteraction) {
        ...
        } else if (activeInteraction.id === "roamingNpc") {
          startRoamingNpcInteraction(activeInteraction);
        }
      }
    },
  ...
  pipeNetwork: createBaseScene("pipeNetwork", {
    enter() {
      initializePipeNetworkScene();
      PipeNavigationMapSystem.enter();
      saveCheckpoint("CP_PIPE_NETWORK");
    },

    update(dt) {
      updatePipeNetworkScene(dt);
      PipeNavigationMapSystem.update(dt);
    },
    ...
    render(ctx) {
      renderPipeNetwork(ctx);
    },

    exit() {
      PipeNavigationMapSystem.leave();
    }
  }),
  ...
  yard: createBaseScene("yard", {
    enter() {
      ...
      const needsOpeningTutorial = isOpeningArrival && !GameState.wakeTutorialCompleted;
      ...
      GameState.currentQuest = isSideRouteCustodyArrival ?
        ...
        (isOpeningArrival ?
        (needsOpeningTutorial ? "" : "quest_yard_first_walk") :
      ...
      if (needsOpeningTutorial) {
        startOverheadThought(OpeningYardThoughtLines);
    ...
    handleInput() {
      ...
      if (InputSystem.actionPressed("interact") && activeInteraction) {
        if (activeInteraction.id === "outsidePlanGate") {
          startOutsidePlanEscape();
        } else if (activeInteraction.id === "prisonGate") {
          ...
          if (!isOfficeTaskEntryAllowed()) {
            captureUnauthorizedOfficeEntrySnapshot();
          } else {
            clearUnauthorizedOfficeEntrySnapshot();
          }
          changeScene("office");
        } else if (activeInteraction.id === "red") {
          const mainDialogueKind = getPendingNpcMainDialogueKind("red");
          if (mainDialogueKind) {
            requireNpcReapproachForSideTalk("red");
          ...
          const mainDialogueKind = getPendingNpcMainDialogueKind("brooks");
          if (mainDialogueKind) {
            requireNpcReapproachForSideTalk("brooks");
  ...
  outsidePlanEscape: createBaseScene("outsidePlanEscape", {
    enter() {
      GameState.currentQuest = "";
    },

    update(dt) {
      updateOutsidePlanEscape(dt);
    },

    handleInput() {},

    render(ctx) {
      renderOutsidePlanEscape(ctx);
    }
  }),
    ...
    handleInput() {
      ...
      if (InputSystem.actionPressed("interact") && activeInteraction) {
        if (activeInteraction.id === "exit") {
          ...
          if (!GameState.hasBible && isBrooksWaitingInLibrary()) {
            requireNpcReapproachForSideTalk("brooks");
            startBrooksDialogue();
          } else if (canStartSideTalk("brooks")) {
            startSideTalk("brooks");
  ...
  office: createBaseScene("office", {
    enter() {
      ...
      if (!isMainStoryOfficeEntryAllowed()) {
        enterFreeOfficeScene();
        return;
      }
      clearUnauthorizedOfficeEntrySnapshot();
      recordPlaystyleEvidenceBeforeStoryOffice();
    ...
    handleInput() {
      ...
      if (InputSystem.actionPressed("interact") && activeInteraction) {
        if (activeInteraction.id === "warden") {
          ...
          recordPlaystyleDiscovery("officeSecret");
  ...
  fail: createBaseScene("fail", {
    ...
    handleInput() {
      if (InputSystem.actionPressed("interact") || InputSystem.actionPressed("continueDialogue")) {
        ...
        if (GameState.failRecovery === "pipeOpened") {
          restorePipeDrownFailureToOpenedPipe();
          return;
        }
...
// ======================================================
// 7.1 Opening Game Notice
// ======================================================
const OpeningNoticeSystem = {
  active: false,
  ready: false,
  remaining: 0,
  startedAt: 0,
  ...
  init() {
    const root = document.getElementById("openingNoticeOverlay");
    if (!root) {
      return;
    }
    this.elements = {
      root,
      status: document.getElementById("openingNoticeStatus"),
      acceptButton: document.getElementById("openingNoticeAcceptButton"),
      rejectButton: document.getElementById("openingNoticeRejectButton")
    };
    this.elements.acceptButton.addEventListener("click", () => this.accept());
    this.elements.rejectButton.addEventListener("click", () => this.reject());
    this.hide();
  },
  ...
  start() {
    ...
    this.active = true;
    this.ready = false;
    this.remaining = OPENING_NOTICE_DURATION;
  ...
  update() {
    ...
    this.remaining = Math.max(0, OPENING_NOTICE_DURATION - elapsedSeconds);
    if (this.remaining === 0) {
      this.ready = true;
    }
    this.sync();
  ...
  sync() {
    if (!this.elements || !this.active) {
      return;
    }
    this.elements.root.hidden = false;
    this.elements.root.setAttribute("aria-hidden", "false");
    this.elements.status.textContent = this.ready ?
      "阅读完成，请选择是否继续" :
      "请完整阅读声明，" + String(Math.max(1, Math.ceil(this.remaining))) + " 秒后可进行选择";
    this.elements.status.classList.toggle("is-ready", this.ready);
    this.elements.acceptButton.disabled = !this.ready;
    this.elements.rejectButton.disabled = !this.ready;
  },
  ...
  hide() {
    ...
    this.elements.status.textContent = "请完整阅读声明，3 秒后可进行选择";
    this.elements.status.classList.remove("is-ready");
    this.elements.acceptButton.disabled = true;
    this.elements.rejectButton.disabled = true;
  }
};
...
function changeScene(sceneName) {
  ...
  // 暂停只是覆盖层：进入和退出时不重新执行互动空间场景的 enter/exit，
  // 避免重置玩家位置、任务或正在显示的头顶心理描写。
  if (sceneName === "pause" && GameState.scene !== "pause") {
    GameState.previousScene = GameState.scene;
    GameState.scene = "pause";
    const pauseScene = Scenes.pause;
    if (pauseScene && pauseScene.enter) {
      pauseScene.enter();
    }
    return;
  }
  if (GameState.scene === "pause" && sceneName === GameState.previousScene) {
    const pauseScene = Scenes.pause;
    if (pauseScene && pauseScene.exit) {
      pauseScene.exit();
    }
...
function initializeCorridorScene() {
  ...
  if (corridor.mode === "postSolitary") {
    ...
    startOverheadThought(CorridorPostSolitaryLines);
...
function updateCorridorScene(dt) {
  finalizeCorridorDoorMemoryUnlock();
  if (isCorridorEscortActive()) {
    cancelCorridorDeveloperMessageHold();
...
function handleCorridorInput() {
  ...
  const corridor = GameState.corridor;
  if (interaction.id === "doorMemory") {
    handleCorridorDoorMemoryInteraction();
    return;
  }
  if (interaction.id === "roamingNpc") {
    startRoamingNpcInteraction(interaction);
    return;
  }
  ...
  if (interaction.id === "cellDoor") {
    if (corridor.mode === "postSolitary") {
      startOverheadThought(["安迪（心理）：现在不能停下。先回院子，到图书馆前找老布。"]);
      return;
    }
...
function getActiveCorridorInteraction() {
  ...
  if (pointInRect(foot, CorridorLayout.doorMemoryZone)) {
    return { id: "doorMemory" };
  }
  ...
  return getNearbyRoamingNpcInteraction("cellCorridor", foot);
}

function resetCorridorDoorMemoryVisit() {
  const corridor = GameState.corridor;
  corridor.doorMemoryIdleTimer = 0;
  corridor.doorMemoryViewedThisVisit = false;
  corridor.doorMemoryUnlockPending = false;
  corridor.developerMessageArmed = false;
  corridor.developerMessageHoldTimer = 0;
  corridor.developerMessageShownThisVisit = false;
}

function isPlayerInCorridorDoorMemoryZone() {
  if (GameState.scene !== "cellCorridor" || isCorridorEscortActive()) {
    return false;
  }
  const foot = corridorWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  return pointInRect(foot, CorridorLayout.doorMemoryZone);
}

function startCorridorDoorMemoryDialogue() {
  const corridor = GameState.corridor;
  corridor.doorMemoryIdleTimer = 0;
  corridor.doorMemoryViewedThisVisit = true;
  recordPlaystyleDiscovery("doorMemory");
  corridor.developerMessageArmed = false;
  corridor.developerMessageHoldTimer = 0;
  corridor.doorMemoryUnlockPending = !AchievementSystem.isUnlocked("person_behind_door");
  stopPlayerForNpcDialogue();
  DialogueSystem.start(CorridorDoorMemoryLines, "dialogue");
}

function handleCorridorDoorMemoryInteraction() {
  const corridor = GameState.corridor;
  if (!AchievementSystem.isUnlocked("person_behind_door") ||
    !corridor.doorMemoryViewedThisVisit || corridor.developerMessageShownThisVisit) {
    startCorridorDoorMemoryDialogue();
    return;
  }
  if (corridor.developerMessageArmed) {
    return;
  }
  corridor.developerMessageArmed = true;
  corridor.developerMessageHoldTimer = 0;
  stopPlayerForNpcDialogue();
}

function updateCorridorDoorMemory(dt) {
  const corridor = GameState.corridor;
  const insideZone = isPlayerInCorridorDoorMemoryZone();
  if (corridor.developerMessageArmed) {
    if (!insideZone || GameState.player.isMoving) {
      cancelCorridorDeveloperMessageHold();
    } else {
      corridor.developerMessageHoldTimer += dt;
      if (corridor.developerMessageHoldTimer >= CORRIDOR_DEVELOPER_HOLD_SECONDS) {
        corridor.developerMessageArmed = false;
        corridor.developerMessageHoldTimer = 0;
        corridor.developerMessageShownThisVisit = true;
        stopPlayerForNpcDialogue();
        DialogueSystem.start(CorridorDeveloperMessageLines);
      }
    }
    return;
  }
  ...
  if (AchievementSystem.isUnlocked("person_behind_door") ||
    corridor.doorMemoryViewedThisVisit || !insideZone || GameState.player.isMoving) {
    corridor.doorMemoryIdleTimer = 0;
    return;
  }

  corridor.doorMemoryIdleTimer += dt;
  if (corridor.doorMemoryIdleTimer >= CORRIDOR_DOOR_MEMORY_IDLE_SECONDS) {
    startCorridorDoorMemoryDialogue();
  }
}

function cancelCorridorDeveloperMessageHold() {
  GameState.corridor.developerMessageArmed = false;
  GameState.corridor.developerMessageHoldTimer = 0;
}

function finalizeCorridorDoorMemoryUnlock() {
  const corridor = GameState.corridor;
  if (!corridor.doorMemoryUnlockPending || DialogueSystem.active) {
    return;
  }
  corridor.doorMemoryUnlockPending = false;
  AchievementSystem.unlock("person_behind_door");
}
...
function handlePauseInput() {
  // 暂停菜单已下线。保留 Esc / P 的动作识别，供成就面板等模态界面关闭使用。
}
...
function registerCellWallSecret() {
  ...
  if (GameState.secrets.wallChecks >= 3) {
    recordPlaystyleDiscovery("wallVoice");
  }
...
function updateLibraryExplorationAchievement() {
  ...
  if (foot && foot.y <= LibraryLayout.imageHeight / 3) {
    recordPlaystyleDiscovery("bookshelfSecret");
...
function updateYardExplorationAchievements() {
  ...
  if (foot.y >= 1170) {
    recordPlaystyleDiscovery("mapEdge");
  ...
  if (Object.keys(GameState.secrets.yardCorners).length >= 4) {
    recordPlaystyleDiscovery("fourCorners");
...
const DeveloperDialogueBoxLayout = {
  sourceW: 1152,
  sourceH: 600,
  // Keep the original centre point while displaying the complete frame at 70%.
  canvasRect: { x: 236.8, y: 198, w: 806.4, h: 420 },
  nameSourceRect: { x: 390, y: 142, w: 230, h: 78 },
  nameFontSize: 36,
  textSourceRect: { x: 405, y: 245, w: 680, h: 285 },
  textFontSize: 28
};
...
const OVERHEAD_THOUGHT_MIN_SECONDS = 2.8;
const OVERHEAD_THOUGHT_MAX_SECONDS = 5.2;
const OVERHEAD_THOUGHT_FADE_SECONDS = 0.3;

const OverheadThoughtSystem = {
  active: false,
  lines: [],
  index: 0,
  lineTimer: 0,
  ...
  isActive() {
    return this.active && this.index < this.lines.length;
  },
  ...
  getCurrentLineDuration() {
    const visibleCharacterCount = this.getCurrentLine().replace(/\s/g, "").length;
    return clamp(
      2.2 + visibleCharacterCount * 0.08,
      OVERHEAD_THOUGHT_MIN_SECONDS,
      OVERHEAD_THOUGHT_MAX_SECONDS
    );
  },
  ...
  handleInput() {
    // 头顶提示只响应空格／交互按钮。普通画布点击（尤其移动摇杆）不能误跳句。
    if (!this.isActive() || !InputSystem.actionPressed("interact")) {
      return false;
    }
    this.advance();
    return true;
  ...
  update(dt) {
    if (!this.isActive() || GameState.scene === "pause") {
      return;
    }
    if (this.justStarted) {
      this.justStarted = false;
    ...
    this.lineTimer += dt;
    if (this.lineTimer >= this.getCurrentLineDuration()) {
      this.advance();
    }
  },

  render(ctx) {
    if (!this.isActive() || shouldHideOverheadThought()) {
      return;
    }
    const duration = this.getCurrentLineDuration();
    const remaining = duration - this.lineTimer;
    const alpha = Math.min(
      1,
      this.lineTimer / OVERHEAD_THOUGHT_FADE_SECONDS,
      remaining / OVERHEAD_THOUGHT_FADE_SECONDS
    );
    renderPlayerOverheadThought(ctx, this.getCurrentLine(), clamp(alpha, 0, 1));
  },
...
function getOverheadThoughtText(line) {
  return String(line || "")
    .replace(/^安迪[（(](?:心理|内心)[）)]\s*[：:]\s*/, "")
    .trim();
}

function startOverheadThought(lines) {
  OverheadThoughtSystem.start(lines);
}
...
// 在获得关键信息后，先保留确认提示，再按指定呈现方式进入后续文字。
// 这样“获得证据”不会被新内容遮住，也不会在玩家移动时突然打断操作。
const NarrativeCueSystem = {
  pendingLines: null,
  nextQuest: null,
  presentation: "dialogue",

  schedule(lines, nextQuest, presentation) {
    this.pendingLines = Array.isArray(lines) ? lines.slice() : [];
    this.nextQuest = nextQuest || null;
    this.presentation = presentation === "overhead" ? "overhead" : "dialogue";
  },

  isActive() {
    return Boolean(this.pendingLines) && !DialogueSystem.active && !OverheadThoughtSystem.isActive();
  },
  ...
  handleInput() {
    ...
    this.nextQuest = null;
    this.presentation = "dialogue";
    if (nextQuest) {
      GameState.currentQuest = nextQuest;
    }
    if (lines.length) {
      if (presentation === "overhead") {
        startOverheadThought(lines);
      } else {
...
function renderNarrativeBox(ctx, text, box, options) {
  ...
  renderNarrativeTextBlock(
    ctx,
    text,
    narrativeSourceRectToCanvas(frameBox, NarrativeBoxLayout.textSourceRect),
    options
  );
  ctx.restore();
}
...
function renderDialogueBox(ctx, line) {
  ...
  if (parsed.speaker === "开发者") {
    renderDeveloperDialogueBox(ctx, parsed);
    return;
  }
...
function renderDeveloperDialogueBox(ctx, parsed) {
  const box = DeveloperDialogueBoxLayout.canvasRect;
  ctx.save();
  ctx.shadowBlur = 0;
  drawAsset(ctx, "dialogue_developer", box.x, box.y, box.w, box.h);
  renderDialogueSpeakerName(
    ctx,
    parsed.speaker,
    developerDialogueSourceRectToCanvas(DeveloperDialogueBoxLayout.nameSourceRect),
    DeveloperDialogueBoxLayout.nameFontSize
  );
  renderDialogueTextBlock(
    ctx,
    parsed.text,
    developerDialogueSourceRectToCanvas(DeveloperDialogueBoxLayout.textSourceRect),
    DeveloperDialogueBoxLayout.textFontSize
  );
  ctx.restore();
}
...
function developerDialogueSourceRectToCanvas(rect) {
  const box = DeveloperDialogueBoxLayout.canvasRect;
  return {
    x: box.x + rect.x * box.w / DeveloperDialogueBoxLayout.sourceW,
    y: box.y + rect.y * box.h / DeveloperDialogueBoxLayout.sourceH,
    w: rect.w * box.w / DeveloperDialogueBoxLayout.sourceW,
    h: rect.h * box.h / DeveloperDialogueBoxLayout.sourceH
  };
}

function renderDialogueSpeakerName(ctx, speaker, rect, fontSizeOverride) {
  if (!speaker) {
    return;
  }
  ...
  ctx.rect(rect.x, rect.y, rect.w, rect.h);
  ctx.clip();
  const fontSize = Number.isFinite(fontSizeOverride) ?
    Math.round(fontSizeOverride) :
    Math.round(clamp(rect.h * 0.82, 12, 22));
...
function renderDialogueTextBlock(ctx, text, rect, fontSizeOverride) {
  ctx.save();
  ctx.beginPath();
  ctx.rect(rect.x, rect.y, rect.w, rect.h);
  ctx.clip();
  ...
  let fontSize = Number.isFinite(fontSizeOverride) ?
    Math.max(14, Math.round(fontSizeOverride)) :
    Math.round(clamp(rect.h * 0.24, 14, 22));
  let lineHeight = 34;
  let lines = [];
  while (fontSize >= 14) {
...
function renderNarrativeTextBlock(ctx, text, rect, options) {
  ctx.save();
  ctx.beginPath();
  ctx.rect(rect.x, rect.y, rect.w, rect.h);
  ctx.clip();
  ...
  let fontSize = options && Number.isFinite(options.fontSize) ?
    options.fontSize :
    Math.round(clamp(rect.h * 0.18, 16, 28));
  let lineHeight = 30;
  let lines = [];
  while (fontSize >= 16) {
    ctx.font = fontSize + "px 'Microsoft YaHei', 'SimHei', monospace";
...
function renderTransferRegisterEvidence(ctx) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("拘禁地图书馆 · 借阅登记", 0, -page.h / 2 + 42);
  ...
  ctx.fillText("PRISON", page.w / 2 - 70, page.h / 2 - 59);
  ctx.restore();
...
// 取证交流不属于主线任务。主线节点完成后立即开放取证阶段，但刚完成
// 主线交谈的 NPC 必须先离开交互范围，再次靠近后才能触发取证内容。
function enableSideTalksAfterMain(npcId) {
  GameState.sideTalk.refreshRequired = false;
  GameState.sideTalk.available = true;
  requireNpcReapproachForSideTalk(npcId);
  saveCheckpoint(GameState.currentCheckpoint);
}

// 兼容旧存档：旧版本可能仍记录为等待重新进入院子后开放取证。
...
function captureSideTalkProgress() {
  return {
    refreshRequired: Boolean(GameState.sideTalk.refreshRequired),
    available: Boolean(GameState.sideTalk.available)
  };
}

function restoreSideTalkProgress(record) {
  if (record && typeof record === "object") {
    GameState.sideTalk.available = Boolean(record.available);
    GameState.sideTalk.refreshRequired = !GameState.sideTalk.available && Boolean(record.refreshRequired);
    return;
  }

  // 旧存档没有记录取证开放状态。人物任务已经完成时直接开放，
  // 避免未取得的人物证据和狱警对话永久关闭。
  GameState.sideTalk.available = Boolean(GameState.redHammerDelivered || GameState.brooksBibleDelivered);
  GameState.sideTalk.refreshRequired = false;
}
...
function canStartSideTalk(npcId) {
  ...
  return !getPendingNpcMainDialogueKind(npcId) &&
    !GameState.sideTalk.reapproachNpcIds.includes(npcId);
}
...
function getPendingNpcMainDialogueKind(npcId) {
  if (npcId === "red") {
    return getPendingRedMainDialogueKind();
  }
  if (npcId === "brooks") {
    if (GameState.scene === "library" && !GameState.hasBible && isBrooksWaitingInLibrary()) {
      return "libraryOpening";
    }
    return getPendingBrooksMainDialogueKind();
  }
  return null;
}

function requireNpcReapproachForSideTalk(npcId) {
  if (!npcId || GameState.sideTalk.reapproachNpcIds.includes(npcId)) {
    return;
  }
  GameState.sideTalk.reapproachNpcIds.push(npcId);
}

function isNpcInsideSideTalkExitRange(npcId) {
  if (GameState.scene === "yard") {
    const foot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
    let actor = null;
    let interactionRange = 0;
    if (npcId === "red") {
      actor = GameState.redNpc;
      interactionRange = 86;
    } else if (npcId === "brooks" && !GameState.libraryTask.brooksInside) {
      actor = GameState.brooksNpc;
      interactionRange = 70;
    } else {
      actor = GameState.yardPrisoners.find((item) => item.assetKey === npcId) || null;
      interactionRange = 74;
    }
    return Boolean(actor) && distance(foot.x, foot.y, actor.x, actor.y) <= interactionRange + 24;
  }

  if (GameState.scene === "library" && npcId === "brooks" && isBrooksWaitingInLibrary()) {
    const foot = canvasPointToLibraryImage(getPlayerFootPoint(GameState.player.x, GameState.player.y));
    return Boolean(foot) &&
      distance(foot.x, foot.y, LibraryLayout.brooks.x, LibraryLayout.brooks.y) <= 150;
  }

  return false;
}

function updateSideTalkReapproachLocks() {
  if (!GameState.sideTalk.reapproachNpcIds.length || DialogueSystem.active ||
    isRedDialogueApproachActive() || isBrooksDialogueApproachActive()) {
    return;
  }
  GameState.sideTalk.reapproachNpcIds = GameState.sideTalk.reapproachNpcIds.filter(
    (npcId) => isNpcInsideSideTalkExitRange(npcId)
  );
}
...
function getEndingEvidenceSnapshot() {
  return {
    direct: {
      brooks: Boolean(GameState.hasBrooksEvidence),
      red: Boolean(GameState.hasFinancialEvidence),
      tommy: Boolean(GameState.hasTommyEvidence),
      haywood: Boolean(GameState.hasHaywoodEvidence),
      floyd: Boolean(GameState.hasFloydEvidence)
    },
    observedEvidenceIds: normalizeObservedConversationEvidenceIds(
...
function recordPlaystyleMapOpen(kind) {
  PlaystyleSystem.recordMapOpen(GameState.playstyle, kind);
}

function recordPlaystyleEvidenceBeforeStoryOffice() {
  PlaystyleSystem.recordEvidenceBeforeStoryOffice(
    GameState.playstyle,
    getCollectedEvidenceCount()
  );
}

function recordPlaystyleOutsidePlanEscape() {
  PlaystyleSystem.recordOutsidePlanEscape(GameState.playstyle);
}

function recordPlaystyleFailureRecovery(failureId) {
  PlaystyleSystem.recordFailureRecovery(GameState.playstyle, failureId);
}

function recordPlaystyleDiscovery(discoveryId) {
  PlaystyleSystem.recordDiscovery(GameState.playstyle, discoveryId);
}

function getPlaystyleResolutionSnapshot() {
  const recoveredFailures = new Set(GameState.playstyle.recoveredFailureIds || []);
  const routeHideClean = GameState.alternateEscapeRoute ?
    !recoveredFailures.has("sideRouteOffice") :
    Boolean(GameState.inspectionPassed && GameState.hammerHiddenInBible);
  const cleanStageCount = [
    routeHideClean,
    !recoveredFailures.has("soilDump"),
    !recoveredFailures.has("pipeBeforeTunnel"),
    !recoveredFailures.has("pipeOpened")
  ].filter(Boolean).length;
  return {
    directEvidenceCount: getCollectedDirectEvidenceCount(),
    observedEvidenceCount: getObservedConversationEvidenceCount(),
    landmarkCount: GameState.yardNavigationMap.discoveredLandmarks.length,
    alternateRouteCompleted: GameState.alternateEscapeRoute,
    standardRoutePlanCompleted: Boolean(
      GameState.inspectionPassed &&
      GameState.hammerHiddenInBible &&
      !GameState.alternateEscapeRoute
    ),
    cleanStageCount
  };
}

function resolveAndLockPlaystyle() {
  return PlaystyleSystem.lockResult(
    GameState.playstyle,
    getPlaystyleResolutionSnapshot()
  );
}
...
function isAmbientConversationSystemBlocked() {
  return GameState.scene !== "yard" ||
    ...
    OverheadThoughtSystem.isActive() ||
...
function getAmbientConversationStandingPositions(first, second) {
  const midpoint = {
    x: (first.x + second.x) / 2,
    y: (first.y + second.y) / 2
  };
  const halfDistance = AMBIENT_CHAT_STANDING_DISTANCE / 2;
  const searchOffsets = [0, -16, 16, -32, 32, -48, 48];

  for (const offsetY of searchOffsets) {
    for (const offsetX of searchOffsets) {
      const left = {
        x: midpoint.x + offsetX - halfDistance,
      ...
      if (!isYardNpcPointWalkable(left.x, left.y) || !isYardNpcPointWalkable(right.x, right.y)) {
        continue;
      }

      // 保留双方原本的左右顺序，避免交谈开始时互相穿过。
      return first.x <= second.x ? [left, right] : [right, left];
    }
  }
...
function stageAmbientConversationParticipants(first, second) {
  const positions = getAmbientConversationStandingPositions(first, second);
  if (!positions) {
    return false;
  }
  ...
  const leftActorIndex = positions[0].x < positions[1].x ? 0 : 1;
  actors[leftActorIndex].facing = "right";
  actors[1 - leftActorIndex].facing = "left";
...
function startAmbientConversation(option) {
  ...
  if (!stageAmbientConversationParticipants(first, second)) {
    return false;
  }
...
// ======================================================
// 9.5 Pipe Navigation Map
// ======================================================
const DefaultPipeNavigationRouteNodeIds = Object.freeze([
  "leftTop",
  "leftCurve",
  "leftOuterMid",
  "leftOuterJunction",
...
const PipeNavigationMapRenderCache = {
  canvas: null,
  maskCanvas: null,
  revision: -1,
  assetElement: null
...
function getPipeNetworkEdgeIndexBetweenNodes(aId, bId) {
  const graph = PipeNetworkLayout.graph;
  const node = graph.nodes[aId];
  if (!node || !graph.nodes[bId]) {
    return -1;
  }
  const edgeIndex = node.edgeIndexes.find((index) => {
    const edge = graph.edges[index];
    return (edge.aId === aId && edge.bId === bId) ||
      (edge.aId === bId && edge.bId === aId);
  });
  return Number.isInteger(edgeIndex) ? edgeIndex : -1;
}

function normalizePipeNavigationRouteNodeIds(value) {
  if (!Array.isArray(value)) {
    return null;
  }
  const graph = PipeNetworkLayout.graph;
  ...
  for (const item of value) {
    let id = typeof item === "string" ? item : item && item.id;
    if (id === "pipe-exit") {
      id = PipeNetworkLayout.exitNodeId;
    }
    if (typeof id !== "string" || !graph.nodes[id]) {
      return null;
    }
    if (nodeIds[nodeIds.length - 1] !== id) {
      nodeIds.push(id);
    }
  }
  if (nodeIds.length < 2 || nodeIds[0] !== "leftTop" ||
    nodeIds[nodeIds.length - 1] !== PipeNetworkLayout.exitNodeId) {
    return null;
  }
  for (let index = 1; index < nodeIds.length; index += 1) {
    if (getPipeNetworkEdgeIndexBetweenNodes(nodeIds[index - 1], nodeIds[index]) < 0) {
      return null;
    }
  }
  return nodeIds;
...
function derivePipeNavigationDrawnEdgeIds(routeNodeIds) {
  const edgeIds = [];
  const seen = new Set();
  for (let index = 1; index < routeNodeIds.length; index += 1) {
    const edgeIndex = getPipeNetworkEdgeIndexBetweenNodes(routeNodeIds[index - 1], routeNodeIds[index]);
    if (edgeIndex < 0) {
      return [];
    }
    const edgeId = PipeNetworkLayout.graph.edges[edgeIndex].id;
    if (!seen.has(edgeId)) {
      seen.add(edgeId);
      edgeIds.push(edgeId);
    }
...
function mergePipeNavigationRanges(ranges) {
  ...
  normalized.forEach((range) => {
    ...
    if (!previous || range[0] > previous[1] + 0.0001) {
...
function normalizePipeNavigationExploredRanges(value) {
  const normalized = {};
  const source = value && typeof value === "object" ? value : {};
  PipeNetworkLayout.graph.edges.forEach((edge) => {
    const ranges = mergePipeNavigationRanges(source[edge.id]);
    if (ranges.length) {
      normalized[edge.id] = ranges;
    }
  });
  return normalized;
}

function normalizePipeNavigationMapProgress(record) {
  if (!record || record.version !== PIPE_NAVIGATION_MAP_VERSION ||
    record.layoutId !== PIPE_NAVIGATION_MAP_LAYOUT_ID) {
    return null;
  }
  const routeNodeIds = normalizePipeNavigationRouteNodeIds(record.routeNodeIds);
  if (!routeNodeIds) {
    return null;
  }
  return {
    ...
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds,
    exploredEdgeRanges: normalizePipeNavigationExploredRanges(record.exploredEdgeRanges)
  };
}

function createDefaultPipeNavigationMapProgress() {
  return {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds: DefaultPipeNavigationRouteNodeIds.slice(),
    exploredEdgeRanges: {}
  };
}

function createEmptyPipeNavigationMapState(renderRevision) {
  return {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds: [],
    ...
    dirty: false,
    persistTimer: 0,
    renderRevision: Number.isFinite(renderRevision) ? renderRevision : 0
  };
}

function resetPipeNavigationMapProgress() {
  const previous = GameState.pipeNavigationMap;
  const nextRevision = previous && Number.isFinite(previous.renderRevision) ?
    previous.renderRevision + 1 : 1;
  GameState.pipeNavigationMap = createEmptyPipeNavigationMapState(nextRevision);
  PipeNavigationMapRenderCache.revision = -1;
}

function restorePipeNavigationMapProgress(record) {
  const normalized = normalizePipeNavigationMapProgress(record);
  if (!normalized) {
  ...
  const nextRevision = previous && Number.isFinite(previous.renderRevision) ?
    previous.renderRevision + 1 : 1;
  GameState.pipeNavigationMap = {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds: normalized.routeNodeIds.slice(),
    drawnEdgeIds: derivePipeNavigationDrawnEdgeIds(normalized.routeNodeIds),
    exploredEdgeRanges: normalizePipeNavigationExploredRanges(normalized.exploredEdgeRanges),
    expanded: false,
    storageLoaded: false,
    dirty: false,
    persistTimer: 0,
...
function capturePipeNavigationMapProgress() {
  const mapState = GameState.pipeNavigationMap;
  const routeNodeIds = normalizePipeNavigationRouteNodeIds(mapState && mapState.routeNodeIds);
  if (!routeNodeIds) {
    return null;
  }
  const exploredEdgeRanges = {};
  const normalizedRanges = normalizePipeNavigationExploredRanges(mapState.exploredEdgeRanges);
  Object.keys(normalizedRanges).forEach((edgeId) => {
    exploredEdgeRanges[edgeId] = normalizedRanges[edgeId].map((range) => range.slice());
  });
  return {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds: routeNodeIds.slice(),
    exploredEdgeRanges
  };
}

function ensurePipeNavigationMapRoute() {
  if (capturePipeNavigationMapProgress()) {
    return;
  }
  restorePipeNavigationMapProgress(createDefaultPipeNavigationMapProgress());
}

function setPipeNavigationMapRouteFromMazeCompletion(detail) {
  const routeNodeIds = detail && detail.layoutId === PIPE_NAVIGATION_MAP_LAYOUT_ID ?
    normalizePipeNavigationRouteNodeIds(detail.route) : null;
  const progress = routeNodeIds ? {
    version: PIPE_NAVIGATION_MAP_VERSION,
    layoutId: PIPE_NAVIGATION_MAP_LAYOUT_ID,
    routeNodeIds,
    exploredEdgeRanges: {}
  } : createDefaultPipeNavigationMapProgress();
  restorePipeNavigationMapProgress(progress);
  GameState.pipeNavigationMap.dirty = true;
  persistPipeNavigationMapProgress(true);
}

function pipeNavigationRouteSignature(record) {
  return record && Array.isArray(record.routeNodeIds) ? record.routeNodeIds.join("\u001f") : "";
}

function mergePipeNavigationExploredRangeRecords(first, second) {
  const merged = {};
  PipeNetworkLayout.graph.edges.forEach((edge) => {
    const ranges = mergePipeNavigationRanges(
      (first && first[edge.id] || []).concat(second && second[edge.id] || [])
    );
    if (ranges.length) {
      merged[edge.id] = ranges;
    }
  });
  return merged;
}

function loadPipeNavigationMapProgress() {
  let mapState = GameState.pipeNavigationMap;
  if (mapState.storageLoaded) {
    ensurePipeNavigationMapRoute();
    return;
  }
  mapState.storageLoaded = true;
  let stored = null;
  try {
    const serialized = localStorage.getItem(PIPE_NAVIGATION_MAP_STORAGE_KEY);
    stored = serialized ? normalizePipeNavigationMapProgress(JSON.parse(serialized)) : null;
  } catch (error) {
    reportNonFatalError("非致命操作失败", error);
  }

  const current = capturePipeNavigationMapProgress();
  if (!current && stored) {
    restorePipeNavigationMapProgress(stored);
    GameState.pipeNavigationMap.storageLoaded = true;
    return;
  }
  if (!current) {
    restorePipeNavigationMapProgress(createDefaultPipeNavigationMapProgress());
  ...
  mapState = GameState.pipeNavigationMap;
  if (stored && pipeNavigationRouteSignature(stored) === pipeNavigationRouteSignature(current)) {
    const mergedRanges = mergePipeNavigationExploredRangeRecords(
      current.exploredEdgeRanges,
      stored.exploredEdgeRanges
    );
    const changedFromCurrent = JSON.stringify(mergedRanges) !== JSON.stringify(current.exploredEdgeRanges);
    const changedFromStored = JSON.stringify(mergedRanges) !== JSON.stringify(stored.exploredEdgeRanges);
    mapState.exploredEdgeRanges = mergedRanges;
    mapState.renderRevision += changedFromCurrent ? 1 : 0;
    mapState.dirty = changedFromStored;
  } else if (stored) {
    mapState.dirty = true;
  }
  mapState.storageLoaded = true;
...
function persistPipeNavigationMapProgress(force) {
  const mapState = GameState.pipeNavigationMap;
  const record = capturePipeNavigationMapProgress();
  if (!record || (!force && !mapState.dirty)) {
    return;
  }
  try {
    localStorage.setItem(PIPE_NAVIGATION_MAP_STORAGE_KEY, JSON.stringify(record));
    mapState.storageLoaded = true;
...
function clearStoredPipeNavigationMapProgress() {
  try {
    localStorage.removeItem(PIPE_NAVIGATION_MAP_STORAGE_KEY);
  } catch (error) {
    reportNonFatalError("非致命操作失败", error);
  }
}

function revealPipeNavigationMapEdgeRange(edgeIndex, startProgress, endProgress) {
  const graph = PipeNetworkLayout.graph;
  const edge = graph.edges[edgeIndex];
  if (!edge || edge.length <= 0 || !Number.isFinite(startProgress) || !Number.isFinite(endProgress)) {
    return false;
  }
  ensurePipeNavigationMapRoute();
  const mapState = GameState.pipeNavigationMap;
  const range = [
    clamp(Math.min(startProgress, endProgress) / edge.length, 0, 1),
    clamp(Math.max(startProgress, endProgress) / edge.length, 0, 1)
  ];
  const previous = mapState.exploredEdgeRanges[edge.id] || [];
  const next = mergePipeNavigationRanges(previous.concat([range]));
  if (JSON.stringify(previous) === JSON.stringify(next)) {
    return false;
  }
  mapState.exploredEdgeRanges[edge.id] = next;
  mapState.dirty = true;
  if (!mapState.drawnEdgeIds.includes(edge.id)) {
    mapState.renderRevision += 1;
  }
  return true;
}

function getPipeNavigationMapDebugState() {
  const mapState = GameState.pipeNavigationMap;
  const graph = PipeNetworkLayout.graph;
  const currentEdge = graph.edges[GameState.pipeNetwork.edgeIndex] || null;
  const litEdgeIds = Array.from(new Set(
    mapState.drawnEdgeIds.concat(Object.keys(mapState.exploredEdgeRanges))
  ));
  return {
    layoutId: mapState.layoutId,
    routeNodeIds: mapState.routeNodeIds.slice(),
    drawnEdgeIds: mapState.drawnEdgeIds.slice(),
    litEdgeIds,
    exploredEdgeRanges: JSON.parse(JSON.stringify(mapState.exploredEdgeRanges)),
    currentEdgeId: currentEdge ? currentEdge.id : null,
    edgeProgressRatio: currentEdge && currentEdge.length > 0 ?
      GameState.pipeNetwork.edgeProgress / currentEdge.length : 0,
    markerSourceX: GameState.pipeNetwork.routeX / PIPE_NETWORK_WORLD_SCALE,
    markerSourceY: GameState.pipeNetwork.routeY / PIPE_NETWORK_WORLD_SCALE,
    expanded: mapState.expanded,
    hintActive: GameState.scene === "pipeNetwork" && GameState.pipeNetwork.entranceHintTimer > 0
  };
}

const PipeNavigationMapSystem = {
  enter() {
    loadPipeNavigationMapProgress();
    ensurePipeNavigationMapRoute();
    const mapState = GameState.pipeNavigationMap;
    mapState.expanded = false;
    revealPipeNavigationMapEdgeRange(
      GameState.pipeNetwork.edgeIndex,
      0,
      GameState.pipeNetwork.edgeProgress
    );
  },

  leave() {
    GameState.pipeNavigationMap.expanded = false;
    persistPipeNavigationMapProgress(true);
  },

  update(dt) {
    const mapState = GameState.pipeNavigationMap;
    if (GameState.scene !== "pipeNetwork" || !mapState.dirty) {
      return;
    }
    mapState.persistTimer += dt;
    if (mapState.persistTimer >= PIPE_NAVIGATION_MAP_PERSIST_INTERVAL_SECONDS) {
      persistPipeNavigationMapProgress(false);
    }
  },
  ...
  handleInput() {
    ...
    if (mapState.expanded) {
      const clickedClose = InputSystem.pointerJustPressed &&
        InputSystem.pointerInRect(PipeNavigationMapLayout.expandedCloseButton);
    ...
    const compactClicked = InputSystem.pointerJustPressed &&
      InputSystem.pointerInRect(PipeNavigationMapLayout.compactFrame);
    ...
    if ((mapKeyPressed || compactClicked) && !modalActive) {
      ...
      recordPlaystyleMapOpen("pipe");
      GameState.pipeNetwork.entranceHintTimer = 0;
      return true;
    }
  ...
  isWorldPaused() {
    return GameState.scene === "pipeNetwork" && GameState.pipeNavigationMap.expanded;
  },
  ...
  render(ctx) {
    ...
    if (GameState.pipeNavigationMap.expanded) {
      renderExpandedPipeNavigationMap(ctx);
      return;
    }
    renderCompactPipeNavigationMap(ctx);
    renderPipeNavigationMapEntranceHint(ctx);
  }
};

function ensurePipeNavigationMapRenderCanvases() {
  if (!PipeNavigationMapRenderCache.canvas) {
    PipeNavigationMapRenderCache.canvas = document.createElement("canvas");
    PipeNavigationMapRenderCache.canvas.width = PIPE_NAVIGATION_MAP_THUMBNAIL_WIDTH;
    PipeNavigationMapRenderCache.canvas.height = PIPE_NAVIGATION_MAP_THUMBNAIL_HEIGHT;
  }
  if (!PipeNavigationMapRenderCache.maskCanvas) {
    PipeNavigationMapRenderCache.maskCanvas = document.createElement("canvas");
    PipeNavigationMapRenderCache.maskCanvas.width = PIPE_NAVIGATION_MAP_THUMBNAIL_WIDTH;
    PipeNavigationMapRenderCache.maskCanvas.height = PIPE_NAVIGATION_MAP_THUMBNAIL_HEIGHT;
  }
}

function appendPipeNavigationMapEdgePath(ctx, edge, startRatio, endRatio) {
  const a = PipeNetworkSourceNodes[edge.aId];
  const b = PipeNetworkSourceNodes[edge.bId];
  if (!a || !b) {
    return;
  }
  const start = clamp(startRatio, 0, 1);
  const end = clamp(endRatio, 0, 1);
  let startX = a.x + (b.x - a.x) * start;
  let startY = a.y + (b.y - a.y) * start;
  let endX = a.x + (b.x - a.x) * end;
  let endY = a.y + (b.y - a.y) * end;
...
function rebuildPipeNavigationMapRenderSurface(assetElement) {
  ensurePipeNavigationMapRenderCanvases();
  const mapState = GameState.pipeNavigationMap;
  const surface = PipeNavigationMapRenderCache.canvas;
  const mask = PipeNavigationMapRenderCache.maskCanvas;
  ...
  maskCtx.save();
  maskCtx.scale(
    PIPE_NAVIGATION_MAP_THUMBNAIL_WIDTH / PipeNetworkLayout.imageWidth,
    PIPE_NAVIGATION_MAP_THUMBNAIL_HEIGHT / PipeNetworkLayout.imageHeight
  );
  maskCtx.beginPath();
  mapState.drawnEdgeIds.forEach((edgeId) => {
    const edgeIndex = PipeNetworkLayout.graph.edgeIndexById[edgeId];
    const edge = PipeNetworkLayout.graph.edges[edgeIndex];
    if (edge) {
      appendPipeNavigationMapEdgePath(maskCtx, edge, 0, 1);
    }
  });
  Object.keys(mapState.exploredEdgeRanges).forEach((edgeId) => {
    const edgeIndex = PipeNetworkLayout.graph.edgeIndexById[edgeId];
    const edge = PipeNetworkLayout.graph.edges[edgeIndex];
    if (!edge) {
      return;
    }
    mapState.exploredEdgeRanges[edgeId].forEach((range) => {
      appendPipeNavigationMapEdgePath(maskCtx, edge, range[0], range[1]);
    });
  });
  maskCtx.lineCap = "round";
  maskCtx.lineJoin = "round";
  maskCtx.lineWidth = PIPE_NAVIGATION_MAP_SOURCE_LINE_WIDTH;
  ...
  if (assetElement) {
    ...
    surfaceCtx.drawImage(assetElement, 0, 0, surface.width, surface.height);
  ...
  PipeNavigationMapRenderCache.revision = mapState.renderRevision;
  PipeNavigationMapRenderCache.assetElement = assetElement;
}

function getPipeNavigationMapRenderSurface() {
  ensurePipeNavigationMapRenderCanvases();
  const asset = getImageAssetRecord("pipe_network_map_thumbnail");
  const assetElement = asset && asset.loaded && !asset.failed ? asset.element : null;
  if (PipeNavigationMapRenderCache.revision !== GameState.pipeNavigationMap.renderRevision ||
    PipeNavigationMapRenderCache.assetElement !== assetElement) {
    rebuildPipeNavigationMapRenderSurface(assetElement);
  }
  return PipeNavigationMapRenderCache.canvas;
}

function mapPipeSourcePointToNavigationRect(point, rect) {
  return {
    x: rect.x + point.x / PipeNetworkLayout.imageWidth * rect.w,
    y: rect.y + point.y / PipeNetworkLayout.imageHeight * rect.h
  };
}

function renderPipeNavigationMapPlot(ctx, rect, expanded) {
  ctx.save();
  ctx.fillStyle = "#050706";
  ctx.fillRect(rect.x, rect.y, rect.w, rect.h);
  ctx.imageSmoothingEnabled = false;
  const surface = getPipeNavigationMapRenderSurface();
  ctx.drawImage(surface, rect.x, rect.y, rect.w, rect.h);
  ctx.fillStyle = "rgba(13, 18, 13, 0.12)";
  ctx.fillRect(rect.x, rect.y, rect.w, rect.h);
  ctx.strokeStyle = "rgba(218, 194, 127, 0.62)";
  ctx.lineWidth = expanded ? 3 : 2;
  ctx.strokeRect(rect.x + 1, rect.y + 1, rect.w - 2, rect.h - 2);
  renderPipeNavigationPlayerMarker(ctx, rect, expanded);
  ctx.restore();
}

function renderPipeNavigationPlayerMarker(ctx, rect, expanded) {
  const network = GameState.pipeNetwork;
  const point = mapPipeSourcePointToNavigationRect({
    x: network.routeX / PIPE_NETWORK_WORLD_SCALE,
    y: network.routeY / PIPE_NETWORK_WORLD_SCALE
  }, rect);
  ...
  ctx.lineWidth = expanded ? 3 : 2;
  ctx.shadowColor = "rgba(118, 215, 255, 0.7)";
...
function renderCompactPipeNavigationMap(ctx) {
  ctx.save();
  renderPipeNavigationMapPlot(ctx, PipeNavigationMapLayout.compactPlot, false);
  ctx.restore();
}

function renderExpandedPipeNavigationMap(ctx) {
  const frame = PipeNavigationMapLayout.expandedFrame;
  const plot = PipeNavigationMapLayout.expandedPlot;
  const closeButton = PipeNavigationMapLayout.expandedCloseButton;
  ctx.save();
  ...
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  ctx.fillText("地下水管图", frame.x + 30, frame.y + 30);
  ...
  renderPipeNavigationMapPlot(ctx, plot, true);
  renderPipeNavigationMapLegend(ctx, frame);
  ctx.restore();
}

function renderPipeNavigationMapLegend(ctx, frame) {
  const items = [
    { color: "#76d7ff", label: "安迪当前位置", shape: "circle" },
    { color: "#c9ad69", label: "已记录路线", shape: "line" },
    { color: "#050706", label: "尚未记录区域", shape: "square" }
  ];
  const x = frame.x + 28;
  const y = frame.y + 150;
  ctx.save();
  ...
  items.forEach((item, index) => {
    ...
    if (item.shape === "circle") {
      ctx.beginPath();
      ctx.arc(x + 8, itemY, 7, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(x, itemY);
      ctx.lineTo(x + 18, itemY);
      ctx.stroke();
    } else {
      ctx.fillRect(x, itemY - 8, 17, 17);
      ctx.strokeStyle = "rgba(218, 194, 127, 0.45)";
      ctx.lineWidth = 1;
      ctx.strokeRect(x, itemY - 8, 17, 17);
    }
    ctx.fillStyle = "#efe4c3";
...
function renderPipeNavigationMapEntranceHint(ctx) {
  if (!SHOW_GAMEPLAY_TEXT_HINTS || GameState.pipeNetwork.entranceHintTimer <= 0) {
    return;
  }
  const box = PipeNavigationMapLayout.entranceHint;
  const alpha = clamp(GameState.pipeNetwork.entranceHintTimer / 0.5, 0, 1);
  ctx.save();
  ctx.globalAlpha = alpha;
  renderPanel(ctx, box.x, box.y, box.w, box.h);
  ctx.fillStyle = "#f5df9d";
  ctx.font = "17px 'Microsoft YaHei', 'SimHei', sans-serif";
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  ctx.fillText("地图会显示你画过或亲自爬过的路线，蓝点表示当前位置。", box.x + 18, box.y + 27);
  ctx.fillStyle = "#ffffff";
  ctx.fillText("点击地图或按 M 查看全图。", box.x + 18, box.y + 56);
  ctx.restore();
}

// ======================================================
// 9.6 Courtyard Navigation Map
// ======================================================
const YardNavigationMapSystem = {
  enterYard() {
    ...
    if (!mapState.availableThisRun) {
      ...
      if (leftCellAfterFirstWake && !GameState.mapTutorialCompleted) {
        mapState.firstWalkIntroPending = true;
        GameState.currentQuest = "";
        return;
      }
    }
    this.activateForCurrentYard();
    if (GameState.mapTutorialCompleted && !GameState.yardRedIntroCompleted) {
      this.startPostTutorialThoughts();
    }
  },

  activateForCurrentYard() {
    const mapState = GameState.yardNavigationMap;
    mapState.firstWalkIntroPending = false;
  ...
  update(dt) {
    const mapState = GameState.yardNavigationMap;
    if (mapState.firstWalkIntroPending) {
      if (GameState.scene === "yard" && GameState.player.isMoving) {
        this.activateForCurrentYard();
      }
      return;
    }
    if (mapState.postTutorialThoughtActive) {
      if (!mapState.expanded && !DialogueSystem.active && !TutorialSystem.isActive()) {
        mapState.postTutorialThoughtTimer += dt;
        if (mapState.postTutorialThoughtTimer >= YARD_MAP_POST_TUTORIAL_THOUGHT_SECONDS) {
          mapState.postTutorialThoughtIndex += 1;
          mapState.postTutorialThoughtTimer = 0;
          if (mapState.postTutorialThoughtIndex >= YardMapPostTutorialThoughtLines.length) {
            mapState.postTutorialThoughtActive = false;
            mapState.postTutorialThoughtIndex = 0;
            GameState.yardRedIntroCompleted = true;
  ...
  handleInput() {
    ...
    if (mapState.expanded) {
      ...
      const clickedAchievement = InputSystem.pointerJustPressed &&
        InputSystem.pointerInRect(YardNavigationMapLayout.expandedAchievementButton);
  ...
  startPostTutorialThoughts() {
    if (GameState.scene !== "yard" || GameState.currentCheckpoint !== "CP_CELL_WAKE" ||
      GameState.hasHammer || GameState.yardRedIntroCompleted) {
      return;
    }
    const mapState = GameState.yardNavigationMap;
    mapState.postTutorialThoughtActive = true;
    mapState.postTutorialThoughtIndex = 0;
    mapState.postTutorialThoughtTimer = 0;
    GameState.currentQuest = "";
  }
...
const TutorialSystem = {
  ...
  update() {
    ...
    const readyAtOpeningYard = isOpeningYardArrival() &&
      GameState.currentCheckpoint === "CP_AFTER_WHITE_LIGHT" &&
      !DialogueSystem.active &&
      !OverheadThoughtSystem.isActive() &&
      !NarrativeCueSystem.isActive() &&
      !GameState.hammerHidePuzzle.active;
    if (readyAtOpeningYard) {
      this.start("wake");
    }
  },
  ...
  completeWakeTutorial() {
    ...
    if (isOpeningYardArrival()) {
      GameState.currentQuest = "quest_yard_first_walk";
    }
    InputSystem.resetAllInput();
    this.hideOverlay();
    saveCheckpoint(GameState.currentCheckpoint);
  ...
  completeMapTutorial() {
    ...
    YardNavigationMapSystem.startPostTutorialThoughts();
    persistYardNavigationMapProgress(true);
    InputSystem.resetAllInput();
    this.hideOverlay();
    saveCheckpoint(GameState.currentCheckpoint);
  ...
  setBodyCopy(prefix, emphasized, suffix) {
    const paragraph = document.createElement("p");
    paragraph.append(prefix);
    const strong = document.createElement("strong");
    strong.textContent = emphasized;
    paragraph.append(strong, suffix);
    this.elements.body.replaceChildren(paragraph);
  },
  ...
  syncOverlay() {
    ...
    if (kind === "wake" && GameState.tutorial.wakeStep === "movement") {
      ...
      this.setBodyCopy("拖动", "左侧摇杆", "控制移动，键盘可使用 WASD。");
      ...
      this.setBodyCopy("靠近发光的人物、物品或门后，使用", "右侧交互键", "进行交谈、调查或开门；对白中也可继续。键盘可按空格。");
      ...
      this.setBodyCopy("你走过的区域会逐步显现，", "黄色标记", "指向当前目标。");
...
function resetYardNavigationMapProgress(options) {
  ...
  if (preserveProgress) {
    ...
    GameState.yardNavigationMap.firstWalkIntroPending = false;
    GameState.yardNavigationMap.postTutorialThoughtActive = false;
    GameState.yardNavigationMap.postTutorialThoughtIndex = 0;
    GameState.yardNavigationMap.postTutorialThoughtTimer = 0;
    GameState.yardNavigationMap.expanded = false;
    return;
  }
...
const YardNavigationQuestTargetOverrides = Object.freeze({
  quest_yard_return_cell_to_hang_poster: "cellDoor",
  quest_yard_red_after_montage_done: "cellDoor",
  quest_cell_map_obtained: "wardenOffice",
  quest_yard_soil_done: "cellDoor"
});

function getYardNavigationQuestTarget() {
  const quest = GameState.currentQuest || "";
  const overrideLandmarkId = YardNavigationQuestTargetOverrides[quest];
  if (overrideLandmarkId) {
    return getYardNavigationLandmark(overrideLandmarkId);
  }
...
function renderExpandedYardNavigationMap(ctx) {
  ...
  const achievementButton = YardNavigationMapLayout.expandedAchievementButton;
  ctx.save();
  ...
  ctx.fillStyle = InputSystem.pointerInRect(achievementButton) ? "#7f6a38" : "#403621";
  ctx.fillRect(achievementButton.x, achievementButton.y, achievementButton.w, achievementButton.h);
  ctx.strokeStyle = "#d9c27f";
  ctx.lineWidth = 2;
  ctx.strokeRect(
    achievementButton.x + 1,
    achievementButton.y + 1,
    achievementButton.w - 2,
    achievementButton.h - 2
  );
  ctx.fillStyle = "#f5df9d";
  ...
  ctx.fillText(
    "成就",
    achievementButton.x + achievementButton.w / 2,
    achievementButton.y + achievementButton.h / 2
  );
  ctx.restore();
}
...
const QuestSystem = {
  labels: {
    quest_cell_hide_hammer: "典狱长即将检查房间。靠近桌子，把石锤藏进圣经。",
    quest_cell_wait_inspection: "石锤已经藏好。到床边站好，等待典狱长检查房间。",
    ...
    quest_yard_soil: "谨慎倒土，当心被狱警发现！！！",
    quest_yard_soil_partial: "谨慎倒土，当心被狱警发现！！！",
    quest_side_route_office_hide: "有人要进来！十秒内躲到上方窗帘后面。",
    quest_office_inspection_hide: "门外传来脚步声！十秒内躲到左上方窗帘后面。",
    quest_office_swap_ledger: "点击账本，用圣经完成调包，把真正的账本带走。"
  },

  thoughts: {
    quest_corridor_to_yard_morning: "院子里的气氛不太对，我得去看看。",
    quest_cell_prepare_confiscation_inspection: "检查要来了，可石锤还在我身上。",
    quest_yard_red: "我先在院子里看看，也许能找到瑞德。",
    quest_cell_soil_pile: "这些土不能留在房间，我得找机会带出去。",
    quest_cell_map_obtained: "地图已经有了，我该去找那本账了。",
    quest_dig_return: "土已经装好了，我得先离开这里。",
    quest_library_empty: "这里现在帮不上忙，我还是先回院子看看。",
    quest_library_wait_brooks: "老布还在外面，我得先和他谈谈。",
    quest_side_route_find_brooks: "也许老布知道些什么，我得去问问他。",
    quest_side_route_morning_patrol: "大门附近戒备森严，我得另找机会。",
    quest_side_route_office_search: "这里也许藏着证据，我得仔细搜一搜。",
    quest_side_route_office_leave: "证据已经到手，我得马上离开这里。",
    quest_office_embroidery: "我得仔细找找，账本一定藏在这间办公室里。",
    quest_side_route_awaiting_continuation: "眼下还没有更多线索，我只能先等等。",
    quest_yard_reserved_interaction: "我在这里暂时找不到新的线索。"
  },

  observedQuest: null,
  pendingThought: null,
  shownThoughtQuests: new Set(),
  ...
  resetRuntimeHints() {
    this.observedQuest = null;
    this.pendingThought = null;
    this.shownThoughtQuests.clear();
  },

  syncCurrentQuest() {
    const quest = GameState.currentQuest || "";
    if (quest === this.observedQuest) {
      return;
    }

    this.observedQuest = quest;
    this.pendingThought = null;
    const text = this.thoughts[quest] || "";
    if (!SHOW_GAMEPLAY_TEXT_HINTS || !text || this.shownThoughtQuests.has(quest)) {
      return;
    }
    this.pendingThought = { quest, text };
  },

  canStartPendingThought() {
    const supportedScene = GameState.scene === "cell" ||
    ...
    return supportedScene &&
      ...
      !OverheadThoughtSystem.isActive() &&
      !NarrativeCueSystem.isActive() &&
      !OpeningNoticeSystem.isActive() &&
      !TutorialSystem.isActive() &&
      !YardNavigationMapSystem.isWorldPaused() &&
      !PipeNavigationMapSystem.isWorldPaused() &&
      !isEvidenceViewerOpen() &&
      !GameState.radioRepairActive &&
      !GameState.posterChoiceActive &&
      !GameState.pipeMazeActive &&
      !GameState.hammerHidePuzzle.active &&
      !GameState.libraryTask.sortingActive &&
      !GameState.office.safeViewOpen &&
      GameState.scene !== "pause";
  },

  update() {
    this.syncCurrentQuest();
    if (!this.pendingThought) {
      return;
    }
    if (!SHOW_GAMEPLAY_TEXT_HINTS || GameState.currentQuest !== this.pendingThought.quest) {
      this.pendingThought = null;
      return;
    }
    if (!this.canStartPendingThought()) {
      return;
    }

    const pending = this.pendingThought;
    this.pendingThought = null;
    this.shownThoughtQuests.add(pending.quest);
    startOverheadThought(["安迪（心理）：" + pending.text]);
  },
...
function startCellWakePrompt() {
  GameState.cellWakePrompt.phase = "question";
  GameState.cellWakePrompt.timer = 0;
  GameState.currentQuest = "quest_cell_wake_question";
}

function handleCellWakePromptInput() {
  if (GameState.cellWakePrompt.phase !== "question") {
    return false;
  }

  const move = InputSystem.getMoveVector();
  const movementPressed = Math.abs(move.x) > 0.05 || Math.abs(move.y) > 0.05;
  const interactionPressed = InputSystem.actionPressed("interact");
  if (!movementPressed && !interactionPressed) {
    return false;
  }

  GameState.cellWakePrompt.phase = "explore";
  GameState.cellWakePrompt.timer = CELL_WAKE_EXPLORE_PROMPT_SECONDS;
  GameState.currentQuest = "quest_cell_wake_explore";
  return true;
}

function updateCellWakePrompt(dt) {
  if (GameState.cellWakePrompt.phase !== "explore") {
    return;
  }

  GameState.cellWakePrompt.timer = Math.max(0, GameState.cellWakePrompt.timer - dt);
  if (GameState.cellWakePrompt.timer > 0) {
    return;
  }

  GameState.cellWakePrompt.phase = "done";
  if (GameState.currentQuest === "quest_cell_wake_explore") {
    GameState.currentQuest = "";
  }
}
...
const CheckpointSystem = {
  ...
  save(checkpointKey) {
    const previousCheckpoint = GameState.currentCheckpoint;
    GameState.currentCheckpoint = checkpointKey;
    if (previousCheckpoint !== checkpointKey) {
      setRoamingGuardCheckpoint(checkpointKey, {
        immediate: !GameState.roamingNpcSystem.initialized,
        reason: "checkpoint"
      });
    }
    try {
  ...
  captureProgress() {
    const fields = [
      ...
      "wakeTutorialCompleted", "mapTutorialCompleted", "yardRedIntroCompleted"
    ];
    const progress = {};
    ...
    progress.sideTalk = captureSideTalkProgress();
    progress.yardMap = captureYardNavigationMapProgress();
    progress.pipeMap = capturePipeNavigationMapProgress();
    progress.playstyle = PlaystyleSystem.captureState(GameState.playstyle);
    return progress;
  },
  ...
  load() {
    try {
      ...
      if (!serialized) {
        return { checkpoint: legacyCheckpoint, progress: null, version: 0 };
      }
      const record = JSON.parse(serialized);
      if (record && [2, 3, 4, 5, 6, 7].includes(record.version) && record.checkpoint === legacyCheckpoint) {
        return { checkpoint: legacyCheckpoint, progress: record.progress || null, version: record.version };
      }
      return { checkpoint: legacyCheckpoint, progress: null, version: 0 };
    } catch (error) {
      reportNonFatalError("非致命操作失败", error);
  ...
  deleteAllSaveData() {
    const keys = getStorageKeysIncludingLegacy(
      YARD_NAVIGATION_MAP_STORAGE_KEY,
      PIPE_NAVIGATION_MAP_STORAGE_KEY,
      GAME_BGM_RESUME_KEY,
  ...
  clearCompletedRunOnEntry() {
    ...
    this.clear({ preserveYardMap: false });
    try {
      removeStorageKeysIncludingLegacy(GAME_BGM_RESUME_KEY);
    } catch (error) {
      reportNonFatalError("非致命操作失败", error);
    }
    return true;
  ...
  clear(options) {
    ...
    try {
      removeStorageKeysIncludingLegacy(
        CHECKPOINT_STORAGE_KEY,
        SAVE_DATA_STORAGE_KEY,
        PIPE_NAVIGATION_MAP_STORAGE_KEY
      );
      if (!preserveYardMap) {
        removeStorageKeysIncludingLegacy(YARD_NAVIGATION_MAP_STORAGE_KEY);
      }
    } catch (error) {
      reportNonFatalError("非致命操作失败", error);
    }
  ...
  restore() {
    ...
    restoreCheckpointProgress(record.checkpoint, record.progress, record.version);
    return true;
  }
};
...
function getCheckpointProgressDefaults(checkpointKey) {
  ...
  const mappedLateBase = Object.assign({}, lateBase, {
    mapDrawn: true,
    hasMap: true,
    pipeMap: createDefaultPipeNavigationMapProgress()
  });
  const defaultsByCheckpoint = {
    ...
    CP_MAP_OBTAINED: mappedLateBase,
    CP_OFFICE_DIALOGUE_DONE: mappedLateBase,
    CP_FINAL_DIG: Object.assign({}, mappedLateBase, {
      hasLedger: true,
      hasBible: false,
      bibleUsed: true,
      ledgerFound: true,
    ...
    CP_PIPE_GAME: Object.assign({}, mappedLateBase, {
      hasLedger: true,
      hasBible: false,
      bibleUsed: true,
      ledgerFound: true,
    ...
    CP_PIPE_NETWORK: Object.assign({}, mappedLateBase, {
      hasLedger: true,
      hasBible: false,
      bibleUsed: true,
      ledgerFound: true,
    ...
    CP_ENDING_SELECT: Object.assign({}, mappedLateBase, {
      hasLedger: true,
      hasBible: false,
      bibleUsed: true,
      ledgerFound: true,
  ...
  if (checkpointKey !== "CP_START" &&
    checkpointKey !== "CP_AFTER_WHITE_LIGHT" && checkpointKey !== "CP_CELL_WAKE") {
...
function applyCheckpointProgress(checkpointKey, storedProgress, sourceVersion) {
  ...
  Object.keys(persistentFields).forEach((field) => {
    if (field === "soilDumpCompletedCount" || field === "sideRouteActive" ||
      field === "sideRouteStage" || field === "sideTalk" || field === "yardMap" ||
      field === "pipeMap" || field === "playstyle") {
      return;
    }
  ...
  Object.keys(combined).forEach((field) => {
    if (field !== "sideTalk" && field !== "playstyle" &&
      Object.prototype.hasOwnProperty.call(GameState, field)) {
      GameState[field] = combined[field];
    }
  });
  const legacyPlaystyle = Number.isFinite(sourceVersion) && sourceVersion < 7;
  GameState.playstyle = PlaystyleSystem.restoreState(combined.playstyle, {
    legacyFallback: legacyPlaystyle
  });
  // 旧版存档会在调包账本后、hasAttributeC 置位前写入 CP_FINAL_DIG。
  // 该检查点固定恢复到牢房墙面，因此必须补齐墙面交互条件，避免永久软锁。
  ...
  restoreSideTalkProgress(combined.sideTalk);
  ...
  if (combined.pipeMap) {
    restorePipeNavigationMapProgress(combined.pipeMap);
  } else {
    resetPipeNavigationMapProgress();
  }
}

function resetAchievementTriggerTransientState() {
  GameState.secrets.menuClicks = 0;
  GameState.secrets.wallChecks = 0;
  GameState.secrets.montageWait = 0;
  GameState.secrets.yardCorners = {};
  GameState.secrets.officeSecretRead = false;
  GameState.achievementPanelOpen = false;
  GameState.achievementDetailId = null;
}

function resetGameStateForNewRun() {
  applyCheckpointProgress("CP_START", {});
  resetCheckpointTransientState();
  resetNewRunStoryState();
  GameState.currentCheckpoint = "CP_START";
  GameState.playTime = 0;
  GameState.fatalError = null;
  GameState.gameBgmStarted = false;
  GameState.gameBgmStopped = false;
...
function resetNewRunStoryState() {
  GameState.surgerySequence.timer = 0;
  GameState.surgerySequence.horrorPlayed = false;

  Object.assign(GameState.libraryTask, {
    brooksInside: false,
    sortingActive: false,
    sortingProgress: 0,
    bookOrder: [],
  ...
  Object.assign(GameState.guardConversation, {
    selectedSource: null,
    activeSource: null,
    activeGuardIndex: null
  });
  ...
  const red = GameState.redNpc;
  const redConfig = getRedNpcConfig();
  Object.assign(red, {
    initialized: false,
    mode: "patrol",
    facing: redConfig ? (redConfig.baseFacing || "down") : "down",
    isMoving: false,
    walkAnimTime: 0,
    pathDistance: 0,
    targetDistance: 0,
    ...
    crowdFidgetTimer: 0,
    fidgetTarget: null,
    narrativeWalkRoute: null,
    narrativeWalkRouteIndex: 0,
    narrativeWalkTarget: null
  });
  if (redConfig) {
    red.x = redConfig.x;
    red.y = redConfig.y;
    red.targetX = redConfig.x;
    red.targetY = redConfig.y;
  }
}

function resetCheckpointTransientState() {
  QuestSystem.resetRuntimeHints();
  ...
  TutorialSystem.resetActive();
  resetAchievementTriggerTransientState();
  ...
  GameState.pipeNavigationMap.expanded = false;
  GameState.sideTalk.activeNpcId = null;
  GameState.sideTalk.reapproachNpcIds = [];
  resetAmbientConversationTransientState();
...
function restoreCheckpointProgress(checkpointKey, storedProgress, sourceVersion) {
  resetCheckpointTransientState();
  applyCheckpointProgress(checkpointKey, storedProgress, sourceVersion);
  GameState.currentCheckpoint = checkpointKey;
  setRoamingGuardCheckpoint(checkpointKey, {
    force: true,
    immediate: true,
  ...
  if (checkpointKey === "CP_PIPE_NETWORK") {
    changeScene("pipeNetwork");
  }
}
...
// @test 偏右侧椭圆深棕色泥地均可撒土；持有小土堆时必须连续长按 5 秒才累计一次倒土，倒土期间锁定移动并播放逐帧动作；第 3 秒最近狱警会缓慢靠近，倒土结束即恢复随机巡逻。
...
function isYardGuardTouchingPlayer() {
  const playerFoot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  const guards = (GameState.hasSoilPile || GameState.soilDump.active) ?
    getNarrativeYardGuards() : GameState.yardGuards;
  return guards.some((guard) => (!guard.scene || guard.scene === "yard") &&
...
function alertClosestYardGuardToSoilDump() {
  ...
  const playerFoot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  const yardGuards = getNarrativeYardGuards().filter((guard) => !guard.scene || guard.scene === "yard");
  if (!soilInteraction || !isPointInYardInteraction(playerFoot, soilInteraction) || yardGuards.length === 0) {
    return;
  }

  const closestGuard = yardGuards.reduce((closest, guard) =>
...
function restoreSoilFailureToPileCheckpoint() {
  recordPlaystyleFailureRecovery("soilDump");
  DialogueSystem.active = false;
  DialogueSystem.lines = [];
  DialogueSystem.index = 0;
  DialogueSystem.justStarted = false;
...
function resetYardGuardsForSoilRetry() {
  ...
  initializeRoamingNpcSystem();
  GameState.yardGuards.forEach((guard) => {
    const narrativeIndex = guard.sourceIndex;
    const defaults = defaultGuards[narrativeIndex];
    if (!defaults) {
      guard.investigatingSoilDump = false;
      if (guard.scene === "yard") {
        guard.targetX = 0;
        guard.targetY = 0;
        guard.roamingMode = "patrol";
      }
      return;
    }
    guard.x = defaults.x;
    ...
    guard.walkAnimTime = 0;
    guard.scene = "yard";
    guard.roamingMode = "soilPatrol";
    guard.destinationScene = null;
    guard.roamingPath = [];
    guard.roamingPathIndex = 0;
    guard.travelElapsed = 0;
  });
  if (GameState.roamingNpcSystem) {
    GameState.roamingNpcSystem.activeTravelerId = null;
  }
}
...
function updateCellInspection(dt) {
  ...
  if (inspection.phase === "escortAssemble") {
    // 狱警与安迪统一使用安迪的最大画布步速，并沿房间正常可移动区域集合。
...
function restoreSideRouteYardAmbientActors() {
  ...
  resetYardGuardsForSoilRetry();
  GameState.roamingNpcSystem.initialized = false;
  GameState.roamingBackgroundPrisoners = [];
  initializeRedNpcState();
  initializeBrooksNpcState();
  initializeYardPrisoners();
}
...
function handleSolitaryInput() {
  ...
  } else if (interaction.id === "roamingNpc") {
    startRoamingNpcInteraction(interaction);
  }
}
...
function getActiveSolitaryInteraction() {
  ...
  return getNearbyRoamingNpcInteraction("solitary", foot);
}
...
function startSideRouteMorningPatrol() {
  ...
  getNarrativeYardGuards().forEach((guard, index) => {
    const path = sideRoute.morningGuardPaths[index];
    const start = path[0];
    rehomeRoamingNpc(guard, "yard", start);
    guard.roamingMode = "gatePatrol";
    guard.x = start.x;
    guard.y = start.y;
  ...
  GameState.yardGuards.filter((guard) => !isNarrativeYardGuard(guard)).forEach((guard) => {
    if (guard.scene === "yard") {
      guard.roamingMode = "patrol";
      guard.targetX = 0;
      guard.targetY = 0;
...
function startSideRouteGateCapture() {
  ...
  getNarrativeYardGuards().forEach((guard, index) => {
    const offset = SideRouteMorningLayout.captureOffsets[index];
...
function updateSideRouteGateCapture(dt) {
  ...
  stopPlayerForNpcDialogue();
  let everyoneArrived = true;
  getNarrativeYardGuards().forEach((guard) => {
...
function restoreSideRouteMorningFailure() {
  recordPlaystyleFailureRecovery("sideRouteMorning");
  GameState.failReason = "";
  GameState.failRecovery = null;
  GameState.failRecoveryTimer = 0;
  GameState.sideRoute.stage = "morningPatrol";
...
function failSideRouteOfficeHide() {
  GameState.failReason = "门外的脚步声停在门口。你还没藏好，被狱警抓住了。";
  GameState.failRecovery = "sideRouteOffice";
  changeScene("fail");
}

function restoreSideRouteOfficeFailure() {
  recordPlaystyleFailureRecovery("sideRouteOffice");
  GameState.failReason = "";
  GameState.failRecovery = null;
  GameState.sideRoute.stage = "officeSearch";
...
function completeCellInspectionSuccess() {
  ...
  saveCheckpoint("CP_INSPECTION_PASSED");
  GameState.currentQuest = "quest_cell_return_red_for_poster";
  startOverheadThought(InspectionAfterthoughtLines);
}
...
function showWallDigPrompt() {
  ...
  if (!GameState.inspectionPassed) {
    ...
    NarrativeCueSystem.schedule([
      "安迪（心理）：该死！典狱长来检查房间了，快在床边站好。"
    ], "quest_cell_commotion", "overhead");
    return;
  }
...
function hangPosterAtWall() {
  ...
  startOverheadThought(PosterHungThoughtLines);
}
...
function deliverDrawnMap() {
  ...
  if (GameState.alternateEscapeRoute) {
    ...
    startOverheadThought(AlternateRouteMapObtainedThoughtLines);
    return;
  }

  GameState.currentQuest = "quest_cell_map_obtained";
  saveCheckpoint("CP_MAP_OBTAINED");
  startOverheadThought(MapObtainedThoughtLines);
}
...
function initializePipeNetworkScene() {
  const network = GameState.pipeNetwork;
  const graph = PipeNetworkLayout.graph;
  const entranceEdgeIndex = graph.edgeIndexById[PipeNetworkLayout.entranceEdgeId];
  const entranceEdge = graph.edges[entranceEdgeIndex];
  // Begin far enough inside the left opening that Andy is fully visible when
  // control is handed back to the player.
  network.edgeIndex = entranceEdgeIndex;
  network.edgeProgress = Math.min(260, entranceEdge.length);
  network.progress = network.edgeProgress;
  network.lastTravelSign = 1;
  network.pendingTurnEdgeIndex = -1;
  network.pendingTurnNodeId = null;
  network.turnBufferTimer = 0;
  network.bufferedInputX = 0;
  network.bufferedInputY = 0;
  network.facing = "right";
  network.isMoving = false;
  network.walkAnimTime = 0;
  network.entranceHintTimer = 4;
  network.exitReached = false;
  updatePipeNetworkPosition(true);
  updatePipeNetworkCamera();
  GameState.currentQuest = "quest_pipe_network";
}

function updatePipeNetworkScene(dt) {
  const network = GameState.pipeNetwork;
  if (DialogueSystem.active || network.exitReached) {
    network.isMoving = false;
    return;
  }

  network.entranceHintTimer = Math.max(0, network.entranceHintTimer - dt);
  network.turnBufferTimer = Math.max(0, network.turnBufferTimer - dt);
  if (network.turnBufferTimer <= 0) {
    clearPipeNetworkTurnBuffer();
  }
  const move = InputSystem.getMoveVector();
  const magnitude = Math.hypot(move.x, move.y);
  const hasLiveInput = magnitude > 0.05;
  const hasBufferedInput = !hasLiveInput && network.turnBufferTimer > 0;
  if (!hasLiveInput && !hasBufferedInput) {
    network.isMoving = false;
    network.walkAnimTime = 0;
    updatePipeNetworkCamera();
    return;
  ...
  const inputX = hasLiveInput ? move.x / magnitude : network.bufferedInputX;
  const inputY = hasLiveInput ? move.y / magnitude : network.bufferedInputY;
  const choice = getPipeNetworkMovementChoice(inputX, inputY);

  if (hasLiveInput) {
    if (choice && choice.turn) {
      bufferPipeNetworkTurn(choice.turn, inputX, inputY);
    } else if (network.pendingTurnEdgeIndex >= 0) {
      clearPipeNetworkTurnBuffer();
    }
  }

  const previousCenterX = network.centerX;
  const previousCenterY = network.centerY;
  const frameDistance = PIPE_NETWORK_MOVE_SPEED * dt;
  if (choice) {
    const result = advancePipeNetwork(
      choice.sign,
      frameDistance * choice.strength,
      inputX,
      inputY
    );
    if (result.escaped) {
  ...
  // The route controls topology, but Andy's centre is free to move inside the
  // torso-sized band around it. Purely perpendicular input therefore produces
  // a small real movement instead of being discarded.
  network.centerX += inputX * frameDistance;
  network.centerY += inputY * frameDistance;
  updatePipeNetworkPosition(false);
  const actualDistance = Math.hypot(
    network.centerX - previousCenterX,
    network.centerY - previousCenterY
  );
  network.isMoving = actualDistance > 0.01;
  if (network.isMoving) {
    updatePipeNetworkFacing({ x: inputX, y: inputY });
    network.walkAnimTime += dt;
  }
  updatePipeNetworkCamera();
}

function getPipeNetworkMovementChoice(inputX, inputY) {
  const network = GameState.pipeNetwork;
  const edge = PipeNetworkLayout.graph.edges[network.edgeIndex];
  const choices = [
    getPipeNetworkEndpointChoice(edge, 1, inputX, inputY),
    getPipeNetworkEndpointChoice(edge, -1, inputX, inputY)
  ].filter(Boolean);
  if (!choices.length) {
    return null;
  ...
  return choices[0].strength > 0.08 ? choices[0] : null;
}

function getPipeNetworkEndpointChoice(edge, sign, inputX, inputY) {
  const network = GameState.pipeNetwork;
  const tangentX = edge.tangentX * sign;
  const tangentY = edge.tangentY * sign;
  const directIntent = inputX * tangentX + inputY * tangentY;
  const distanceToNode = sign > 0 ? edge.length - network.edgeProgress : network.edgeProgress;
  const nodeId = sign > 0 ? edge.bId : edge.aId;
  let strength = directIntent;
  let turn = null;

  if (distanceToNode <= PIPE_NETWORK_TURN_ASSIST_DISTANCE) {
    turn = getBestPipeNetworkTurn(nodeId, network.edgeIndex, inputX, inputY);
    if (turn && turn.score >= PIPE_NETWORK_TURN_MIN_DOT) {
      // Once the next branch is clearly requested, keep moving toward the node
      // even if the current segment is perpendicular to that input.
  ...
  return { sign, strength: clamp(strength, 0.35, 1), turn, nodeId, distanceToNode };
}

function getBestPipeNetworkTurn(nodeId, currentEdgeIndex, inputX, inputY) {
  const graph = PipeNetworkLayout.graph;
  const node = graph.nodes[nodeId];
  let best = null;
  node.edgeIndexes.forEach((edgeIndex) => {
    if (edgeIndex === currentEdgeIndex) {
      return;
    }
    const edge = graph.edges[edgeIndex];
    const sign = edge.aId === nodeId ? 1 : -1;
    const x = edge.tangentX * sign;
    const y = edge.tangentY * sign;
    const score = inputX * x + inputY * y;
    if (!best || score > best.score) {
      best = { edgeIndex, nodeId, sign, score, x, y };
    }
  });
  return best;
}

function bufferPipeNetworkTurn(turn, inputX, inputY) {
  const network = GameState.pipeNetwork;
  network.pendingTurnEdgeIndex = turn.edgeIndex;
  network.pendingTurnNodeId = turn.nodeId;
  network.turnBufferTimer = PIPE_NETWORK_TURN_BUFFER_SECONDS;
  network.bufferedInputX = inputX;
  network.bufferedInputY = inputY;
}

function clearPipeNetworkTurnBuffer() {
  const network = GameState.pipeNetwork;
  network.pendingTurnEdgeIndex = -1;
  network.pendingTurnNodeId = null;
  network.turnBufferTimer = 0;
...
function getBufferedPipeNetworkTurn(nodeId, currentEdgeIndex) {
  const network = GameState.pipeNetwork;
  if (
    network.turnBufferTimer <= 0 ||
    network.pendingTurnNodeId !== nodeId ||
    network.pendingTurnEdgeIndex < 0 ||
    network.pendingTurnEdgeIndex === currentEdgeIndex
  ) {
    return null;
  }
  const edge = PipeNetworkLayout.graph.edges[network.pendingTurnEdgeIndex];
  const sign = edge.aId === nodeId ? 1 : -1;
  return {
    edgeIndex: network.pendingTurnEdgeIndex,
    nodeId,
    sign,
    score: 1,
    x: edge.tangentX * sign,
    y: edge.tangentY * sign
  };
}

function advancePipeNetwork(initialSign, distance, inputX, inputY) {
  const network = GameState.pipeNetwork;
  const graph = PipeNetworkLayout.graph;
  ...
  while (remaining > 0.001 && transitions < 8) {
    const edge = graph.edges[network.edgeIndex];
    const distanceToNode = sign > 0 ? edge.length - network.edgeProgress : network.edgeProgress;
    const step = Math.min(remaining, Math.max(0, distanceToNode));
    const previousEdgeProgress = network.edgeProgress;
    network.edgeProgress = clamp(network.edgeProgress + sign * step, 0, edge.length);
    revealPipeNavigationMapEdgeRange(network.edgeIndex, previousEdgeProgress, network.edgeProgress);
    network.progress = network.edgeProgress;
    network.lastTravelSign = sign;
    updatePipeNetworkFacing({ x: edge.tangentX * sign, y: edge.tangentY * sign });
    moved += step;
    remaining -= step;

    if (distanceToNode - step > 0.001) {
      break;
    }

    const nodeId = sign > 0 ? edge.bId : edge.aId;
    if (nodeId === PipeNetworkLayout.exitNodeId) {
      completePipeNetworkEscape();
      return { distance: moved, escaped: true };
    }

    const nextTurn = getBufferedPipeNetworkTurn(nodeId, network.edgeIndex) ||
      getBestPipeNetworkTurn(nodeId, network.edgeIndex, inputX, inputY);
    if (!nextTurn || nextTurn.score < PIPE_NETWORK_TURN_MIN_DOT) {
      break;
    }

    network.edgeIndex = nextTurn.edgeIndex;
    const nextEdge = graph.edges[network.edgeIndex];
    network.edgeProgress = nextTurn.sign > 0 ? 0 : nextEdge.length;
    network.progress = network.edgeProgress;
    sign = nextTurn.sign;
    network.lastTravelSign = sign;
    clearPipeNetworkTurnBuffer();
  ...
  return { distance: moved, escaped: false };
}

function updatePipeNetworkFacing(travel) {
  const network = GameState.pipeNetwork;
  if (Math.abs(travel.x) >= Math.abs(travel.y)) {
    network.facing = travel.x < 0 ? "left" : "right";
  } else {
    network.facing = travel.y < 0 ? "up" : "down";
  }
}

function updatePipeNetworkPosition(snapPlayerToRoute) {
  const network = GameState.pipeNetwork;
  const graph = PipeNetworkLayout.graph;
  const edge = graph.edges[network.edgeIndex];
  const a = graph.nodes[edge.aId];
  network.routeX = a.x + edge.tangentX * network.edgeProgress;
  network.routeY = a.y + edge.tangentY * network.edgeProgress;

  if (snapPlayerToRoute || !Number.isFinite(network.centerX) || !Number.isFinite(network.centerY)) {
    network.centerX = network.routeX;
    network.centerY = network.routeY - PIPE_NETWORK_TORSO_CENTER_Y_OFFSET;
    return;
  }

  const torsoCenterX = clamp(
    network.centerX,
    network.routeX - PIPE_NETWORK_TORSO_HALF_W,
    network.routeX + PIPE_NETWORK_TORSO_HALF_W
  );
  const torsoCenterY = clamp(
    network.centerY + PIPE_NETWORK_TORSO_CENTER_Y_OFFSET,
    network.routeY - PIPE_NETWORK_TORSO_HALF_H,
    network.routeY + PIPE_NETWORK_TORSO_HALF_H
  );
  network.centerX = torsoCenterX;
  network.centerY = torsoCenterY - PIPE_NETWORK_TORSO_CENTER_Y_OFFSET;
}

function updatePipeNetworkCamera() {
  const network = GameState.pipeNetwork;
  network.cameraX = clamp(
    network.centerX - CANVAS_WIDTH / 2,
    0,
    Math.max(0, PipeNetworkLayout.worldWidth - CANVAS_WIDTH)
  );
  network.cameraY = clamp(
    network.centerY - CANVAS_HEIGHT / 2,
    0,
    Math.max(0, PipeNetworkLayout.worldHeight - CANVAS_HEIGHT)
  );
}

function completePipeNetworkEscape() {
  const network = GameState.pipeNetwork;
  if (network.exitReached) {
    return;
  }
  network.exitReached = true;
  network.isMoving = false;
  // Reuse the established success presentation only after the player has
  // physically reached the right-hand opening in the new large scene.
  changeScene("pipe");
...
function openEndingSelectFile() {
  const escapeStyle = resolveAndLockPlaystyle();
  saveCheckpoint("CP_ENDING_SELECT");
  saveGameBgmResumeTime();
  if (GlobalControls) {
    GlobalControls.releaseAll();
  }
  if (!window.BeyondWallsEndingHost) {
    handleFatalError(new Error("Ending host is unavailable."));
    return false;
  }
  window.BeyondWallsEndingHost.open(resolveEndingId(), {
    escapeStyle,
    evidenceSnapshot: getEndingEvidenceSnapshot()
  });
  return true;
}
...
function failPipeNoiseByGuard() {
  GameState.failReason = "狱警听到了声音，你要被抓了，脱困失败。";
...
function failPipeDrowned() {
  ...
  GameState.failRecovery = "pipeOpened";
  resetPipeSmashState();
  changeScene("fail");
}

function restorePipeDrownFailureToOpenedPipe() {
  recordPlaystyleFailureRecovery("pipeOpened");
  DialogueSystem.active = false;
  DialogueSystem.lines = [];
  DialogueSystem.index = 0;
  DialogueSystem.justStarted = false;
  ...
  const pipe = GameState.pipe;
  pipe.smashSuccessCount = PIPE_SMASH_REQUIRED_COUNT;
  pipe.smashCompleted = true;
  pipe.smashWindowScored = false;
  pipe.cueVisible = false;
  pipe.cueLit = false;
  ...
  pipe.drownTimer = 0;
  pipe.playerX = clamp(
    PipeLayout.dropZone.x - pipe.playerW - 12,
    PipeLayout.walkRect.x,
    PipeLayout.walkRect.x + PipeLayout.walkRect.w - pipe.playerW
  );
  pipe.playerY = getTunnelPlayerY(PipeLayout, pipe.playerH);
  pipe.facing = "right";
  GameState.currentQuest = "quest_pipe_opened";
}

function restorePipeFailureToFinalDigCheckpoint() {
  recordPlaystyleFailureRecovery("pipeBeforeTunnel");
  DialogueSystem.active = false;
  DialogueSystem.lines = [];
  DialogueSystem.index = 0;
  DialogueSystem.justStarted = false;
...
function isMainStoryOfficeEntryAllowed() {
  return hasOfficeAttributeA() && !GameState.ledgerSwapped;
}

function isOfficeTaskEntryAllowed() {
  return isMainStoryOfficeEntryAllowed() || (
    GameState.sideRoute.active && GameState.sideRoute.stage === "officeSearch"
  );
}

function captureUnauthorizedOfficeEntrySnapshot() {
  GameState.unauthorizedOfficeEntrySnapshot = {
...
function clearUnauthorizedOfficeEntrySnapshot() {
  GameState.unauthorizedOfficeEntrySnapshot = null;
}

function restoreUnauthorizedOfficeEntrySnapshot() {
  const snapshot = GameState.unauthorizedOfficeEntrySnapshot;
  if (!snapshot || snapshot.scene !== "yard") {
    clearUnauthorizedOfficeEntrySnapshot();
    changeScene("yard");
  ...
  changeScene("yard");
  GameState.previousScene = snapshot.previousScene;
  GameState.currentQuest = snapshot.currentQuest;
  GameState.currentCheckpoint = snapshot.currentCheckpoint;
  GameState.player.x = snapshot.player.x;
  GameState.player.y = snapshot.player.y;
  GameState.player.facing = snapshot.player.facing;
  GameState.player.vx = 0;
  GameState.player.vy = 0;
  GameState.player.isMoving = false;
  GameState.player.walkAnimTime = 0;
  GameState.camera.x = snapshot.camera.x;
  GameState.camera.y = snapshot.camera.y;
  clearUnauthorizedOfficeEntrySnapshot();
}
...
function enterFreeOfficeScene() {
  if (!GameState.unauthorizedOfficeEntrySnapshot && GameState.previousScene === "yard") {
    captureUnauthorizedOfficeEntrySnapshot();
  }
  resetOfficeSceneState();
  GameState.player.lyingInBed = false;
  ...
  if (!GameState.officeFirstWarningSeen) {
    ...
    saveCheckpoint(GameState.currentCheckpoint);
    DialogueSystem.start([
      "典狱长：谁允许你进来的？这里不是你该来的地方。",
      "典狱长：马上出去。再让我看到你擅闯，就让狱警把你关起来。"
    ]);
    return;
  }
  ...
  GameState.office.wardenPhase = "gone";
  GameState.office.inspectionScheduled = true;
  GameState.office.inspectionTimer = randomRange(
    OFFICE_INSPECTION_DELAY_MIN_SECONDS,
    OFFICE_INSPECTION_DELAY_MAX_SECONDS
  );
...
function failFreeOfficeInspection() {
  const visitorName = GameState.office.inspectionVisitorKind === "warden" ? "典狱长" : "狱警";
  GameState.failReason = "门外的脚步声停在门口。你还没藏好，被" + visitorName + "抓住了。";
  GameState.failRecovery = "officeInspection";
  changeScene("fail");
}

function restoreFreeOfficeInspectionFailure() {
  recordPlaystyleFailureRecovery("officeInspection");
  GameState.failReason = "";
  GameState.failRecovery = null;
  GameState.failRecoveryTimer = 0;
  restoreUnauthorizedOfficeEntrySnapshot();
}
...
function startOfficeWardenDialogue() {
  ...
  GameState.currentQuest = "quest_office_warden";
  DialogueSystem.start([
    "典狱长：安迪，找我有事？这些年你为拘禁地打理账务，做得还行啊。",
    "安迪：我只是来确认账目和文件，先生。",
    "典狱长：你给我好好干，我回去睡觉了。"
  ]);
}
...
function updateOfficeWarden(dt) {
  ...
  if (remaining <= 8) {
    ...
    startOverheadThought(OfficeWardenGoneThoughtLines);
    return;
  }
...
function swapOfficeLedger() {
  ...
  GameState.finalDigUnlocked = true;
  // CP_FINAL_DIG 会直接恢复到牢房墙面，保存前必须满足最终挖掘的交互门槛。
  GameState.hasAttributeC = true;
  GameState.currentQuest = "quest_office_leave";
  saveCheckpoint("CP_FINAL_DIG");
  startOverheadThought([
...
function leaveOfficeScene() {
  if (isFreeOfficeMode() && GameState.unauthorizedOfficeEntrySnapshot) {
    restoreUnauthorizedOfficeEntrySnapshot();
    return;
  }
...
const MiniGameHost = {
  ...
  open(type) {
    ...
    this.shell.classList.remove("is-full-page", "is-radio-repair");
    this.shell.classList.toggle("is-fixed-ratio", type === "bookshelf");
    this.shell.classList.toggle("is-radio-repair", type === "gear");
    this.overlay.classList.add("is-open");
    this.overlay.setAttribute("aria-hidden", "false");
  ...
  async mountComponent(type, config, token) {
    ...
    if (!styles || !styles.miniGameFont || !styles[config.styleKey]) {
      throw new Error("Bundled mini-game styles are unavailable: " + type);
    }
    const style = document.createElement("style");
    style.textContent = styles.miniGameFont + "\n" + styles[config.styleKey];
  ...
  close(restoreFocus = true) {
    ...
    this.viewport.replaceChildren();
    if (this.shell) {
      this.shell.classList.remove("is-fixed-ratio", "is-full-page", "is-radio-repair");
    }
    ...
    if (restoreFocus && canvas) {
      try {
        ...
        reportExpectedBrowserLimitation("浏览器不支持 preventScroll 聚焦选项", error);
        canvas.focus();
      }
    }
  },
  ...
  cancelCompletionTimer() {
    ...
    window.clearTimeout(this.completionTimerId);
  ...
  handleComponentEvent(message) {
    ...
    if (completedPipeMaze) {
      setPipeNavigationMapRouteFromMazeCompletion(message.detail);
    }
    ...
    const completedMountToken = this.mountToken;
    this.completionPending = true;
    this.completionTimerId = window.setTimeout(() => {
      this.completionTimerId = null;
      if (completedMountToken !== this.mountToken || completedType !== this.type || !this.completionPending) {
        return;
      }
      this.close();
...
function updateBrooksDialogueReward() {
  ...
  if (GameState.brooksDialogueKind === "libraryDelivery" && !DialogueSystem.active) {
    ...
    enableSideTalksAfterMain("brooks");
  }
}
...
function updatePlayer(dt) {
  ...
  if (isOpeningYardArrival()) {
    move = applyOpeningYardGateApproachSlowdown(move);
  }
  const player = GameState.player;
...
function updateOpeningYardPlayer(dt) {
  updatePlayer(dt);
  updateOpeningYardGateApproach(dt);
}

function isPlayerInOpeningYardGateApproachZone() {
  if (!isOpeningYardArrival()) {
    return false;
  }
  const foot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  return pointInRect(foot, OpeningYardLayout.gateApproachZone);
}

function applyOpeningYardGateApproachSlowdown(move) {
  if (!isPlayerInOpeningYardGateApproachZone()) {
    return move;
  }

  const foot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  const gate = OpeningYardLayout.gateLightSeam;
  const gateDx = gate.x - foot.x;
  const gateDy = gate.thresholdY - foot.y;
  const gateDistance = Math.hypot(gateDx, gateDy);
  if (gateDistance === 0) {
    return move;
  }

  const towardX = gateDx / gateDistance;
  const towardY = gateDy / gateDistance;
  const towardAmount = move.x * towardX + move.y * towardY;
  if (towardAmount <= 0) {
    return move;
  }

  const slowedAmount = towardAmount * (1 - OpeningYardLayout.gateApproachSpeedScale);
  return {
    x: move.x - towardX * slowedAmount,
    y: move.y - towardY * slowedAmount
  };
}

function updateOpeningYardGateApproach(dt) {
  const opening = GameState.opening;
  if (isPlayerInOpeningYardGateApproachZone()) {
    opening.gateApproachActive = true;
  }
  if (opening.gateApproachActive) {
    opening.gateApproachTimer += dt;
  }
}
...
function getActiveYardInteraction() {
  ...
  // 有待完成的 NPC 主线交谈拥有绝对优先级，包括与场景交互区重叠时。
  const npcInteraction = getNearbyYardNpcInteraction(foot);
  if (npcInteraction && getPendingNpcMainDialogueKind(npcInteraction.id)) {
    return npcInteraction;
  }

  // 没有 NPC 主线时，倒土任务继续优先于闲聊和取证，避免误触附近人物。
  const soilInteraction = getYardInteraction("soil");
  if (GameState.hasSoilPile && soilInteraction && isPointInYardInteraction(foot, soilInteraction)) {
    return soilInteraction;
  }

  let interaction = YardLayout.interactions.find((item) => isPointInYardInteraction(foot, item)) || null;
  if (interaction && interaction.id === "prisonGate") {
    if (isOutsidePlanEscapeAvailable()) {
      return Object.assign({}, interaction, { id: "outsidePlanGate", label: "尝试逃离" });
    }
    if (!isPrisonGateEscapeAvailable()) {
      interaction = null;
    }
  }
  ...
  return interaction || npcInteraction;
}
...
function isOutsidePlanEscapeAvailable() {
  return GameState.alternateEscapeRoute &&
    GameState.inspectionPassed &&
    !GameState.sideRoute.active &&
    !GameState.posterType &&
    !GameState.outsidePlanEscape.seenThisRun;
}
...
function startOutsidePlanEscape() {
  if (!isOutsidePlanEscapeAvailable()) {
    return;
  }
  recordPlaystyleOutsidePlanEscape();
  const escape = GameState.outsidePlanEscape;
  const foot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  escape.phase = "running";
  escape.timer = 0;
  escape.startFoot = { x: foot.x, y: foot.y };
  escape.returnQuest = GameState.currentQuest;
  escape.seenThisRun = true;
  GameState.player.facing = "up";
  changeScene("outsidePlanEscape");
}

function updateOutsidePlanEscape(dt) {
  const escape = GameState.outsidePlanEscape;
  if (escape.phase === "running") {
    escape.timer = Math.min(OUTSIDE_PLAN_RUN_SECONDS, escape.timer + dt);
    const progress = clamp(escape.timer / OUTSIDE_PLAN_RUN_SECONDS, 0, 1);
    const start = escape.startFoot || { x: 620, y: 260 };
    const imageX = start.x + (620 - start.x) * progress;
    const imageY = start.y + (165 - start.y) * progress;
    setOutsidePlanPlayerFoot(imageX, imageY, dt);
    updateYardCamera();
    if (progress >= 1) {
      GameState.player.isMoving = false;
      GameState.player.walkAnimTime = 0;
      escape.phase = "blackout";
      escape.timer = 0;
    }
    return;
  }

  if (escape.phase === "blackout") {
    escape.timer += dt;
    if (escape.timer >= OUTSIDE_PLAN_BLACK_SECONDS) {
      escape.phase = "dialogue";
      escape.timer = 0;
      DialogueSystem.start(OutsidePlanDialogueLines, "dialogue");
    }
    return;
  }

  if (escape.phase === "dialogue" && !DialogueSystem.active) {
    finishOutsidePlanEscape();
  }
}

function setOutsidePlanPlayerFoot(imageX, imageY, dt) {
  const world = yardImageToWorldPoint(imageX, imageY);
  const player = GameState.player;
  player.x = world.x - player.w / 2;
  player.y = world.y - player.h + 8;
  player.vx = 0;
...
function finishOutsidePlanEscape() {
  const escape = GameState.outsidePlanEscape;
  const returnQuest = escape.returnQuest || "quest_cell_return_red_for_poster";
  AchievementSystem.unlock("outside_the_plan");
  escape.phase = "idle";
  escape.timer = 0;
  escape.startFoot = null;
  escape.returnQuest = "";
  changeScene("yard");
  setPlayerFootToYardImage(620, 320);
  GameState.player.facing = "down";
  GameState.currentQuest = returnQuest;
  updateYardCamera();
}

function renderOutsidePlanEscape(ctx) {
  if (GameState.outsidePlanEscape.phase === "running") {
    renderYard(ctx);
    return;
  }
...
function resetOutsidePlanEscapeState() {
  const escape = GameState.outsidePlanEscape;
  escape.phase = "idle";
  escape.timer = 0;
  escape.startFoot = null;
  escape.returnQuest = "";
  escape.seenThisRun = false;
}
...
function getNearbyYardNpcInteraction(foot) {
  ...
  GameState.yardPrisoners.forEach((prisoner) => {
    if ((!prisoner.scene || prisoner.scene === "yard") &&
      (prisoner.assetKey === "tommy" || prisoner.assetKey === "haywood" || prisoner.assetKey === "floyd") &&
  ...
  if (canStartGuardConversation() && soilInteraction) {
    GameState.yardGuards.forEach((guard, index) => {
      if ((!guard.scene || guard.scene === "yard") &&
        isPointInYardInteraction({ x: guard.x, y: guard.y }, soilInteraction)) {
        candidates.push({
          id: "yardGuard",
          label: "与巡逻狱警交谈",
          npc: guard,
...
function getNearbySideRouteGateGuardInteraction(foot) {
  ...
  const nearby = getNarrativeYardGuards()
  ...
  return nearby ? { id: "sideRouteGuard", label: "与大门狱警交谈", guardIndex: nearby.index } : null;
}
...
function getActiveLibraryInteraction() {
  ...
  return getNearbyRoamingNpcInteraction("library", foot);
}

function getPlayerImagePointForRoamingScene(sceneName) {
  const foot = getPlayerFootPoint(GameState.player.x, GameState.player.y);
  if (sceneName === "yard") return yardWorldToImagePoint(foot);
  if (sceneName === "library") return canvasPointToLibraryImage(foot);
  if (sceneName === "cellCorridor") return corridorWorldToImagePoint(foot);
  if (sceneName === "cell") return canvasPointToCellImage(foot);
  if (sceneName === "solitary") return foot;
  return null;
}

function getNearbyRoamingNpcInteraction(sceneName, playerPoint) {
  if (!playerPoint || DialogueSystem.active) {
    return null;
  }
  const candidates = getRoamingNpcsInScene(sceneName).filter((agent) => {
    if (agent.source === "background" || agent.roamingMode === "travel") {
      return false;
    }
    if (agent.role === "guard") {
      return canStartGuardConversation();
    }
    return canStartSideTalk(agent.roamingId);
  }).map((agent) => ({
    agent,
    range: agent.role === "guard" ? 78 : 82,
    distance: distance(playerPoint.x, playerPoint.y, agent.x, agent.y)
  })).filter((entry) => entry.distance <= entry.range)
    .sort((first, second) => first.distance - second.distance);
  const nearest = candidates[0];
  return nearest ? {
    id: "roamingNpc",
    npcId: nearest.agent.roamingId,
    guardIndex: nearest.agent.role === "guard" ? nearest.agent.sourceIndex : null
  } : null;
}

function startRoamingNpcInteraction(interaction) {
  if (!interaction || interaction.id !== "roamingNpc") {
    return false;
  }
  const agent = getRoamingNpcById(interaction.npcId);
  if (!agent || agent.scene !== GameState.scene) {
    return false;
  }
  const playerPoint = getPlayerImagePointForRoamingScene(agent.scene);
  if (playerPoint) {
    agent.facing = directionFromDelta(playerPoint.x - agent.x, playerPoint.y - agent.y);
    agent.visualFacing = agent.facing;
    GameState.player.facing = directionFromDelta(agent.x - playerPoint.x, agent.y - playerPoint.y);
  }
  stopRoamingNpc(agent);
  if (agent.role === "guard") {
    startGuardConversation("roaming", agent.sourceIndex);
  } else {
    startSideTalk(agent.roamingId);
  }
  return true;
}
...
function updateYardPrisoners(dt) {
  ...
  GameState.yardPrisoners.forEach((prisoner) => {
    if ((prisoner.scene && prisoner.scene !== "yard") || prisoner.roamingMode === "travel") {
      return;
    }
...
const RoamingNavigationGridCache = Object.create(null);

function getRoamingNpcConfig(npcId) {
  return RoamingNpcConfigs.find((config) => config.id === npcId) || null;
}

function decorateRoamingNpc(agent, config) {
  if (!agent || !config) {
    return agent;
  }
  agent.roamingId = config.id;
  agent.role = config.role;
  agent.source = config.source;
  agent.sourceIndex = config.sourceIndex;
  agent.scene = agent.scene || "yard";
  agent.roamingMode = agent.roamingMode || "patrol";
  agent.destinationScene = agent.destinationScene || null;
  agent.roamingPath = Array.isArray(agent.roamingPath) ? agent.roamingPath : [];
  agent.roamingPathIndex = Number.isFinite(agent.roamingPathIndex) ? agent.roamingPathIndex : 0;
  agent.travelElapsed = Number.isFinite(agent.travelElapsed) ? agent.travelElapsed : 0;
  agent.waitTimer = Number.isFinite(agent.waitTimer) ? agent.waitTimer : randomRange(0.4, 1.4);
  agent.h = agent.h || YARD_CHARACTER_HEIGHT;
  agent.assetKey = agent.assetKey || config.assetKey;
  agent.name = typeof agent.name === "string" ? agent.name : config.name;
  agent.speed = Number.isFinite(agent.speed) && agent.speed > 0 ? agent.speed : getRoamingNpcDefaultSpeed(config);
  agent.facing = agent.facing || "down";
  agent.visualFacing = agent.visualFacing || agent.facing;
  agent.isMoving = Boolean(agent.isMoving);
  agent.walkAnimTime = Number.isFinite(agent.walkAnimTime) ? agent.walkAnimTime : 0;
  agent.targetX = Number.isFinite(agent.targetX) ? agent.targetX : agent.x;
  agent.targetY = Number.isFinite(agent.targetY) ? agent.targetY : agent.y;
  return agent;
}

function getRoamingNpcDefaultSpeed(config) {
  if (config.role === "guard") {
    return 54 + config.sourceIndex * 3;
  }
  if (config.source === "background") {
    return 48 + config.sourceIndex * 5;
  }
  const namedConfig = YardRandomPrisonerConfigs[config.sourceIndex];
  return namedConfig ? (namedConfig.minSpeed + namedConfig.maxSpeed) / 2 : 52;
}

function createRoamingBackgroundPrisoner(config) {
  const source = YardLayout.backgroundPrisoners[config.sourceIndex];
  const start = source && source.path && source.path[0] ? source.path[0] : YardLayout.entryPoint;
  return decorateRoamingNpc({
    assetKey: config.assetKey,
    name: "",
    x: start.x,
    y: start.y,
    targetX: start.x,
    targetY: start.y,
    h: source && source.h ? source.h : YARD_CHARACTER_HEIGHT,
    facing: ["right", "down", "left"][config.sourceIndex % 3],
    isMoving: false,
    walkAnimTime: randomRange(0, PRISONER_WALK_FRAME_SECONDS * 4),
    walkFrameSeconds: PRISONER_WALK_FRAME_SECONDS,
    idleFrameIndex: 1
  }, config);
}

function rollRoamingGuardCount(randomValue) {
  let roll = clamp(Number.isFinite(randomValue) ? randomValue : Math.random(), 0, 0.999999999);
  for (let index = 0; index < ROAMING_GUARD_COUNT_WEIGHTS.length; index += 1) {
    roll -= ROAMING_GUARD_COUNT_WEIGHTS[index].weight;
    if (roll < 0) {
      return ROAMING_GUARD_COUNT_WEIGHTS[index].count;
    }
  }
  return ROAMING_GUARD_MAX_COUNT;
}

function getRoamingGuardConfig(sourceIndex) {
  return getRoamingNpcConfig("guard_" + (sourceIndex + 1));
}

function getNarrativeYardGuards() {
  return GameState.yardGuards
    .filter((guard) => guard && guard.sourceIndex < ROAMING_GUARD_MIN_COUNT)
    .sort((first, second) => first.sourceIndex - second.sourceIndex);
}

function isNarrativeYardGuard(guard) {
  return Boolean(guard && guard.sourceIndex < ROAMING_GUARD_MIN_COUNT);
}

function findDispersedYardGuardSpawn(existingGuards) {
  const bounds = getYardWalkBounds();
  let fallback = null;
  for (let attempt = 0; attempt < 240; attempt += 1) {
    ...
    if (existingGuards.every((guard) =>
      guard.scene !== "yard" || distance(guard.x, guard.y, point.x, point.y) >= ROAMING_GUARD_SPAWN_SPACING
    )) {
      return point;
    }
  }
  return fallback || { x: YardLayout.entryPoint.x, y: YardLayout.entryPoint.y };
}

function getRoamingGuardSceneSpawn(sceneName, sourceIndex, existingGuards) {
  if (sceneName === "yard") {
    return findDispersedYardGuardSpawn(existingGuards);
  }
  const preferredByScene = {
    library: { x: 626, y: 1010 },
    cellCorridor: { x: 376, y: 1120 },
    solitary: { x: 440, y: 440 }
  };
  const preferred = preferredByScene[sceneName] || { x: YardLayout.entryPoint.x, y: YardLayout.entryPoint.y };
  const navigation = getRoamingSceneNavigation(sceneName);
  const offsets = [
    [0, 0], [32, 0], [-32, 0], [0, 32], [0, -32],
    [64, 0], [-64, 0], [32, 32], [-32, 32], [0, 64]
  ];
  const rotated = offsets.slice(sourceIndex % offsets.length).concat(offsets.slice(0, sourceIndex % offsets.length));
  return rotated.map((offset) => ({ x: preferred.x + offset[0], y: preferred.y + offset[1] }))
    .find((point) => navigation && navigation.isWalkable(point) && existingGuards.every((guard) =>
      guard.scene !== sceneName || distance(guard.x, guard.y, point.x, point.y) >= 32
    )) || preferred;
}

function createRoamingYardGuard(sourceIndex, sceneName, existingGuards) {
  const config = getRoamingGuardConfig(sourceIndex);
  const scene = sceneName || "yard";
  const start = getRoamingGuardSceneSpawn(scene, sourceIndex, existingGuards || GameState.yardGuards);
  return decorateRoamingNpc({
    x: start.x,
    y: start.y,
    targetX: start.x,
    ...
    speed: 0,
    speedTimer: 0,
    fast: sourceIndex % 2 === 0,
    investigatingSoilDump: false,
    facing: ["down", "left", "up", "right"][sourceIndex % 4],
    visualFacing: ["down", "left", "up", "right"][sourceIndex % 4],
    turnTimer: 0,
    crowdTimer: 0,
    ambientRestTarget: false,
    ambientRestTimer: 0,
    isMoving: false,
    walkAnimTime: randomRange(0, 1),
    scene,
    roamingMode: "patrol",
    ...
    roamingPathIndex: 0,
    travelElapsed: 0,
    waitTimer: randomRange(0.4, 1.4),
    retirePending: false
  }, config);
}

function rebuildRoamingGuardRoster(targetCount) {
  const count = clamp(targetCount, ROAMING_GUARD_MIN_COUNT, ROAMING_GUARD_MAX_COUNT);
  const rebuilt = [];
  for (let sourceIndex = 0; sourceIndex < count; sourceIndex += 1) {
    rebuilt.push(createRoamingYardGuard(sourceIndex, "yard", rebuilt));
  }
  GameState.yardGuards = rebuilt;
  GameState.roamingNpcSystem.pendingRetireIds = [];
  GameState.roamingNpcSystem.activeTravelerId = null;
}

function getOffscreenGuardSpawnScene() {
  return ["cellCorridor", "yard", "library", "solitary"]
    .find((sceneName) => sceneName !== GameState.scene &&
      (sceneName !== "library" || isRoamingLibraryUnlocked())) || "yard";
}

function refreshPendingRoamingGuardIds() {
  GameState.roamingNpcSystem.pendingRetireIds = GameState.yardGuards
    .filter((guard) => guard.retirePending)
    .sort((first, second) => second.sourceIndex - first.sourceIndex)
    .map((guard) => guard.roamingId);
}

function reconcileRoamingGuardRoster(options) {
  const system = GameState.roamingNpcSystem;
  const immediate = Boolean(options && options.immediate);
  const targetCount = clamp(system.guardTargetCount || ROAMING_GUARD_MIN_COUNT,
    ROAMING_GUARD_MIN_COUNT, ROAMING_GUARD_MAX_COUNT);
  if (immediate) {
    rebuildRoamingGuardRoster(targetCount);
    return;
  }

  GameState.yardGuards.forEach((guard) => {
    if (guard.sourceIndex < targetCount && guard.retirePending) {
      guard.retirePending = false;
      if (guard.roamingMode === "travel") {
        cancelRoamingTransition(guard);
      }
    }
  });

  while (GameState.yardGuards.length < targetCount) {
    const used = new Set(GameState.yardGuards.map((guard) => guard.sourceIndex));
    let sourceIndex = 0;
    while (used.has(sourceIndex) && sourceIndex < ROAMING_GUARD_MAX_COUNT) {
      sourceIndex += 1;
    }
    const spawnScene = getOffscreenGuardSpawnScene();
    GameState.yardGuards.push(createRoamingYardGuard(sourceIndex, spawnScene, GameState.yardGuards));
    GameState.yardGuards.sort((first, second) => first.sourceIndex - second.sourceIndex);
  }

  GameState.yardGuards
    .filter((guard) => guard.sourceIndex >= targetCount)
    .sort((first, second) => second.sourceIndex - first.sourceIndex)
    .forEach((guard) => {
      if (guard.scene !== GameState.scene) {
        if (system.activeTravelerId === guard.roamingId) {
          system.activeTravelerId = null;
        }
        GameState.yardGuards = GameState.yardGuards.filter((candidate) => candidate !== guard);
      } else {
        guard.retirePending = true;
      }
    });
  refreshPendingRoamingGuardIds();
}

function setRoamingGuardCheckpoint(checkpointKey, options) {
  const system = GameState.roamingNpcSystem;
  const force = Boolean(options && options.force);
  if (!force && system.guardRollCheckpoint === checkpointKey &&
    system.guardTargetCount >= ROAMING_GUARD_MIN_COUNT) {
    return system.guardTargetCount;
  }
  system.guardTargetCount = rollRoamingGuardCount();
  system.guardRollCheckpoint = checkpointKey;
  system.guardRollReason = options && options.reason ? options.reason : (force ? "reload" : "checkpoint");
  reconcileRoamingGuardRoster({ immediate: Boolean(options && options.immediate) });
  return system.guardTargetCount;
}

function initializeRoamingNpcSystem() {
  initializeYardPrisoners();
  if (GameState.roamingBackgroundPrisoners.length !== 3) {
    GameState.roamingBackgroundPrisoners = RoamingNpcConfigs
      .filter((config) => config.source === "background")
      .map(createRoamingBackgroundPrisoner);
  }

  const system = GameState.roamingNpcSystem;
  if (system.guardTargetCount < ROAMING_GUARD_MIN_COUNT ||
    system.guardRollCheckpoint !== GameState.currentCheckpoint ||
    GameState.yardGuards.length < ROAMING_GUARD_MIN_COUNT) {
    setRoamingGuardCheckpoint(GameState.currentCheckpoint, {
      force: true,
      immediate: true,
      reason: "initialize"
    });
  ...
  RoamingNpcConfigs.forEach((config) => {
    let agent = null;
    if (config.source === "named") {
      agent = GameState.yardPrisoners[config.sourceIndex];
    } else if (config.source === "background") {
      agent = GameState.roamingBackgroundPrisoners[config.sourceIndex];
    } else if (config.source === "guard") {
      agent = GameState.yardGuards[config.sourceIndex];
    }
    decorateRoamingNpc(agent, config);
  });

  if (!system.initialized) {
    system.initialized = true;
    system.travelCooldown = randomRange(ROAMING_NPC_TRAVEL_MIN_SECONDS, ROAMING_NPC_TRAVEL_MAX_SECONDS);
    system.activeTravelerId = null;
    system.portalLocks = Object.create(null);
  }
}

function resetRoamingNpcSystem() {
  GameState.yardPrisoners = [];
  GameState.roamingBackgroundPrisoners = [];
  GameState.yardGuards = [];
  const brooksConfig = getBrooksNpcConfig();
  if (brooksConfig) {
    Object.assign(GameState.brooksNpc, {
      initialized: false,
      mode: "patrol",
      x: brooksConfig.x,
      y: brooksConfig.y,
      facing: brooksConfig.baseFacing || "up",
      isMoving: false,
      walkAnimTime: 0,
      targetX: brooksConfig.x,
      targetY: brooksConfig.y,
      waitTimer: 0,
      entryPauseTimer: 0,
      ambientRestTarget: false,
      pendingDialogue: null
  ...
  GameState.roamingNpcSystem.initialized = false;
  GameState.roamingNpcSystem.travelCooldown = 0;
  GameState.roamingNpcSystem.activeTravelerId = null;
  GameState.roamingNpcSystem.portalLocks = Object.create(null);
  GameState.roamingNpcSystem.guardTargetCount = 0;
  GameState.roamingNpcSystem.guardRollCheckpoint = null;
  GameState.roamingNpcSystem.guardRollReason = null;
  GameState.roamingNpcSystem.pendingRetireIds = [];
}

function getRoamingNpcAgents() {
  initializeRoamingNpcSystem();
  return GameState.yardPrisoners
    .concat(GameState.roamingBackgroundPrisoners, GameState.yardGuards)
    .filter((agent) => agent && agent.roamingId);
}

function getRoamingNpcById(npcId) {
  return getRoamingNpcAgents().find((agent) => agent.roamingId === npcId) || null;
}

function getRoamingNpcsInScene(sceneName) {
  return getRoamingNpcAgents().filter((agent) => agent.scene === sceneName);
}

function isRoamingNpcSystemEnabled() {
  return !["CP_START", "CP_AFTER_WHITE_LIGHT"].includes(GameState.currentCheckpoint) &&
    !isOpeningYardArrival();
}

function getRoamingSceneNavigation(sceneName) {
  if (sceneName === "yard") {
    return {
      bounds: getYardWalkBounds(),
      isWalkable(point) { return isYardNpcPointWalkable(point.x, point.y); }
    };
  }
  if (sceneName === "library") {
    return {
      bounds: { minX: 166, minY: 276, maxX: 1088, maxY: 1086 },
      isWalkable(point) {
        return pointInPolygon(point, LibraryLayout.walkPolygon) && !isPointInAnyRect(point, LibraryLayout.obstacleRects);
      }
    };
  }
  if (sceneName === "cellCorridor") {
    return {
      bounds: { minX: 145, minY: 170, maxX: 675, maxY: 1210 },
      isWalkable(point) { return isCorridorFootPointWalkable(point); }
    };
  }
  if (sceneName === "cell") {
    return {
      bounds: { minX: 184, minY: 402, maxX: 1085, maxY: 1106 },
      isWalkable(point) { return isCellNpcPointWalkable(point.x, point.y); }
    };
  }
  if (sceneName === "solitary") {
    return {
      bounds: { minX: 350, minY: 238, maxX: 930, maxY: 640 },
      isWalkable(point) { return pointInPolygon(point, SolitaryLayout.walkPolygon); }
    };
  }
  return null;
}

function isRoamingSegmentClear(sceneName, from, target) {
  const navigation = getRoamingSceneNavigation(sceneName);
  if (!navigation || !navigation.isWalkable(from) || !navigation.isWalkable(target)) {
    return false;
  }
  const length = distance(from.x, from.y, target.x, target.y);
  const steps = Math.max(1, Math.ceil(length / ROAMING_NPC_ROUTE_SAMPLE_DISTANCE));
  for (let index = 1; index < steps; index += 1) {
    const ratio = index / steps;
    if (!navigation.isWalkable({
...
function getRoamingNavigationGrid(sceneName) {
  if (RoamingNavigationGridCache[sceneName]) {
    return RoamingNavigationGridCache[sceneName];
  }
  const navigation = getRoamingSceneNavigation(sceneName);
  if (!navigation) {
    return null;
  }
  const step = ROAMING_NPC_NAV_GRID_SIZE;
  const nodes = [];
  const lookup = Object.create(null);
  let row = 0;
  for (let y = navigation.bounds.minY + step / 2; y <= navigation.bounds.maxY; y += step, row += 1) {
    let col = 0;
    for (let x = navigation.bounds.minX + step / 2; x <= navigation.bounds.maxX; x += step, col += 1) {
      const point = { x, y, col, row, key: col + ":" + row };
      if (navigation.isWalkable(point)) {
        nodes.push(point);
        lookup[point.key] = point;
      }
    }
  }
  const grid = { step, nodes, lookup };
  RoamingNavigationGridCache[sceneName] = grid;
  return grid;
}

function findNearestRoamingGridNode(sceneName, point, grid) {
  return grid.nodes
    .filter((node) => isRoamingSegmentClear(sceneName, point, node))
    .sort((first, second) =>
      distance(point.x, point.y, first.x, first.y) - distance(point.x, point.y, second.x, second.y)
    )[0] || null;
}

function getRoamingGridNeighbors(sceneName, grid, node) {
  const neighbors = [];
  for (let rowOffset = -1; rowOffset <= 1; rowOffset += 1) {
    for (let colOffset = -1; colOffset <= 1; colOffset += 1) {
      ...
      const neighbor = grid.lookup[(node.col + colOffset) + ":" + (node.row + rowOffset)];
      if (!neighbor) {
        continue;
      }
      if (rowOffset !== 0 && colOffset !== 0) {
        const horizontal = grid.lookup[(node.col + colOffset) + ":" + node.row];
        const vertical = grid.lookup[node.col + ":" + (node.row + rowOffset)];
        if (!horizontal || !vertical) {
          continue;
        }
      }
      if (!isRoamingSegmentClear(sceneName, node, neighbor)) {
        continue;
      }
...
function reconstructRoamingGridPath(cameFrom, current, lookup) {
  const path = [lookup[current]];
  while (cameFrom[current]) {
    current = cameFrom[current];
    path.unshift(lookup[current]);
  }
  return path.map((node) => ({ x: node.x, y: node.y }));
}

function compressRoamingPath(sceneName, from, path) {
  const compressed = [];
  let anchor = from;
  let index = 0;
  while (index < path.length) {
    ...
    for (let candidate = index; candidate < path.length; candidate += 1) {
      if (!isRoamingSegmentClear(sceneName, anchor, path[candidate])) {
        break;
      }
      furthest = candidate;
    }
    compressed.push(path[furthest]);
...
function createRoamingPath(sceneName, from, target) {
  if (isRoamingSegmentClear(sceneName, from, target)) {
    return [{ x: target.x, y: target.y }];
  }
  const grid = getRoamingNavigationGrid(sceneName);
  if (!grid) {
    return null;
  }
  const start = findNearestRoamingGridNode(sceneName, from, grid);
  const goal = findNearestRoamingGridNode(sceneName, target, grid);
  if (!start || !goal) {
    return null;
  }
  const open = [start.key];
  const openLookup = new Set(open);
  const cameFrom = Object.create(null);
  const gScore = Object.create(null);
  const fScore = Object.create(null);
  gScore[start.key] = 0;
  fScore[start.key] = distance(start.x, start.y, goal.x, goal.y);

  while (open.length > 0) {
    open.sort((first, second) => (fScore[first] || Number.POSITIVE_INFINITY) - (fScore[second] || Number.POSITIVE_INFINITY));
    const currentKey = open.shift();
    openLookup.delete(currentKey);
    if (currentKey === goal.key) {
      const gridPath = reconstructRoamingGridPath(cameFrom, currentKey, grid.lookup);
      gridPath.push({ x: target.x, y: target.y });
      return compressRoamingPath(sceneName, from, gridPath);
    }
    const current = grid.lookup[currentKey];
    getRoamingGridNeighbors(sceneName, grid, current).forEach((neighbor) => {
      const tentative = gScore[currentKey] + distance(current.x, current.y, neighbor.x, neighbor.y);
      const knownScore = Number.isFinite(gScore[neighbor.key]) ? gScore[neighbor.key] : Number.POSITIVE_INFINITY;
      if (tentative >= knownScore) {
        return;
      }
      cameFrom[neighbor.key] = currentKey;
      gScore[neighbor.key] = tentative;
      fScore[neighbor.key] = tentative + distance(neighbor.x, neighbor.y, goal.x, goal.y);
      if (!openLookup.has(neighbor.key)) {
        open.push(neighbor.key);
        openLookup.add(neighbor.key);
      }
    });
  }
  return null;
...
function getRandomRoamingPoint(agent) {
  const navigation = getRoamingSceneNavigation(agent.scene);
  if (!navigation) {
    return { x: agent.x, y: agent.y };
  }
  let bounds = navigation.bounds;
  if (agent.scene === "yard" && agent.source === "named") {
    const named = YardRandomPrisonerConfigs[agent.sourceIndex];
    if (named && named.patrolBounds) {
      bounds = named.patrolBounds;
    }
  }
  const sceneAgents = getRoamingNpcsInScene(agent.scene).filter((other) => other !== agent);
  for (let attempt = 0; attempt < 60; attempt += 1) {
    const point = {
      x: randomRange(bounds.minX, bounds.maxX),
      y: randomRange(bounds.minY, bounds.maxY)
    };
    if (!navigation.isWalkable(point) || distance(agent.x, agent.y, point.x, point.y) < 70) {
      continue;
    }
    if (sceneAgents.some((other) => distance(other.x, other.y, point.x, point.y) < ROAMING_NPC_SOFT_SPACING)) {
      continue;
    }
    return point;
  }
...
function stopRoamingNpc(agent) {
  agent.isMoving = false;
  agent.walkAnimTime = 0;
}

function isRoamingNpcDialogueTarget(agent) {
  if (GameState.sideTalk.activeNpcId === agent.roamingId) {
    return true;
  }
  if (agent.role === "guard" && GameState.guardConversation.activeGuardIndex === agent.sourceIndex &&
    Boolean(GameState.guardConversation.activeSource)) {
    return true;
  }
  const ambientKey = agent.role === "guard" ? "guard:" + agent.sourceIndex : agent.roamingId;
  return isAmbientConversationParticipant(ambientKey);
}

function isRoamingNpcCurrentSceneModalBlocked(agent) {
  if (agent.scene !== GameState.scene) {
    return false;
  }
  return GameState.libraryTask.sortingActive ||
    GameState.hammerHidePuzzle.active ||
    GameState.mapRevealActive ||
    GameState.pipeMazeActive;
}

function moveRoamingNpcAlongPath(agent, dt) {
  const target = agent.roamingPath[agent.roamingPathIndex];
  if (!target) {
    return distance(agent.x, agent.y, agent.targetX, agent.targetY) <= 4;
  }
  ...
  if (remaining <= 4) {
    agent.x = target.x;
    agent.y = target.y;
    agent.roamingPathIndex += 1;
    return agent.roamingPathIndex >= agent.roamingPath.length;
  }
  let step = Math.min(remaining, Math.max(38, agent.speed) * dt);
  const navigation = getRoamingSceneNavigation(agent.scene);
  let nextX = agent.x + dx / remaining * step;
  let nextY = agent.y + dy / remaining * step;
  while (navigation && step > 0.05 && !navigation.isWalkable({ x: nextX, y: nextY })) {
    step /= 2;
    nextX = agent.x + dx / remaining * step;
    nextY = agent.y + dy / remaining * step;
  }
  if (navigation && !navigation.isWalkable({ x: nextX, y: nextY })) {
    if (agent.roamingMode === "travel") {
      const replanned = createRoamingPath(
        agent.scene,
        { x: agent.x, y: agent.y },
        { x: agent.targetX, y: agent.targetY }
      );
      agent.roamingPath = replanned || [];
    } else {
      agent.roamingPath = [];
      agent.targetX = agent.x;
      agent.targetY = agent.y;
      agent.waitTimer = ROAMING_NPC_TRAVEL_RETRY_SECONDS;
    }
    agent.roamingPathIndex = 0;
    stopRoamingNpc(agent);
    return false;
  }
  agent.x = nextX;
  agent.y = nextY;
  agent.facing = directionFromDelta(dx, dy);
  agent.visualFacing = agent.facing;
  agent.isMoving = true;
  agent.walkAnimTime += dt;
  return false;
}

function chooseRoamingLocalTarget(agent) {
  const target = getRandomRoamingPoint(agent);
  const path = createRoamingPath(agent.scene, { x: agent.x, y: agent.y }, target);
  agent.roamingPath = path || [];
  agent.roamingPathIndex = 0;
  agent.targetX = target.x;
  agent.targetY = target.y;
  if (!path || path.length === 0) {
    agent.waitTimer = randomRange(0.8, 1.8);
    stopRoamingNpc(agent);
  }
}

function updateGenericRoamingNpc(agent, dt) {
  if (isRoamingNpcDialogueTarget(agent) || isRoamingNpcCurrentSceneModalBlocked(agent)) {
    stopRoamingNpc(agent);
    return;
  }
  if (agent.waitTimer > 0) {
    agent.waitTimer = Math.max(0, agent.waitTimer - dt);
    stopRoamingNpc(agent);
    return;
  }
  if (!agent.roamingPath || agent.roamingPathIndex >= agent.roamingPath.length) {
    chooseRoamingLocalTarget(agent);
    return;
  }
  if (moveRoamingNpcAlongPath(agent, dt)) {
    agent.roamingPath = [];
    agent.roamingPathIndex = 0;
    agent.waitTimer = randomRange(0.5, agent.role === "guard" ? 1.4 : 1.9);
    stopRoamingNpc(agent);
  }
}

function getRoamingPortalLink(fromScene, toScene) {
  return RoamingPortalLinks[fromScene + ":" + toScene] || null;
}

function getRoamingPortalLockKey(firstScene, secondScene) {
  return [firstScene, secondScene].sort().join(":");
}

function getRoamingPortalLandingPoint(agent, destinationScene, preferredPoint) {
  const navigation = getRoamingSceneNavigation(destinationScene);
  const others = getRoamingNpcsInScene(destinationScene).filter((other) => other !== agent);
  const offsets = [
    [0, 0], [32, 0], [-32, 0], [0, 32], [0, -32],
    [32, 32], [-32, 32], [32, -32], [-32, -32],
    [64, 0], [-64, 0], [0, 64], [0, -64]
  ];
  const landing = offsets.map((offset) => ({
    x: preferredPoint.x + offset[0],
    y: preferredPoint.y + offset[1]
  })).find((point) => navigation && navigation.isWalkable(point) &&
    others.every((other) => distance(other.x, other.y, point.x, point.y) >= 32));
  return landing || { x: preferredPoint.x, y: preferredPoint.y };
}

function chooseWeightedRoamingDestination(weights) {
...
function getRoamingDestinationOptions(agent) {
  const config = getRoamingNpcConfig(agent.roamingId);
  if (!config) {
    return {};
  }
  if (agent.scene === "yard") {
    const weights = Object.assign({}, config.yardWeights);
    if (!isRoamingLibraryUnlocked()) {
      delete weights.library;
    }
    return weights;
  }
  if (agent.scene === "cellCorridor") {
    const weights = Object.assign({}, config.corridorWeights);
    // 安迪的牢房不是漫游拓扑的一部分；查房演员由独立剧情系统控制。
    delete weights.cell;
    return weights;
  }
  if (agent.scene === "library" || agent.scene === "cell" || agent.scene === "solitary") {
    return { [agent.scene === "library" ? "yard" : "cellCorridor"]: 100 };
  }
  return {};
}

function isRoamingLibraryUnlocked() {
  return Boolean(
    GameState.hasHammer ||
    GameState.hammerHiddenInBible ||
    GameState.hammerConfiscated ||
...
function isRoamingTransitionAllowed(agent, destinationScene) {
  const config = getRoamingNpcConfig(agent.roamingId);
  return Boolean(destinationScene !== "cell" &&
    config && config.allowedScenes.includes(destinationScene) &&
    (destinationScene !== "library" || isRoamingLibraryUnlocked()) &&
    getRoamingPortalLink(agent.scene, destinationScene));
}

function beginRoamingTransition(agent, destinationScene, ignorePortalLock) {
  if (!agent || !isRoamingTransitionAllowed(agent, destinationScene)) {
    return false;
  }
  const portalLockKey = getRoamingPortalLockKey(agent.scene, destinationScene);
  if (!ignorePortalLock && GameState.roamingNpcSystem.portalLocks[portalLockKey] > 0) {
    return false;
  }
  const link = getRoamingPortalLink(agent.scene, destinationScene);
  const path = createRoamingPath(agent.scene, { x: agent.x, y: agent.y }, link.from);
  if (!path || path.length === 0) {
    agent.waitTimer = ROAMING_NPC_TRAVEL_RETRY_SECONDS;
    return false;
  }
  agent.roamingMode = "travel";
  agent.destinationScene = destinationScene;
  agent.roamingPath = path;
  agent.roamingPathIndex = 0;
  agent.targetX = link.from.x;
  agent.targetY = link.from.y;
  agent.travelElapsed = 0;
  agent.waitTimer = 0;
  GameState.roamingNpcSystem.activeTravelerId = agent.roamingId;
  return true;
}

function finishRoamingTransition(agent) {
  const fromScene = agent.scene;
  const destinationScene = agent.destinationScene;
  const link = getRoamingPortalLink(fromScene, destinationScene);
  if (!link) {
    cancelRoamingTransition(agent);
    return;
  }
  const lockKey = getRoamingPortalLockKey(fromScene, destinationScene);
  GameState.roamingNpcSystem.portalLocks[lockKey] = ROAMING_NPC_PORTAL_LOCK_SECONDS;
  const landing = getRoamingPortalLandingPoint(agent, destinationScene, link.spawn);
  agent.scene = destinationScene;
  agent.x = landing.x;
  agent.y = landing.y;
  agent.targetX = agent.x;
  agent.targetY = agent.y;
  agent.roamingMode = "patrol";
  agent.destinationScene = null;
  agent.roamingPath = [];
  agent.roamingPathIndex = 0;
  agent.travelElapsed = 0;
  agent.waitTimer = randomRange(0.7, 1.8);
  stopRoamingNpc(agent);
  GameState.roamingNpcSystem.activeTravelerId = null;
  GameState.roamingNpcSystem.travelCooldown = randomRange(
    ROAMING_NPC_TRAVEL_MIN_SECONDS,
    ROAMING_NPC_TRAVEL_MAX_SECONDS
  );
}

function cancelRoamingTransition(agent) {
  agent.roamingMode = "patrol";
  agent.destinationScene = null;
  agent.roamingPath = [];
  agent.roamingPathIndex = 0;
  ...
  stopRoamingNpc(agent);
  GameState.roamingNpcSystem.activeTravelerId = null;
  GameState.roamingNpcSystem.travelCooldown = ROAMING_NPC_TRAVEL_RETRY_SECONDS;
}

function updateRoamingTransition(agent, dt) {
  if (isRoamingNpcDialogueTarget(agent) || isRoamingNpcCurrentSceneModalBlocked(agent)) {
    stopRoamingNpc(agent);
    return;
  }
  ...
  const completed = moveRoamingNpcAlongPath(agent, dt);
  if (distance(beforeX, beforeY, agent.x, agent.y) > 0.1 || completed) {
    agent.travelElapsed = 0;
  } else {
    agent.travelElapsed += dt;
  }
  if (completed) {
    finishRoamingTransition(agent);
  } else if (agent.travelElapsed >= ROAMING_NPC_TRAVEL_TIMEOUT_SECONDS) {
    cancelRoamingTransition(agent);
  }
}

function scheduleRoamingTransition(candidateAgents) {
  const system = GameState.roamingNpcSystem;
  if (system.activeTravelerId || system.travelCooldown > 0) {
    return;
  }
  const successes = (candidateAgents || getRoamingNpcAgents()).filter((agent) => {
    const config = getRoamingNpcConfig(agent.roamingId);
    if (!config || agent.roamingMode !== "patrol" || agent.retirePending || isRoamingNpcDialogueTarget(agent)) {
      return false;
    }
    const options = getRoamingDestinationOptions(agent);
    return Object.keys(options).length > 0 && Math.random() <= config.travelChance;
  });
  if (successes.length === 0) {
    system.travelCooldown = randomRange(
      ROAMING_NPC_TRAVEL_MIN_SECONDS,
      ROAMING_NPC_TRAVEL_MAX_SECONDS
    );
    return;
  }
  const agent = successes[Math.floor(Math.random() * successes.length)];
  const destination = chooseWeightedRoamingDestination(getRoamingDestinationOptions(agent));
  if (!destination || !beginRoamingTransition(agent, destination)) {
    system.travelCooldown = ROAMING_NPC_TRAVEL_RETRY_SECONDS;
  }
}

function rehomeRoamingNpc(agent, sceneName, point) {
  ...
  agent.roamingPathIndex = 0;
  agent.travelElapsed = 0;
  agent.waitTimer = randomRange(0.7, 1.6);
  stopRoamingNpc(agent);
  if (GameState.roamingNpcSystem.activeTravelerId === agent.roamingId) {
    GameState.roamingNpcSystem.activeTravelerId = null;
  }
}

function getRetiringGuardDestination(guard) {
  return Object.keys(RoamingPortalLinks)
    .filter((key) => key.startsWith(guard.scene + ":"))
    .map((key) => ({
      destination: key.slice(key.indexOf(":") + 1),
      portal: RoamingPortalLinks[key]
    }))
    .filter((entry) => isRoamingTransitionAllowed(guard, entry.destination))
    .sort((first, second) =>
      distance(guard.x, guard.y, first.portal.from.x, first.portal.from.y) -
      distance(guard.x, guard.y, second.portal.from.x, second.portal.from.y)
    )[0]?.destination || null;
}

function updateRoamingGuardRoster() {
  reconcileRoamingGuardRoster();
  const system = GameState.roamingNpcSystem;
  const pending = GameState.yardGuards
    .filter((guard) => guard.retirePending)
    .sort((first, second) => second.sourceIndex - first.sourceIndex);
  ...
  const visibleRetiring = GameState.yardGuards
    .filter((guard) => guard.retirePending && guard.scene === GameState.scene)
    .sort((first, second) => second.sourceIndex - first.sourceIndex)[0];
  if (visibleRetiring && visibleRetiring.roamingMode !== "travel" && !system.activeTravelerId) {
    const destination = getRetiringGuardDestination(visibleRetiring);
    if (destination) {
      visibleRetiring.waitTimer = 0;
      beginRoamingTransition(visibleRetiring, destination, true);
    }
  }
  refreshPendingRoamingGuardIds();
}

function isSideRouteMorningGuardStage() {
  return GameState.sideRoute.active &&
    (GameState.sideRoute.stage === "morningPatrol" || GameState.sideRoute.stage === "morningGateCaught");
}

function enforceRoamingNarrativeOwnership() {
  const agents = getRoamingNpcAgents();
  // 安迪的牢房始终是私人区域。典狱长与狱警查房使用 cellInspection 剧情演员，
  // 任何持久漫游 NPC 即使因旧状态或调试操作落入牢房，也会立即撤回走廊。
  agents.filter((agent) => agent.scene === "cell").forEach((agent, index) => {
    rehomeRoamingNpc(agent, "cellCorridor", { x: 600 - index * 26, y: 755 + index * 28 });
  });
  if (GameState.sideRoute.active && !isSideRouteMorningGuardStage()) {
    agents.filter((agent) => agent.scene !== "yard").forEach((agent, index) => {
      rehomeRoamingNpc(agent, "yard", { x: 1040 - (index % 3) * 42, y: 1010 + Math.floor(index / 3) * 38 });
    });
  }
  if (!isRoamingLibraryUnlocked()) {
    agents.filter((agent) => agent.scene === "library").forEach((agent, index) => {
      rehomeRoamingNpc(agent, "yard", { x: 165 + index * 24, y: 1132 });
    });
  }
  if (isCorridorEscortActive()) {
    agents.filter((agent) => agent.scene === "cellCorridor").forEach((agent, index) => {
      rehomeRoamingNpc(agent, "yard", { x: 1080 - index * 28, y: 965 + index * 22 });
    });
  }
  const soilNarrativeActive = !isSideRouteMorningGuardStage() &&
    (GameState.hasSoilPile || GameState.soilDump.active);
  const soilAnchors = [
    { x: 690, y: 470 },
    { x: 830, y: 650 },
    { x: 900, y: 820 }
  ...
  getNarrativeYardGuards().forEach((guard, index) => {
    if (soilNarrativeActive) {
      if (guard.scene !== "yard" || guard.roamingMode !== "soilPatrol") {
        rehomeRoamingNpc(guard, "yard", soilAnchors[index]);
        guard.roamingMode = "soilPatrol";
        guard.targetX = 0;
        guard.targetY = 0;
      }
    } else if (guard.roamingMode === "soilPatrol") {
      guard.roamingMode = "patrol";
      guard.targetX = 0;
      guard.targetY = 0;
      guard.waitTimer = randomRange(0.7, 1.6);
    } else if (!isSideRouteMorningGuardStage() && guard.roamingMode === "gatePatrol") {
      guard.roamingMode = "patrol";
      guard.targetX = 0;
      guard.targetY = 0;
      delete guard.captureTarget;
  ...
  const soilInteraction = getYardInteraction("soil");
  const gateBounds = getSideRouteMorningGateBounds(ROAMING_GUARD_GATE_BUFFER);
  GameState.yardGuards.filter((guard) => !isNarrativeYardGuard(guard) && guard.scene === "yard")
    .forEach((guard) => {
      const target = { x: guard.targetX || guard.x, y: guard.targetY || guard.y };
      const currentInsideSoil = soilNarrativeActive &&
        isPointInExpandedYardInteraction(guard, soilInteraction, ROAMING_GUARD_SOIL_BUFFER);
      const currentInsideGate = isSideRouteMorningGuardStage() && pointInRect(guard, gateBounds);
      if (currentInsideSoil || currentInsideGate) {
        const evacuationTarget = getYardGuardReservedAreaExit(guard, currentInsideSoil ?
          {
            x: soilInteraction.rect.x - ROAMING_GUARD_SOIL_BUFFER,
            y: soilInteraction.rect.y - ROAMING_GUARD_SOIL_BUFFER,
            w: soilInteraction.rect.w + ROAMING_GUARD_SOIL_BUFFER * 2,
            h: soilInteraction.rect.h + ROAMING_GUARD_SOIL_BUFFER * 2
          } : gateBounds);
        guard.targetX = evacuationTarget.x;
        guard.targetY = evacuationTarget.y;
        guard.ambientRestTimer = 0;
        guard.ambientRestTarget = false;
        return;
      }
      const targetInsideReservedArea = (soilNarrativeActive &&
        isPointInExpandedYardInteraction(target, soilInteraction, ROAMING_GUARD_SOIL_BUFFER)) ||
        (isSideRouteMorningGuardStage() && pointInRect(target, gateBounds));
      if (targetInsideReservedArea) {
        guard.targetX = 0;
        guard.targetY = 0;
        guard.ambientRestTimer = 0;
      }
...
function updateRoamingNpcSystem(dt) {
  if (!isRoamingNpcSystemEnabled() || GameState.scene === "pause") {
    return;
  }
  initializeRoamingNpcSystem();
  updateRoamingGuardRoster();
  enforceRoamingNarrativeOwnership();
  const morningGuardStage = isSideRouteMorningGuardStage();
  if ((GameState.sideRoute.active && !morningGuardStage) || isCorridorEscortActive() || isCellInspectionActive()) {
    return;
  }

  const system = GameState.roamingNpcSystem;
  system.travelCooldown = Math.max(0, system.travelCooldown - dt);
  Object.keys(system.portalLocks).forEach((key) => {
    system.portalLocks[key] = Math.max(0, system.portalLocks[key] - dt);
    if (system.portalLocks[key] <= 0) {
      delete system.portalLocks[key];
    }
  });

  const activeAgents = morningGuardStage ?
    GameState.yardGuards.filter((guard) => !isNarrativeYardGuard(guard)) :
    getRoamingNpcAgents();
  activeAgents.forEach((agent) => {
    if (agent.roamingMode === "travel") {
      updateRoamingTransition(agent, dt);
      return;
    }
    if (agent.scene === "yard" && agent.source === "named") {
      return;
    }
    if (agent.scene === "yard" && agent.source === "guard") {
      return;
    }
    updateGenericRoamingNpc(agent, dt);
  });
  if (!morningGuardStage) {
    updateYardPrisoners(dt);
  }
  updateYardGuards(dt);
  scheduleRoamingTransition(activeAgents);
}

function forceRoamingTransition(npcId, destinationScene) {
  initializeRoamingNpcSystem();
  const agent = getRoamingNpcById(npcId);
  if (!agent) {
    throw new Error("Unknown roaming NPC: " + npcId);
  }
  if (!isRoamingTransitionAllowed(agent, destinationScene)) {
    throw new Error("Roaming transition not allowed: " + agent.scene + " -> " + destinationScene);
  }
  const active = GameState.roamingNpcSystem.activeTravelerId;
  if (active && active !== npcId) {
    const activeAgent = getRoamingNpcById(active);
    if (activeAgent) {
      cancelRoamingTransition(activeAgent);
    }
  }
  GameState.roamingNpcSystem.travelCooldown = 0;
  if (!beginRoamingTransition(agent, destinationScene, true)) {
    throw new Error("No roaming path to portal for " + npcId);
  }
  return getRoamingNpcDebugState(agent);
}

function getRoamingNpcDebugState(agent) {
  return {
    id: agent.roamingId,
    characterId: agent.assetKey,
    role: agent.role,
    scene: agent.scene,
    x: Math.round(agent.x),
    y: Math.round(agent.y),
    targetX: Math.round(agent.targetX),
    targetY: Math.round(agent.targetY),
    mode: agent.retirePending ? "retiring" : agent.roamingMode,
    destinationScene: agent.destinationScene || null,
    allowedScenes: (getRoamingNpcConfig(agent.roamingId) || { allowedScenes: [] }).allowedScenes.slice()
  };
}

function getRoamingNpcDebugStates() {
  return getRoamingNpcAgents().map(getRoamingNpcDebugState);
}
...
function updateYardGuards(dt) {
  ...
  GameState.yardGuards.forEach((guard, index) => {
    if ((guard.scene && guard.scene !== "yard") ||
      guard.roamingMode === "travel" || guard.roamingMode === "gatePatrol" ||
      guard.retirePending) {
      return;
    }
    ...
    if (guard.roamingMode === "patrol") {
      const crowdedGuard = GameState.yardGuards.find((otherGuard) =>
        otherGuard !== guard && otherGuard.scene === "yard" && otherGuard.roamingMode === "patrol" &&
        otherGuard.sourceIndex < guard.sourceIndex &&
        distance(guard.x, guard.y, otherGuard.x, otherGuard.y) < ROAMING_GUARD_CROWD_DISTANCE
      );
      guard.crowdTimer = crowdedGuard ? (guard.crowdTimer || 0) + dt : 0;
      if (guard.crowdTimer >= ROAMING_GUARD_CROWD_SECONDS) {
        guard.crowdTimer = 0;
        const separationTarget = getYardGuardSeparationTarget(guard, crowdedGuard);
        guard.targetX = separationTarget.x;
        guard.targetY = separationTarget.y;
        guard.ambientRestTimer = 0;
        guard.ambientRestTarget = false;
        guard.speed = YARD_GUARD_FAST_SPEED;
        guard.speedTimer = randomRange(2.2, 3.4);
      }
    } else {
      guard.crowdTimer = 0;
    }
    ...
    if (!isYardNpcPointWalkable(nextX, nextY) ||
      isYardGuardEnteringReservedArea(guard, { x: nextX, y: nextY })) {
      guard.isMoving = false;
      guard.investigatingSoilDump = false;
      chooseYardGuardTarget(guard, soilInteraction);
      return;
...
function chooseYardGuardTarget(guard, soilInteraction) {
  const soilNarrativeActive = GameState.hasSoilPile || GameState.soilDump.active;
  const gateNarrativeActive = isSideRouteMorningGuardStage();
  const soilCoreGuard = soilNarrativeActive && guard.roamingMode === "soilPatrol" && isNarrativeYardGuard(guard);
  const bounds = getYardWalkBounds();
  const soilRegions = [
    { minX: 620, maxX: 755, minY: 405, maxY: 545 },
    { minX: 735, maxX: 875, minY: 565, maxY: 705 },
    { minX: 820, maxX: 950, minY: 745, maxY: 880 }
  ];
  const patrolBounds = soilCoreGuard ? soilRegions[guard.sourceIndex] : bounds;
  const gateBounds = getSideRouteMorningGateBounds(ROAMING_GUARD_GATE_BUFFER);
  const isAllowedPatrolPoint = (point) => {
    if (!isYardNpcPointWalkable(point.x, point.y)) {
      return false;
    }
    if (soilCoreGuard) {
      return isPointInYardInteraction(point, soilInteraction);
    }
    if (soilNarrativeActive && !isNarrativeYardGuard(guard) &&
      isPointInExpandedYardInteraction(point, soilInteraction, ROAMING_GUARD_SOIL_BUFFER)) {
      return false;
    }
    if (gateNarrativeActive && !isNarrativeYardGuard(guard) && pointInRect(point, gateBounds)) {
      return false;
    }
    return true;
  };

  const socialTarget = guard.roamingMode === "patrol" ? getAmbientSocialTarget("guard") : null;
  if (socialTarget && isAllowedPatrolPoint(socialTarget)) {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    nextX = randomRange(patrolBounds.minX, patrolBounds.maxX);
    nextY = randomRange(patrolBounds.minY, patrolBounds.maxY);

    const farEnough = distance(guard.x, guard.y, nextX, nextY) >= (soilCoreGuard ? 55 : minTargetDistance);
    const notCrowded = GameState.yardGuards.every((otherGuard) => {
      if (otherGuard === guard || otherGuard.scene !== "yard") {
        return true;
      }
      const otherX = otherGuard.targetX || otherGuard.x;
      const otherY = otherGuard.targetY || otherGuard.y;
      return distance(otherX, otherY, nextX, nextY) >= ROAMING_GUARD_TARGET_SPACING;
    });

    if (farEnough && notCrowded && isAllowedPatrolPoint({ x: nextX, y: nextY })) {
      guard.targetX = nextX;
      guard.targetY = nextY;
      return;
    }
...
function isPointInExpandedYardInteraction(point, interaction, buffer) {
  if (!interaction) {
    return false;
  }
  const expanded = {
    x: interaction.rect.x - buffer,
    y: interaction.rect.y - buffer,
    w: interaction.rect.w + buffer * 2,
    h: interaction.rect.h + buffer * 2
  };
  return interaction.shape === "ellipse" ? pointInEllipse(point, expanded) : pointInRect(point, expanded);
}

function getSideRouteMorningGateBounds(buffer) {
  const points = SideRouteMorningLayout.guardPaths.flat();
  const minX = Math.min.apply(null, points.map((point) => point.x));
  const maxX = Math.max.apply(null, points.map((point) => point.x));
  const minY = Math.min.apply(null, points.map((point) => point.y));
  const maxY = Math.max.apply(null, points.map((point) => point.y));
  return {
    x: minX - buffer,
    y: minY - buffer,
...
function getYardGuardReservedAreaExit(guard, reservedRect) {
  const margin = 14;
  const candidates = [
    { x: reservedRect.x - margin, y: guard.y },
    { x: reservedRect.x + reservedRect.w + margin, y: guard.y },
    { x: guard.x, y: reservedRect.y - margin },
    { x: guard.x, y: reservedRect.y + reservedRect.h + margin }
  ].filter((point) => isYardNpcPointWalkable(point.x, point.y));
  return candidates.sort((first, second) =>
    distance(guard.x, guard.y, first.x, first.y) - distance(guard.x, guard.y, second.x, second.y)
  )[0] || getRandomYardWalkPoint();
}

function getYardGuardSeparationTarget(guard, crowdedGuard) {
  let dx = guard.x - crowdedGuard.x;
  let dy = guard.y - crowdedGuard.y;
  if (Math.hypot(dx, dy) < 1) {
    const angle = guard.sourceIndex * 1.7;
    dx = Math.cos(angle);
    dy = Math.sin(angle);
  }
  const length = Math.hypot(dx, dy);
  ...
  const candidates = [];
  [0, Math.PI / 4, -Math.PI / 4, Math.PI / 2, -Math.PI / 2].forEach((offset) => {
    [260, 180].forEach((separation) => {
      candidates.push({
        x: guard.x + Math.cos(baseAngle + offset) * separation,
        y: guard.y + Math.sin(baseAngle + offset) * separation
      });
    });
  });
  return candidates.find((point) => isYardNpcPointWalkable(point.x, point.y) &&
    !isYardGuardEnteringReservedArea(guard, point)) || getRandomYardWalkPoint();
}

function isYardGuardEnteringReservedArea(guard, nextPoint) {
  if (isNarrativeYardGuard(guard)) {
    return false;
  }
  const soilInteraction = getYardInteraction("soil");
  if ((GameState.hasSoilPile || GameState.soilDump.active) &&
    !isPointInExpandedYardInteraction(guard, soilInteraction, ROAMING_GUARD_SOIL_BUFFER) &&
    isPointInExpandedYardInteraction(nextPoint, soilInteraction, ROAMING_GUARD_SOIL_BUFFER)) {
    return true;
  }
  const gateBounds = getSideRouteMorningGateBounds(ROAMING_GUARD_GATE_BUFFER);
  return isSideRouteMorningGuardStage() && !pointInRect(guard, gateBounds) && pointInRect(nextPoint, gateBounds);
}
...
function renderDeleteSaveConfirmation(ctx) {
  ...
  ctx.fillText("所有互动空间进度将被永久删除", CANVAS_WIDTH / 2, 318);
...
function renderWhiteLight(ctx) {
  ...
  if (GameState.opening.phase === "inhale01") {
    ...
    renderWhiteLightSubtitle(ctx, TextData.whiteLightLines[0]);
    return;
  }

  if (GameState.opening.phase === "inhale02") {
    renderIntroStill(ctx, "inhale_02");
    renderWhiteLightSubtitle(ctx, TextData.whiteLightLines[1]);
    return;
  }

  if (GameState.opening.phase === "inhale03") {
    renderIntroStill(ctx, "inhale_03");
    renderWhiteLightSubtitle(ctx, TextData.whiteLightLines[2]);
    return;
  }
...
function renderWhiteLightSubtitle(ctx, text) {
  ctx.save();
  ctx.fillStyle = "rgba(0, 0, 0, 0.62)";
  ctx.fillRect(0, 548, CANVAS_WIDTH, 142);
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  renderWrappedText(ctx, text || "", CANVAS_WIDTH / 2, 602, 1100, 38);

  if (SHOW_GAMEPLAY_TEXT_HINTS) {
    ctx.fillStyle = "rgba(255, 255, 255, 0.7)";
    ctx.shadowBlur = 0;
    ctx.font = "19px 'Microsoft YaHei', 'SimHei', monospace";
    ctx.fillText("点击画面或按空格键继续", CANVAS_WIDTH / 2, 658);
  }
  ctx.restore();
}
...
function renderOpeningPrelude(ctx, page, timer, duration) {
  ...
  if (safePage.title) {
    ...
    ctx.fillText(safePage.title, CANVAS_WIDTH / 2, 132);
  }
  ...
  renderPreludeBodyText(ctx, safePage.body || "", CANVAS_WIDTH / 2, safePage.title ? 246 : 198, 980, 48);
...
function renderCorridorInteractionHint(ctx) {
  ...
  if (interaction.id === "doorMemory") {
    const rect = corridorImageRectToScreen(CorridorLayout.doorMemoryZone);
    ctx.save();
...
function renderCorridorEntities(ctx) {
  ...
  if (isCorridorEscortActive()) {
    ...
    getRoamingNpcsInScene("cellCorridor").forEach((agent) => {
      entities.push({ type: "roaming", agent, y: agent.y * CORRIDOR_WORLD_SCALE });
    });
  }
  ...
  entities.forEach((entity) => {
    ...
    if (entity.type === "roaming") {
      renderRoamingNpc(ctx, entity.agent, corridorImagePointToScreen);
      return;
    }
...
function renderCellEntities(ctx) {
  if (isCellInspectionActive()) {
...
function renderSolitaryEntities(ctx) {
  const entities = [{ type: "player", y: getPlayerFootPoint(GameState.player.x, GameState.player.y).y }];
  getRoamingNpcsInScene("solitary").forEach((agent) => {
    entities.push({ type: "roaming", agent, y: agent.y });
  });
  ...
  entities.forEach((entity) => {
    if (entity.type === "player") {
      ...
      renderRoamingNpc(ctx, entity.agent, (x, y) => ({ x, y }));
    }
  });
}
...
function renderPipeNetwork(ctx) {
  ctx.fillStyle = "#020302";
  ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
  renderPipeNetworkBackground(ctx);
  renderPipeNetworkExit(ctx);
  renderPipeNetworkVision(ctx);
  renderPipeNetworkPlayer(ctx);
  QuestSystem.render(ctx);
}

function renderPipeNetworkBackground(ctx) {
  const record = getImageAssetRecord("pipe_network");
  if (!record || !record.loaded || record.failed) {
    drawAsset(ctx, "pipe_network", 0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    return;
  }

  const network = GameState.pipeNetwork;
  const sourceX = network.cameraX / PIPE_NETWORK_WORLD_SCALE;
  const sourceY = network.cameraY / PIPE_NETWORK_WORLD_SCALE;
  const sourceW = CANVAS_WIDTH / PIPE_NETWORK_WORLD_SCALE;
  const sourceH = CANVAS_HEIGHT / PIPE_NETWORK_WORLD_SCALE;
  ctx.drawImage(
    record.element,
    sourceX,
    sourceY,
...
function renderPipeNetworkExit(ctx) {
  const network = GameState.pipeNetwork;
  const exit = PipeNetworkLayout.graph.nodes[PipeNetworkLayout.exitNodeId];
  const x = exit.x - network.cameraX;
  const y = exit.y - network.cameraY;
  if (x < -80 || x > CANVAS_WIDTH + 80 || y < -80 || y > CANVAS_HEIGHT + 80) {
    return;
  }

  ctx.save();
  const glow = ctx.createRadialGradient(x, y, 4, x, y, 82);
  glow.addColorStop(0, "rgba(255, 226, 149, 0.72)");
  glow.addColorStop(1, "rgba(255, 226, 149, 0)");
  ctx.fillStyle = glow;
  ctx.fillRect(x - 90, y - 90, 180, 180);
  ctx.fillStyle = "rgba(255, 237, 190, 0.9)";
  ctx.font = "bold 18px 'Microsoft YaHei', 'SimHei', monospace";
  ctx.textAlign = "right";
  ctx.textBaseline = "middle";
  ctx.fillText("出口  →", Math.min(CANVAS_WIDTH - 18, x - 18), y - 64);
  ctx.restore();
}

function renderPipeNetworkVision(ctx) {
  const network = GameState.pipeNetwork;
  const x = network.centerX - network.cameraX;
  const y = network.centerY - network.cameraY;
  const light = ctx.createRadialGradient(
    x,
    y,
    PIPE_NETWORK_VISION_INNER_RADIUS,
    x,
    y,
    PIPE_NETWORK_VISION_OUTER_RADIUS
  );
  light.addColorStop(0, "rgba(0, 0, 0, 0)");
  light.addColorStop(0.58, "rgba(0, 0, 0, 0.3)");
...
function renderPipeNetworkPlayer(ctx) {
  const network = GameState.pipeNetwork;
  const x = network.centerX - network.cameraX - PIPE_NETWORK_PLAYER_W / 2;
  const y = network.centerY - network.cameraY - PIPE_NETWORK_PLAYER_H / 2;
  const player = {
    w: PIPE_NETWORK_PLAYER_W,
    h: PIPE_NETWORK_PLAYER_H,
    facing: network.facing,
    isMoving: network.isMoving,
    walkAnimTime: network.walkAnimTime
  };
  ...
  ctx.beginPath();
  ctx.ellipse(
    x + PIPE_NETWORK_PLAYER_W / 2,
    y + PIPE_NETWORK_PLAYER_H - 8,
    PIPE_NETWORK_PLAYER_W * 0.38,
    13,
    0,
    0,
    Math.PI * 2
  ...
  ctx.fill();
  ctx.restore();
  renderAndyWalkCycle(ctx, player, x, y);
}
...
function renderYard(ctx) {
  ...
  renderOpeningYardGateLight(ctx);
  renderYardLandmarks(ctx);
  const playerRenderedWithActors = renderYardBackgroundActors(ctx);
  if (!playerRenderedWithActors) {
    renderPlayer(ctx);
  }
  renderOpeningYardGateThought(ctx);
  renderYardMapThought(ctx);
...
function shouldHideOverheadThought() {
  const supportedScene = GameState.scene === "cell" ||
    GameState.scene === "yard" ||
    GameState.scene === "cellCorridor" ||
    GameState.scene === "office" ||
...
function getPlayerOverheadThoughtAnchor() {
  const player = GameState.player;
  if (GameState.scene === "dig") {
    const dig = GameState.dig;
  ...
  if (GameState.scene === "cell" && player.lyingInBed) {
    const lyingRect = getScaledCharacterDrawRect(
      player.x,
      player.y,
      BED_LYING_PLAYER_WIDTH,
      BED_LYING_PLAYER_HEIGHT
    );
    return {
      x: lyingRect.x + lyingRect.h / 2,
      y: lyingRect.y - 14
    };
  }
...
function renderPlayerOverheadThought(ctx, text, alpha) {
  const anchor = getPlayerOverheadThoughtAnchor();
  const maxWidth = 520;
  const maxLines = 2;
  const fontSize = 20;
  const lineHeight = 28;
  ...
  let lines = buildWrappedLines(ctx, text, maxWidth);
  if (lines.length > maxLines) {
    lines = lines.slice(0, maxLines);
    lines[maxLines - 1] = fitTextToWidth(ctx, lines[maxLines - 1], maxWidth, "...");
  }
  const widestLine = lines.reduce((width, line) => Math.max(width, ctx.measureText(line).width), 0);
  const inventoryVisible = shouldRenderInventory() && getFixedInventoryItems().length > 0;
  const rightSafeEdge = inventoryVisible ?
    CANVAS_WIDTH - InventoryLayout.boxW - InventoryLayout.right - 12 :
    CANVAS_WIDTH - canvasMargin;
  const x = clamp(
    anchor.x,
    canvasMargin + widestLine / 2,
    rightSafeEdge - widestLine / 2
  );
  const minimumBottomY = canvasMargin + fontSize + (lines.length - 1) * lineHeight;
  const bottomY = Math.max(minimumBottomY, anchor.y);
...
function renderYardMapThought(ctx) {
  const mapState = GameState.yardNavigationMap;
  if (GameState.scene !== "yard" || mapState.expanded || DialogueSystem.active ||
    OverheadThoughtSystem.isActive() || TutorialSystem.isActive() ||
    (!mapState.firstWalkIntroPending && !mapState.postTutorialThoughtActive)) {
    return;
  }
  const player = GameState.player;
  const screen = getPlayerScreenPosition(player.x, player.y);
  const drawRect = getScaledCharacterDrawRect(screen.x, screen.y, player.w, player.h);
  const text = mapState.firstWalkIntroPending ?
    "这里是哪？是劳作区吗...." :
    (YardMapPostTutorialThoughtLines[mapState.postTutorialThoughtIndex] || "");
  let alpha = 1;
  if (mapState.postTutorialThoughtActive) {
    const remaining = YARD_MAP_POST_TUTORIAL_THOUGHT_SECONDS - mapState.postTutorialThoughtTimer;
    alpha = Math.min(
      1,
      mapState.postTutorialThoughtTimer / YARD_MAP_POST_TUTORIAL_THOUGHT_FADE_SECONDS,
      remaining / YARD_MAP_POST_TUTORIAL_THOUGHT_FADE_SECONDS
    );
  }
  ctx.save();
  ctx.globalAlpha = clamp(alpha, 0, 1);
  ...
  const x = clamp(drawRect.x + drawRect.w / 2, halfWidth + 12, CANVAS_WIDTH - halfWidth - 12);
  const y = Math.max(28, drawRect.y - 18);
...
function renderLibrary(ctx) {
  ...
  renderLibraryEntities(ctx);
  QuestSystem.render(ctx);
  renderLibrarySortingTask(ctx);
}

function renderLibraryEntities(ctx) {
  const playerFoot = canvasPointToLibraryImage(getPlayerFootPoint(GameState.player.x, GameState.player.y));
  const entities = [{ type: "player", y: playerFoot ? playerFoot.y : 0 }];
  if (isBrooksWaitingInLibrary()) {
    entities.push({ type: "brooks", y: LibraryLayout.brooks.y });
  }
  getRoamingNpcsInScene("library").forEach((agent) => {
    entities.push({ type: "roaming", agent, y: agent.y });
  });
  entities.sort((first, second) => first.y - second.y);
  entities.forEach((entity) => {
    if (entity.type === "player") {
      renderPlayer(ctx);
    } else if (entity.type === "brooks") {
      renderLibraryBrooks(ctx);
    } else {
      renderRoamingNpc(ctx, entity.agent, libraryImageToCanvasPoint);
    }
  });
}
...
function renderLibraryInteractionHints(ctx) {
  ...
  if (activeInteraction.id === "roamingNpc") {
    return;
  }
...
function getPendingCharacterRect(imageX, imageY, h, imagePointToCanvas) {
  const foot = imagePointToCanvas(imageX, imageY);
  const characterScale = getCurrentCharacterRenderScale();
  const height = characterScale === 1 ? h : Math.max(1, Math.round(h * characterScale));
  const width = characterScale === 1 ? YARD_CHARACTER_FALLBACK_WIDTH :
    Math.max(1, Math.round(YARD_CHARACTER_FALLBACK_WIDTH * characterScale));
  return { x: foot.x - width / 2, y: foot.y - height, w: width, h: height };
}

function renderRoamingNpc(ctx, agent, imagePointToCanvas) {
  if (!agent) {
    return null;
  }
  const facing = agent.visualFacing || agent.facing || "down";
  let rect = null;
  if (DirectionalNpcSpriteSheets[agent.assetKey]) {
    rect = renderDirectionalNpcSprite(
      ctx,
      agent.assetKey,
      agent.x,
      agent.y,
      agent.h || YARD_CHARACTER_HEIGHT,
      facing,
      agent.isMoving,
      agent.walkAnimTime,
      imagePointToCanvas
    );
  } else if (agent.assetKey === "guard") {
    rect = renderDirectionalNpcSprite(
      ctx,
      "guard",
      agent.x,
    ...
    rect = renderRoamingPrisonerFrame(ctx, agent, facing, imagePointToCanvas);
  }
  if (!rect) {
    rect = renderRoamingTriptychFallback(ctx, agent, facing, imagePointToCanvas);
  }
  if (rect && agent.name) {
    renderYardNameTag(ctx, agent.name, rect.x + rect.w / 2, rect.y - 4);
  }
  return rect;
}

function renderRoamingPrisonerFrame(ctx, agent, facing, imagePointToCanvas) {
  const frameSet = PrisonerWalkFrames[agent.assetKey];
  const frames = frameSet ? frameSet[facing] || frameSet.down : null;
  if (!frames || frames.length === 0) {
    return null;
  }
  const frameDuration = agent.walkFrameSeconds || PRISONER_WALK_FRAME_SECONDS;
  const idleFrameIndex = typeof agent.idleFrameIndex === "number" ? agent.idleFrameIndex : 1;
  const frameIndex = agent.isMoving ? Math.floor(agent.walkAnimTime / frameDuration) % frames.length : idleFrameIndex;
  const frame = getRenderableAnimationFrame(frames, frameIndex);
  const record = getImageAssetRecord(frame.assetKey);
  if (!record || record.failed) {
    return null;
  }
  if (!record.loaded) {
    return getPendingCharacterRect(agent.x, agent.y, agent.h || YARD_CHARACTER_HEIGHT, imagePointToCanvas);
  }
  const sourceW = Math.max(1, getSourceWidth(record.element));
  const sourceH = Math.max(1, getSourceHeight(record.element));
  const foot = imagePointToCanvas(agent.x, agent.y);
  const characterScale = getCurrentCharacterRenderScale();
  const height = Math.max(1, Math.round((agent.h || YARD_CHARACTER_HEIGHT) * characterScale));
  const width = Math.max(1, Math.round(height * sourceW / sourceH));
  const x = foot.x - width / 2;
  const y = foot.y - height;
  ctx.save();
  if (frame.flip) {
    ctx.translate(x + width, y);
    ctx.scale(-1, 1);
    ctx.drawImage(record.element, 0, 0, sourceW, sourceH, 0, 0, width, height);
  } else {
    ctx.drawImage(record.element, 0, 0, sourceW, sourceH, x, y, width, height);
  }
  ctx.restore();
  return { x, y, w: width, h: height };
}

function renderRoamingTriptychFallback(ctx, agent, facing, imagePointToCanvas) {
  const size = getTriptychCharacterSize(agent.assetKey, agent.h || YARD_CHARACTER_HEIGHT, facing);
  const foot = imagePointToCanvas(agent.x, agent.y);
  const rect = { x: foot.x - size.w / 2, y: foot.y - size.h, w: size.w, h: size.h };
  renderTriptychCharacter(ctx, agent.assetKey, rect.x, rect.y, rect.w, rect.h, facing);
  return rect;
}
...
function renderOpeningYardGateLight(ctx) {
  const opening = GameState.opening;
  if (!isOpeningYardArrival() || !opening.gateApproachActive) {
    return;
  }

  const light = OpeningYardLayout.gateLightSeam;
  const top = yardImagePointToScreen(light.x, light.topY);
  const threshold = yardImagePointToScreen(light.x, light.thresholdY);
  const fade = clamp(opening.gateApproachTimer / OpeningYardLayout.gateLightFadeSeconds, 0, 1);
  const pulse = 0.94 + Math.sin(opening.gateApproachTimer * 2.6) * 0.06;
  const alpha = fade * pulse;

  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.filter = "blur(7px)";
  const seamGradient = ctx.createLinearGradient(top.x, top.y, threshold.x, threshold.y);
  seamGradient.addColorStop(0, "rgba(255, 239, 175, 0)");
  seamGradient.addColorStop(0.55, "rgba(255, 231, 151, " + (0.12 * alpha) + ")");
  seamGradient.addColorStop(1, "rgba(255, 218, 120, " + (0.34 * alpha) + ")");
  ctx.strokeStyle = seamGradient;
  ctx.lineWidth = Math.max(5, 3.4 * YARD_WORLD_SCALE);
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(top.x, top.y);
  ctx.bezierCurveTo(top.x - 1.5, top.y + (threshold.y - top.y) * 0.32,
    threshold.x + 1.2, top.y + (threshold.y - top.y) * 0.7, threshold.x, threshold.y);
  ctx.stroke();
  ctx.filter = "none";

  OpeningYardLayout.gateLightPools.forEach((pool, index) => {
    const center = yardImagePointToScreen(pool.x, pool.y);
    const shimmer = 1 + Math.sin(opening.gateApproachTimer * 1.8 + index * 1.7) * 0.035;
    renderOpeningYardSoftGlow(
      ctx,
      center.x,
      center.y,
      pool.radiusX * YARD_WORLD_SCALE * shimmer,
      pool.radiusY * YARD_WORLD_SCALE * shimmer,
      pool.alpha * alpha
    );
  });
  ctx.restore();
...
function renderOpeningYardSoftGlow(ctx, x, y, radiusX, radiusY, alpha) {
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(radiusX, radiusY);
  const glow = ctx.createRadialGradient(0, -0.12, 0, 0, 0, 1);
  glow.addColorStop(0, "rgba(255, 230, 154, " + alpha + ")");
  glow.addColorStop(0.38, "rgba(255, 220, 132, " + (alpha * 0.55) + ")");
  glow.addColorStop(0.76, "rgba(246, 205, 108, " + (alpha * 0.14) + ")");
  glow.addColorStop(1, "rgba(238, 195, 99, 0)");
  ctx.fillStyle = glow;
  ctx.beginPath();
...
function renderOpeningYardGateThought(ctx) {
  const opening = GameState.opening;
  const elapsed = opening.gateApproachTimer - OpeningYardLayout.gateThoughtDelaySeconds;
  if (!isOpeningYardArrival() || !opening.gateApproachActive || elapsed <= 0 ||
    DialogueSystem.active || OverheadThoughtSystem.isActive() || TutorialSystem.isActive()) {
    return;
  }
  ...
  const screen = getPlayerScreenPosition(player.x, player.y);
  const drawRect = getScaledCharacterDrawRect(screen.x, screen.y, player.w, player.h);
  const alpha = clamp(elapsed / OpeningYardLayout.gateThoughtFadeSeconds, 0, 1);
  ...
  ctx.textBaseline = "bottom";
  ctx.lineWidth = 4;
  ctx.strokeStyle = "rgba(9, 7, 4, 0.92)";
  ctx.fillStyle = "#fff0bd";
  const halfWidth = ctx.measureText(OpeningGateApproachThought).width / 2;
  const x = clamp(drawRect.x + drawRect.w / 2, halfWidth + 14, CANVAS_WIDTH - halfWidth - 14);
  const y = Math.max(28, drawRect.y - 14);
  ctx.strokeText(OpeningGateApproachThought, x, y);
  ctx.fillText(OpeningGateApproachThought, x, y);
  ctx.restore();
}
...
function renderYardBackgroundActors(ctx) {
  ...
  if (!GameState.sideRoute.active) {
    ...
    const entities = [];
    GameState.roamingBackgroundPrisoners
      .filter((prisoner) => prisoner.scene === "yard")
      .forEach((prisoner) => {
        entities.push({
          y: prisoner.y,
          render() {
    ...
    GameState.yardPrisoners
      .filter((prisoner) => !prisoner.scene || prisoner.scene === "yard")
      .forEach((prisoner) => {
        entities.push({
          y: prisoner.y,
          render() {
            const rect = renderYardPrisoner(ctx, prisoner, prisoner);
            if (prisoner.name && rect) {
    ...
    GameState.yardGuards
      .filter((guard) => !guard.scene || guard.scene === "yard")
      .forEach((guard) => {
        entities.push({
          y: guard.y,
          render() {
    ...
    YardLayout.npcs.forEach((npc, index) => {
      ...
      const actor = npc.id === "red" ? GameState.redNpc :
        (npc.id === "brooks" ? GameState.brooksNpc : npc);
      entities.push({
        y: actor.y,
        render() {
          if (npc.id === "red") {
    ...
    const playerFoot = yardWorldToImagePoint(getPlayerFootPoint(GameState.player.x, GameState.player.y));
    entities.push({ y: playerFoot.y, render() { renderPlayer(ctx); } });
    entities.sort((first, second) => first.y - second.y);
    entities.forEach((entity) => entity.render());
    return true;
  }

  if (GameState.sideRoute.backgroundCrowd.length > 0) {
    GameState.sideRoute.backgroundCrowd.forEach((prisoner) => {
      renderYardPrisoner(ctx, prisoner, prisoner);
    });
  }

  GameState.yardPrisoners.filter((prisoner) => !prisoner.scene || prisoner.scene === "yard").forEach((prisoner) => {
  ...
  if (!GameState.sideRoute.active ||
    ...
    GameState.sideRoute.stage === "returnToCellSleep") {
    GameState.yardGuards.filter((guard) => !guard.scene || guard.scene === "yard").forEach((guard) => {
      const rect = renderYardGuard(ctx, guard);
      if (rect) {
        renderYardNameTag(ctx, "狱警", rect.x + rect.w / 2, rect.y - 4);
      }
...
function renderSideRouteYardCustody(ctx) {
  ...
  renderYardNameTag(ctx, "典狱长", wardenRect.x + wardenRect.w / 2, wardenRect.y - 4);
...
function renderYardPrisonerFrame(ctx, assetKey, imageX, imageY, h, fallbackFacing, flip, fallbackAssetKey) {
  ...
  if (!record.loaded) {
    return getPendingCharacterRect(imageX, imageY, h, yardImagePointToScreen);
  }
...
function renderFail(ctx) {
  ...
  } else if (SHOW_GAMEPLAY_TEXT_HINTS && GameState.failRecovery === "pipeOpened") {
    ctx.font = "18px monospace";
    ctx.fillText("右下按钮 / 空格键：回到刚凿开管道后", CANVAS_WIDTH / 2, 410);
...
function getOperatingRoomBlackoutDuration() {
  return getOperatingRoomBlackoutTypewriterDuration() +
    OPERATING_ROOM_BLACKOUT_SECRET_FADE_SECONDS +
    OPERATING_ROOM_BLACKOUT_HOLD_SECONDS;
}
...
function renderOperatingRoomBlackout(ctx) {
  ...
  ctx.textBaseline = "middle";
  renderOperatingRoomBlackoutMessage(
    ctx,
...
function renderOperatingRoomBlackoutMessage(ctx, text, x, y, maxWidth, lineHeight) {
  const lines = [];
  let line = "";
  let lineStart = 0;
  ...
  const secretStart = OPERATING_ROOM_BLACKOUT_MESSAGE.length - OPERATING_ROOM_BLACKOUT_SECRET.length;
  const secretEnd = secretStart + OPERATING_ROOM_BLACKOUT_SECRET.length;
  const typewriterDuration = getOperatingRoomBlackoutTypewriterDuration();
  const fadeElapsed = Math.max(0, GameState.surgerySequence.timer - typewriterDuration);
  const fadeRatio = clamp(fadeElapsed / OPERATING_ROOM_BLACKOUT_SECRET_FADE_SECONDS, 0, 1);
  const easedFadeRatio = fadeRatio * fadeRatio * (3 - 2 * fadeRatio);

  ctx.textAlign = "left";
  lines.forEach((currentLine, lineIndex) => {
    const lineEnd = currentLine.start + currentLine.text.length;
    const secretLocalStart = clamp(secretStart - currentLine.start, 0, currentLine.text.length);
    const secretLocalEnd = clamp(secretEnd - currentLine.start, 0, currentLine.text.length);
    const beforeSecret = currentLine.text.slice(0, secretLocalStart);
    const secret = currentLine.text.slice(secretLocalStart, secretLocalEnd);
    const afterSecret = currentLine.text.slice(secretLocalEnd);
    let drawX = x - ctx.measureText(currentLine.text).width / 2;
    const drawY = y + lineIndex * lineHeight;

    ctx.fillStyle = "#f1eee7";
    if (beforeSecret) {
      ctx.fillText(beforeSecret, drawX, drawY);
      drawX += ctx.measureText(beforeSecret).width;
    }

    if (secret && lineEnd > secretStart) {
      const firstSecretCharacter = currentLine.start + secretLocalStart - secretStart;
      secret.split("").forEach((char, index) => {
        ctx.fillStyle = getOperatingRoomSecretColor(
          firstSecretCharacter + index,
          easedFadeRatio
        );
    ...
    if (afterSecret) {
      ctx.fillStyle = "#f1eee7";
      ctx.fillText(afterSecret, drawX, drawY);
    }
  });
}

function getOperatingRoomSecretColor(characterIndex, fadeProgress) {
  const start = [241, 238, 231];
  const bloodRed = [145, 9, 18];
  const gradientEndIndex = Math.max(1, OPERATING_ROOM_BLACKOUT_SECRET_GRADIENT_CHARACTERS - 1);
  const positionRatio = clamp(characterIndex / gradientEndIndex, 0, 1);
  const exponentialScale = Math.exp(OPERATING_ROOM_BLACKOUT_SECRET_GRADIENT_RATE) - 1;
  const positionProgress = (
    Math.exp(OPERATING_ROOM_BLACKOUT_SECRET_GRADIENT_RATE * positionRatio) - 1
  ) / exponentialScale;
  const progress = positionProgress * fadeProgress;
  const channels = start.map((channel, index) => (
    Math.round(channel + (bloodRed[index] - channel) * progress)
  ));
...
function renderPipeInteractionHints(ctx) {
  if (GameState.pipe.smashCompleted && GameState.pipe.phase === "smash") {
    const exitRect = pipeImageRectToCanvas(PipeLayout.dropZone);
    ctx.save();
    ctx.strokeStyle = "#f5df9d";
    ctx.lineWidth = 4;
    ctx.fillStyle = "rgba(245, 223, 157, 0.13)";
...
function renderPipeVictoryTitle(ctx, alpha) {
  renderPipeVictoryGlowText(ctx, "你成功的脱离了拘禁地", CANVAS_WIDTH / 2, 86, 34, alpha);
}
...
function renderPipeCrawlPlayer(ctx, pipe, x, y) {
  const frameIndex = pipe.isMoving ?
    ...
    0;
  const frame = getRenderableAnimationFrame(PipeCrawlFrames, frameIndex);
...
function renderQuestText(ctx, text, options) {
  const renderOptions = options || {};
  renderNarrativeBox(ctx, text, renderOptions.box || null, renderOptions);
}
...
function createMainGlobalControlsAdapter() {
  return {
    onMove(payload) {
      requestGameBgmStart();
...
function restoreMainGlobalControls() {
  if (!GlobalControls) return;
  GlobalControls.releaseAll();
  GlobalControls.setAdapter(createMainGlobalControlsAdapter());
...
function getGlobalControlContext() {
  ...
  if (GameState.scene === "outsidePlanEscape") {
    return Object.assign({}, hidden, {
      actionVisible: DialogueSystem.active,
      actionEnabled: DialogueSystem.active,
      actionLabel: DialogueSystem.active ? "继续" : "",
      actionHighlighted: DialogueSystem.active
    });
  }
  if (GameState.hammerHidePuzzle.active ||
    isPosterPickupAnimationActive() ||
  ...
  const interactionReady = DialogueSystem.active || OverheadThoughtSystem.isActive() ||
    NarrativeCueSystem.isActive() ||
...
function initGlobalControls() {
  ...
  GlobalControls = window.BeyondWallsControls.mount({
    adapter: createMainGlobalControlsAdapter()
  });
  window.BeyondWallsMainControls = GlobalControls;
}

function stopAllGameAudio() {
  AudioSystem.oneShots.splice(0).forEach(stopAudioElement);
  Object.keys(AudioSystem.loopGroups).forEach((slot) => AudioSystem.stopLoopGroup(slot));
}

function returnToTitleFromEnding() {
  if (window.BeyondWallsEndingHost) {
    window.BeyondWallsEndingHost.close();
  }
  MiniGameHost.close(false);
  stopAllGameAudio();
  CheckpointSystem.clear({ preserveYardMap: false });
  applyCheckpointProgress("CP_START", {});
  resetCheckpointTransientState();
  try {
    ...
    reportNonFatalError("返回标题时清理音乐进度失败", error);
  }
  GameState.playTime = 0;
  GameState.fatalError = null;
  GameState.gameBgmStarted = false;
...
window.BeyondWallsGetAchievementProgress = function () {
  return {
    unlocked: AchievementSystem.getUnlockedCount(),
    total: AchievementDefinitions.length
  };
};
window.BeyondWallsReturnToTitle = returnToTitleFromEnding;

const MobileControls = {
  render(ctx) {
    if (window.BeyondWallsEndingHost && window.BeyondWallsEndingHost.isActive()) {
      return;
    }
...
function renderActionButton(ctx) {
  ...
  const officeInteractionReady = GameState.scene === "office" && Boolean(getActiveOfficeInteraction());
  const pipeInteractionReady = GameState.scene === "pipe" && Boolean(getActivePipeInteraction());
  const interactionReady = OverheadThoughtSystem.isActive() || NarrativeCueSystem.isActive() ||
...
function getContextActionLabel() {
  ...
  if (OverheadThoughtSystem.isActive()) return "继续";
  ...
  if (GameState.scene === "cell") {
    ...
    if (activeCellInteraction && activeCellInteraction.id === "roamingNpc") {
      return "交谈";
    }
    return "";
  }
  if (GameState.scene === "cellCorridor") {
    ...
    const interaction = getActiveCorridorInteraction();
    if (interaction && interaction.id === "doorMemory") return "调查";
    if (interaction && interaction.id === "roamingNpc") return "交谈";
  ...
  if (GameState.scene === "solitary") {
    ...
    if (solitaryInteraction && solitaryInteraction.id === "roamingNpc") return "交谈";
    return "";
  }
  if (GameState.scene === "dig") {
  ...
  if (GameState.scene === "library") {
    ...
    if (activeInteraction && activeInteraction.id === "roamingNpc") return "交谈";
    return "";
  }
  if (GameState.scene === "office" && GameState.office.safeViewOpen) {
...
function gameLoop(timestamp) {
  ...
  const openingNoticeHandled = OpeningNoticeSystem.handleInput();
  const openingNoticeOpen = OpeningNoticeSystem.isActive();
  const tutorialHandled = !openingNoticeOpen && TutorialSystem.handleInput();
  const tutorialOpen = TutorialSystem.isActive();
  const tutorialBlocksWorld = openingNoticeHandled || openingNoticeOpen || tutorialHandled || tutorialOpen;
  const navigationMapHandled = !tutorialBlocksWorld &&
    (YardNavigationMapSystem.handleInput() || PipeNavigationMapSystem.handleInput());
  ...
  const overheadThoughtHandled = !tutorialBlocksWorld && !navigationMapHandled && !navigationMapOpen &&
    !evidenceViewerHandled && OverheadThoughtSystem.handleInput();
  const narrativeCueHandled = !tutorialBlocksWorld && !navigationMapHandled && !navigationMapOpen &&
    !evidenceViewerHandled && !overheadThoughtHandled && NarrativeCueSystem.handleInput();
  if (!tutorialBlocksWorld && !navigationMapHandled && !navigationMapOpen &&
    !evidenceViewerHandled && !overheadThoughtHandled && !narrativeCueHandled) {
    scene.handleInput();
  }
  ...
  if (!tutorialBlocksUpdate && !isEvidenceViewerOpen() && !navigationMapOpen) {
    scene.update(dt);
    updateRoamingNpcSystem(dt);
    updateSideTalkReapproachLocks();
    DialogueSystem.update(dt);
    OverheadThoughtSystem.update(dt);
    updateRedDialogueReward();
    updateBrooksDialogueReward();
    updateSideTalk();
    updateGuardConversation();
  }
  OpeningNoticeSystem.update();
  ...
  scene = Scenes[GameState.scene];
  scene.render(ctx);
  OverheadThoughtSystem.render(ctx);
  ...
  PipeNavigationMapSystem.render(ctx);
...
function handleFatalError(error) {
  ...
  loopStopped = true;
  if (typeof window.BeyondWallsShowFatalError === "function") {
    window.BeyondWallsShowFatalError(error);
  } else {
  ...
  if (ctx) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("呜呜，互动空间出 bug 了，请退出后重新进入试试吧！", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 - 18);
    ctx.fillText("希望可以联系开发者，我们一起把互动空间变得更好！", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 24);
    if (DEBUG_MODE) {
      ctx.font = "16px monospace";
